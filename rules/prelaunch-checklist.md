...
Last checklist audit: 2026-07-19.
...
| Security response readiness | GitHub private vulnerability reporting enabled for `kovojs/kovo`; a named monitored owner or rotation; a successful private-report drill; response-target ownership; and the deployed `security.txt`.  | Security Response Readiness Evidence Ledger below. | pending |
...
| 2026-07-19 | Codex    | GitHub private vulnerability reporting setting and repository security-process artifacts.       | `gh api repos/kovojs/kovo/private-vulnerability-reporting` returned `{"enabled":false}`. Policy and `security.txt` artifacts exist, but private intake, monitored ownership, and the response drill remain unavailable. | pending                                 |

## Runnable Local Checklist

Run these local checks before requesting external sign-off. They only prove the repo evidence packet
...
## Security Response Readiness Evidence Ledger

| Date       | Reviewer | Private intake evidence                                                                           | Monitoring and drill evidence                                            | Published policy evidence                                                                                                                 | Disposition                                                                                                                                          | Status  |
| ---------- | -------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 2026-07-19 | Codex    | GitHub API returned `{"enabled":false}` for `kovojs/kovo`; the intended route is not operational. | No named monitored owner or successful private-report drill is recorded. | `SECURITY.md` records the disabled state and response targets; `site/public/.well-known/security.txt` points readers back to that policy. | Enable private reporting, assign monitored ownership, run the acknowledgement/triage exercise, and verify the deployed well-known URL before launch. | pending |
