...
vitest.browser.config.ts --run packages/runtime/src/index.browser.test.ts`, and
      `pnpm exec tsc --noEmit --pretty false`.
      Additional evidence 2026-06-13: delegated handler import/state/signal integration was split
      out of the broad runtime barrel into
      `packages/runtime/src/delegated-runtime-integration.test.ts`, leaving
      `packages/runtime/src/index.test.ts` focused on loader installation, inline parity, query
      hydration, enhanced mutation bridge, and disposal smoke coverage while preserving SPEC §4.7
      assertions. Same-session evidence: `pnpm exec vitest --run packages/runtime/src/index.test.ts
packages/runtime/src/delegated-runtime-integration.test.ts`, `pnpm exec vitest --run
packages/runtime/src`, `pnpm exec vp check packages/runtime/src/index.test.ts
packages/runtime/src/delegated-runtime-integration.test.ts IMPLEMENT_v1.md
plans/codebase-quality-round2.md`, and `git diff --check`.
      Additional evidence 2026-06-13: `packages/runtime/src/apply-mutation-response.ts` deleted
      the internal `applyMutationResponseBodyToRuntime` body/apply wrapper; DOM apply, enhanced
      mutation apply, broadcast replay, and deferred stream parts now parse transport bodies first
      and converge on `applyMutationResponseChunksToRuntime` as the single decoded mutation apply
      primitive (SPEC §9.1/§9.2). Same-session evidence: `pnpm exec vitest --run
packages/runtime/src/mutation-response.test.ts packages/runtime/src/mutation-apply.test.ts
packages/runtime/src/apply-deferred-stream.test.ts packages/runtime/src/broadcast.test.ts
packages/runtime/src/index-exports.test.ts`, `pnpm exec vitest --run packages/runtime/src`,
      `pnpm exec vitest --config vitest.browser.config.ts --run
packages/runtime/src/index.browser.test.ts`, `pnpm --filter @jiso/runtime run
check:inline-loader`, and `pnpm exec tsc --noEmit --pretty false`.
...
plans/codebase-quality-round2.md`, and `git diff --check`.
      Additional evidence 2026-06-13: quoted source destructured receiver slots such as
      `{ "db": writer }` now use parsed property-name facts for source-mode degradation, so touch
      extraction and query-loader diagnostics emit FW406 instead of dropping the surface or
...
- [x] P8 output format/versioning and agent-answerability acceptance are represented by `fw-explain/v1`/`fw-check/v1` snapshots and commerce graph-answerability tests.
      ...
      Additional evidence 2026-06-13: `@jiso/test/fw-explain-fixtures`
      now owns the SPEC §10.4/§16.5 mutation/query optimistic matrix projection from
      `fw-explain/v1`, including static-invalidation mismatch and unhandled-status facts.
      Commerce source-truth consumes the shared fixture instead of a local projection loop.
      Same-session evidence: `pnpm exec vitest --run packages/test/src/fw-explain-fixtures.test.ts packages/test/src/package-exports.test.ts`,
      `pnpm exec vitest --run examples/commerce/src/source-truth.test.ts`, `pnpm run check:build`,
      and `node --test --test-name-pattern "P10 commerce graph assertions answer behavior mechanically|P10 commerce invalidation is expressed through graph facts|P4 commerce touch graph is a committed generated artifact" tests/fw-check.node.mjs`.
      Additional evidence 2026-06-13: `@jiso/test/graph-fixtures`
      now owns graph static behavior projections for component targets, domains, routes,
      invalidations, optimistic rows, and touch-graph keys. Commerce source-truth and
      `fw-check` consume those facts instead of direct generated-graph object walks.
      Same-session evidence:
...
- [x] P9 `@jiso/test` has pglite-backed harnessing, static-vs-observed write/read verification, FW402/FW403/FW404/FW405/FW407/FW408/FW410 diagnostics, raw pglite interception, and structural optimistic property checks.
      ...
      plus targeted `pnpm exec vp check ...` and `git diff --check`.
      Additional evidence 2026-06-13: `@jiso/test/fw-explain-fixtures`
      now exposes rawless query, mutation, and page assertion facts for SPEC §10.4/§16.5 graph
      answerability. Commerce source-truth and `fw-check` P10/starter gates consume those facts
      instead of locally parsing each `fw-explain/v1` field for consumers, invalidations,
      optimistic status summaries, query reads, page metadata, and stylesheets. Same-session
      evidence:
      `pnpm exec vitest --run packages/test/src/fw-explain-fixtures.test.ts packages/test/src/package-exports.test.ts`,
      `pnpm exec vitest --run examples/commerce/src/source-truth.test.ts`, `pnpm run check:build`,
      and
      ...
      plus targeted `pnpm exec vp check ...` and `git diff --check`.
      Additional evidence 2026-06-13: `@jiso/test/html-fragment`
      now owns reusable form-field maps and keyed-element projections through
      `htmlFormFieldsByName`, `htmlKeyValues`, and `htmlKeyTextMap`; commerce app and
      source-truth tests consume those public fixture seams instead of local HTML projections
      for SPEC §6.3 form rendering and SPEC §10.4/§16.5 keyed mutation fragments. Same-session
      evidence:
      ...
      plus targeted `pnpm exec vp check ...` and `git diff --check`.
      Additional evidence 2026-06-13: `@jiso/test/headers` now owns case-insensitive response
      header value projection and Set-Cookie pair extraction; commerce app and app-shell tests
      consume those public fixture seams instead of local header/cookie parsers. Same-session
      evidence:
...
IMPLEMENT_v1.md plans/ui.md`.
      ...
      contract free of false boolean markup. `/components/field` now renders a disabled native
      fieldset containing a named control so the gallery behavior contract proves disabled
      grouping, not only label/description IDREF wiring. Same-session evidence:
...
- [ ] D8 app shell (request dispatch, document assembly, node adapter, Vite+ plugin, static export) is planned in `plans/app-shell.md`; design agreed 2026-06-11 (lives in `@jiso/server`, web-standard `Request → Response`, closed dispatch table with no middleware, L0/L1-only static export); SPEC §9.5 and S8/R1/R2/R3/R4 are implemented, R5 has dev middleware and manifest/build planning helpers, R6 static export writes HTML, `/c/` modules, and configured static assets, R7 starter adoption is partially proven, commerce now has a shell-backed HTTP document/query/module serve entry including `/`, shared-shell `/_m/` mutation dispatch is proven by commerce enhanced/no-JS HTTP tests, and the docs site ships through `vp run export`; the flat-HTML compatibility layer is no longer the default export path, while R7 remains open for any remaining starter/serve adoption gaps.
      ...
      Evidence 2026-06-13: Vite static-export inventory/manifest dry-run option helpers now
      reject `outDir` with FW229 instead of silently discarding write targets, preserving the
      SPEC §9.5 distinction between export introspection and output-writing export tasks.
...
Runtime closure evidence (2026-06-13): mutation response body parsing is transport-local before decoded chunks enter the shared query/fragment apply path; enhanced submit, broadcast replay, deferred stream chunks, DOM apply, and store-only apply now converge on `applyMutationResponseChunksToRuntime` after deleting the internal body/apply wrapper, while decoded chunk tests keep parser-independent coverage (SPEC §9.1, §9.2, §9.4). Proved by `pnpm exec vitest --run packages/runtime/src/mutation-response.test.ts packages/runtime/src/mutation-apply.test.ts packages/runtime/src/apply-deferred-stream.test.ts packages/runtime/src/broadcast.test.ts packages/runtime/src/index-exports.test.ts`, `pnpm exec vitest --run packages/runtime/src`, `pnpm exec vitest --config vitest.browser.config.ts --run packages/runtime/src/index.browser.test.ts`, `pnpm --filter @jiso/runtime run check:inline-loader`, `pnpm exec tsc --noEmit --pretty false`, and `pnpm exec vp check packages/runtime/src/apply-mutation-response.ts packages/runtime/src/mutation-apply.ts packages/runtime/src/apply-deferred-stream.ts packages/runtime/src/broadcast.ts packages/runtime/src/mutation-response.test.ts IMPLEMENT_v1.md plans/codebase-quality-round2.md`.
...
- **D7 — UI libraries (`plans/ui.md`).** After P5 (all framework prerequisites — composition lowering, merge rules FW231–233, triggers, stamps, isomorphic FW302, `/_q/` — exist; waves verify rather than wait). F-track framework seams first (package prefix registration + FW234 SPEC PR, behavior-attribute namespace, primitive-author `defaultPrevented` lint, platform-coverage audit for anchor positioning/`@starting-style`); then `@jiso/headless-ui` in three waves (L0-heavy → stateful L1 islands → list-driven/isomorphic) with behavior contracts ported from Base UI/APG references. H0 shared state-attribute, keyboard/menu navigation, typeahead, change-detail, and positioning fallback helpers have landed in `@jiso/headless-ui`; H1 is complete with disclosure, collapsible, accordion, dialog, alert-dialog, popover, tooltip, hover-card, separator, progress, meter, avatar, toggle, checkbox, and switch primitives. `@jiso/ui` vendored shadcn-style via `fw add` now has package-sourced TSX catalog output with clean FW235/FW225 compile coverage (**TSX-only vendoring — D9 constraint: vendored components are app source and FW235 applies**); `examples/gallery` is the docs + conformance surface (keyboard/ARIA parity gates, axe, self-baselined visual regression, §4.6 merge goldens) and has a browser-backed compiled interactive seed for toggle/checkbox/disclosure. Deferred by decision: carousel/calendar/resizable (W4, post-v1), Chart (out entirely).
  UI closure evidence (2026-06-13): field controls now carry native constraint/autofill attrs through headless helpers, styled wrappers, and gallery contracts while preserving SPEC §6.3 real named-control form integration.
  UI closure evidence (2026-06-13): menubar submenu item activation now covers Enter, Space, and legacy Spacebar through the headless primitive, disabled/canceled selection restoration, headless exports, TSX-authored gallery handlers, refreshed generated artifacts, and browser-backed compiled gallery behavior (SPEC §4.6, §5.2).
