...
history — and then proves its entire behavior surface without running a browser: `fw check` over
the app graph, `fw explain` as a queryable dependency graph, and harness tests that verify
...
`fw check` is the CI gate over that graph — touch-graph consistency and optimistic exhaustiveness
in one stable, diffable output:

{{snippet:07-verification/src/app.test.ts#fw-check-test}}

## Query the graph with fw explain

`fw explain` prints the compiler's and data plane's decisions as stable text, so agents consume
...
The unguarded audit rides the same surface: `fw explain --unguarded` lists every mutation, route,
and query reachable without `authed`. This app's answer is zero.

## Verify writes in harness tests

`@jiso/test` executes mutations as functions, with write verification on: every observed write
...
SPEC §11.4. `fw check` exhaustiveness and consistency gate: SPEC §10.6. `fw explain` stable text
