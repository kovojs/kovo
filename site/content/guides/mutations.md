...
description: Typed writes over real forms — FormData coercion, CSRF, guards, the enhanced round-trip, and the 422 path.
order: 2
---
...
- **`errors`** declares the failure vocabulary. `context.fail('OUT_OF_STOCK', …)` is typed against
  it, and consumers receive an exhaustive discriminated union (SPEC §6.3, §9.2).
- **`guard`** composes combinators; `authed` refines `req.session` so the user is non-null inside
  the handler, and every mutation joins the `fw explain --unguarded` audit (SPEC §10.3).
...
guards (SPEC §6.6). In app code that means rendering the field:
...
and that posture is visible in the `fw explain --endpoints` audit (SPEC §6.6, §11.4).
...
- **`FW-Targets`** is read off the live DOM's `fw-deps` stamps at submit time. The server holds no
  session of what's on screen — it answers a stateless question (SPEC §9.1). This matters for
  [deployment](/guides/deployment/).
- **`<fw-query>`** chunks replace the client's query values and run each query's update plan across
  every dependent island (SPEC §9.1, §4.8).
...
- **`FW-Changes`** is the sanitized summary of committed writes — `{domain, keys}` only, never
  mutation input or failure detail (SPEC §9.1).
...
(SPEC §9.1, §9.2). You do not write this twice — the server helper renders both modes from one
declaration:
...
Validation failures (schema, with field paths) and declared error codes return HTTP 422 with a
fragment that re-renders the form with messages; the enhanced path morphs just the form, the
no-JS path re-renders the page (SPEC §9.2). The commerce app's out-of-stock failure on the
enhanced path is exactly:
...
one diffable artifact (SPEC §5.3). See [reading fw check & fw explain](/guides/fw-explain/).
