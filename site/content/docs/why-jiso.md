---
title: Why Jiso?
description: What Jiso is for, the trade-offs it makes, and how it compares to Next, Remix, htmx, Qwik, and Astro.
order: 1
---
...
Jiso is built so that rename fails the build instead — at the query that selected the old column,
...
Jiso is a framework for **multi-page apps that are interactive the moment they paint**: commerce,
...
- **People generating apps with AI.** Jiso is designed to be the most checkable target an agent
...
Say a product's price moves from a top-level column to a nested field. Here's the chain Jiso
...
handler each button calls — and stamps it into the HTML. When you open View Source on a Jiso page,
...
Jiso makes sharp choices, and they cost you things. Here's where it's the wrong tool:

- **Long-lived, single-heap client apps.** Figma-class canvases, video editors, DAWs, anything
  built around one mutable client session that lives across navigations — Jiso won't grow a global
...
- **Browser-uniform fancy transitions.** Instant prerendering and invoker commands are
  Chromium-led. Jiso degrades gracefully — real navigations, real forms — but it doesn't polyfill
...
And the honest status line: **Jiso is pre-v1 and not published to npm.** You can build with it today
...
| If you've used…     | What's familiar                                   | What Jiso changes                                                                                                                                                          |
| ------------------- | ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **Astro**           | Islands, MPA, ship-little-JS                      | Same islands-on-an-MPA shape, but Jiso owns the typed data/mutation/invalidation graph across the stack, where Astro stays framework-agnostic and content-first            |

The one-line version: if you like Astro's islands and htmx's wire but want the database-to-DOM
wiring to be a _type error when it's wrong_, that's the gap Jiso fills.

## Next steps

- [Quickstart](/docs/quickstart/) — get a page rendering and see the checks fire.
- [Thinking in Jiso](/docs/mental-model/) — the mental model, built through one small app.
