...
// The left docs rail is sticky and scrollable on desktop. Keep the current
// section/page within that rail after first load and enhanced navigation without
// moving the main document scroll position.
const DOCS_SIDEBAR_SCRIPT = `(()=>{const sync=()=>requestAnimationFrame(()=>{const nav=document.querySelector('[data-docs-sidebar="desktop"]');if(!nav||nav.clientHeight===0)return;const current=nav.querySelector('[data-current-section="true"],[data-current-page="true"]');if(!current)return;const nr=nav.getBoundingClientRect();const cr=current.getBoundingClientRect();if(cr.top>=nr.top&&cr.bottom<=nr.bottom)return;const top=cr.top-nr.top+nav.scrollTop;nav.scrollTop=Math.max(0,top-nav.clientHeight*0.35)});if(document.readyState==='loading')addEventListener('DOMContentLoaded',sync,{once:true});else sync();addEventListener('kovo:navigate',sync)})()`;
...
export const siteDocumentTemplate: DocumentTemplate = ({ parts }) =>
  [
    ...
    `<script>${DOCS_SIDEBAR_SCRIPT}</script>`,
    FONT_PRELOADS,
    parts.head,
