...
describe('gallery llms section', () => {
  it('produces one page per catalog component with mirror + live url', () => {
    ...
    expect(accordion!.mirror).toBe('/components/accordion.md');
    expect(accordion!.url).toBe('/components/accordion/');
    ...
    expect(accordion!.source).toContain(`${ORIGIN}/components/accordion/`);
  });

  it('renders a Components section in the llms.txt index, linking each mirror', () => {
    const index = buildLlmsIndex([buildGalleryLlmsSection(ORIGIN)], { origin: ORIGIN });
    expect(index).toContain('## Components');
    expect(index).toContain(
      `- [Accordion](${ORIGIN}/components/accordion.md): ${galleryComponentCatalog[0]!.summary}`,
    );
    // check-links.mjs extracts each llms.txt link's path and requires a file under
    // dist/ — every component link must therefore be a .md mirror path, not the
    // live HTML route (which has no extension and would resolve to a directory).
    for (const entry of galleryComponentCatalog) {
      expect(index).toContain(`(${ORIGIN}/components/${entry.component}.md)`);
    }
  });
  ...
  it('includes each component body in the llms-full corpus', () => {
    ...
    expect(full).toContain('URL: https://kovo.sh/components/accordion/');
