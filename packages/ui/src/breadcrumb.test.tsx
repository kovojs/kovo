...
describe('BreadcrumbLink href sanitization (SECURITY_FINDINGS.md H3)', () => {
  it('neutralizes a javascript: href to the safe fallback', () => {
    const html = String(
      BreadcrumbLink.definition.render({
        children: 'Account',
  ...
  it('preserves a safe relative href', () => {
    const html = String(
      BreadcrumbLink.definition.render({ children: 'Account', href: '/account' }),
    );
    expect(html).toContain('href="/account"');
  });
  it('omits the href entirely when none is supplied or the link is current', () => {
    expect(String(BreadcrumbLink.definition.render({ children: 'Home' }))).not.toContain('href=');
    expect(
      String(BreadcrumbLink.definition.render({ children: 'Now', current: true, href: '/now' })),
    ).not.toContain('href=');
  });
});
