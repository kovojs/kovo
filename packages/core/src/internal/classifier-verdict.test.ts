...
describe('classifier verdicts', () => {
  it('allows only proven-safe verdicts through the enforcer', () => {
    ...
    expect(() => enforceOrThrow(provenUnsafe(['contacts']), () => new Error('closed unsafe'))).toThrow(
      'closed unsafe',
    );
    expect(() => enforceOrThrow(unproven('parse ambiguity'), () => new Error('closed unproven'))).toThrow(
      'closed unproven',
    );
  });
});
