...
export const diagnosticDefinitions = {
  ...
  KV240: {
    code: 'KV240',
    help: [
      'Would lower to: one query-shape fact per query name and one component-local alias per canonical runtime query for server render, client updates, and binding validation.',
  ...
  KV402: {
    code: 'KV402',
    help: [
      'Would lower to: a mutation whose declared writes and invalidations cover every domain observed by the static and runtime touch graphs.',
  ...
  KV404: {
    code: 'KV404',
    help: [
      'Would lower to: a Drizzle table mapped to one Kovo domain, or an explicit write-only exempt table that no query reads.',
  ...
  KV406: {
    code: 'KV406',
    // SPEC §10.3/§11.2 plus fundamental-fixes-followup-3 DEC-F: keep this static finding
    // blocking where emitted, but describe it as defense-in-depth beside the runtime declared-table choke.
    help: [
      'Would lower to: declared mutation touch metadata plus a managed write handle whose runtime DB policy rejects writes outside the declared tables.',
  ...
  KV416: {
    code: 'KV416',
    help: [
      'Would lower to: a production delta payload whose app-build token covers the full dev render contract and whose delta applies back to the same HTML.',
  ...
  KV418: {
    code: 'KV418',
    help: [
      'Would lower to: a csrf-exempt surface that authenticates by a signature/verifier (e.g. a webhook), not by an ambient browser cookie.',
  ...
  KV428: {
    code: 'KV428',
    help: [
      'Would lower to: an upload served Content-Disposition: attachment + X-Content-Type-Options: nosniff, with the served Content-Type minted from sniffed bytes (server truth), not the client-declared MIME.',
  ...
  KV430: {
    code: 'KV430',
    help: [
      'Would lower to: an s.* wire schema carrying an explicit breadth/depth bound (.max(...)) on every unbounded array/record reachable from an untrusted source.',
  ...
  KV431: {
    code: 'KV431',
    help: [
      'Would lower to: a completeness manifest listing every client module the document references against the integrity/CSP allowlist.',
  ...
  KV432: {
    code: 'KV432',
    help: [
      'Would lower to: a Set-Cookie minted through the typed cookie builder with HttpOnly + Secure(prod) + an explicit SameSite derived from the cookie class.',
  ...
  KV434: {
    code: 'KV434',
    help: [
      'Would lower to: a wire string validator backed by a blessed linear matcher (email/url/uuid/slug) or a compile-visible literal pattern with known exponential structures rejected; runtime also applies an input-size cap, not a CPU bound.',
  ...
  KV437: {
    code: 'KV437',
    help: [
      'Would lower to: a client handler module whose captured imports and same-file module constants are explicitly proven client-safe before emission.',
  ...
  KV445: {
    code: 'KV445',
    help: [
      'Would lower to: a durable task deployment whose preset declares and emits a real JobRunner drainer for every registered task().',
  ...
  KV446: {
    code: 'KV446',
    help: [
      'Would lower to: durable task enqueue/drain operations persisted in the node preset JobRunner store with the same durability contract the app can actually host.',
  ...
  KV449: {
    code: 'KV449',
    help: [
      'Would lower to: a compiler-owned kovo-security-operation-ir/v1 operation with an exact reviewed door plus a kovo-security-semantic-graph/v3 root-binding-to-invocation-to-sink trace.',
