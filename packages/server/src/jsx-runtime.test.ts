...
describe('server jsx runtime', () => {
  ...
  it('renders CSRF for mutationFormAttributes spreads through the retained mutation value', () => {
    ...
    expect(
      validateCsrfToken({ csrf: hiddenInputValue(rendered, 'csrf') }, request, csrf, {
        audience: 'attacker/write',
      }),
    ).toBe(false);
    expect(rendered.match(/name="csrf"/g)).toHaveLength(1);
  });

  it('keeps retained mutation key and csrf fields immutable', () => {
    for (const property of ['key', 'csrf'] as const) {
      ...
      expect(Reflect.set(definition, property, property === 'key' ? 'attacker/write' : true)).toBe(
        false,
      );
      expect(Reflect.deleteProperty(definition, property)).toBe(false);
      expect(
        Reflect.defineProperty(definition, property, {
          configurable: true,
          get: () => (property === 'key' ? 'attacker/write' : true),
        }),
      ).toBe(false);
      expect(html(jsx('form', { ...attributes, children: '' }))).toContain('action="/_m/cart/add"');
    }
  });
  ...
  it('pins anonymous CSRF cookie options before caller mutation', () => {
    const request = { session: { id: 's1' } };
    let reads = 0;
  ...
  it('keeps runtime iframe sources behind the finite reviewed sandbox posture', () => {
    ...
    expect(
      html(
        jsx('iframe', {
          src: trustedUrl('data:text/html,<script>parent.pwned=true</script>', {
  ...
  it('refuses plain script and style element text while preserving trusted raw HTML escapes', () => {
    ...
    expect(
      html(
        jsx('style', {
          rawHtml: trustedHtml('body{color:green}', {
            reason: 'framework server rendering test fixture',
          }),
        }),
      ),
    ).toBe('<style>body{color:green}</style>');
  });
  ...
  it('renders raw HTML sinks only from trusted values', () => {
    ...
    expect(
      html(
        jsx('section', {
          rawHtml: trustedHtml(browserTrustedHtml, {
            reason: 'framework server rendering test fixture',
          }),
        }),
      ),
    ...
    expect(
      html(
        jsx('section', {
          html: trustedHtml('<em>html helper</em>', {
            reason: 'framework server rendering test fixture',
          }),
        }),
      ),
    ).toBe('<section><em>html helper</em></section>');
  });
  ...
  it('renders trusted HTML child values without escaping', () => {
    expect(
      html(
        jsx('section', {
          children: trustedHtml('<kovo-defer></kovo-defer>', {
            reason: 'framework server rendering test fixture',
          }),
        }),
      ),
    ).toBe('<section><kovo-defer></kovo-defer></section>');
  });
  ...
  it('renders only Kovo-minted TrustedUrl values through URL attribute neutralization', () => {
    ...
    expect(
      html(
        jsx('a', {
          href: trustedUrl('javascript:alert(1)', {
            reason: 'framework server rendering test fixture',
          }),
          children: 'vouched',
        }),
      ),
    ).toBe('<a href="javascript:alert(1)">vouched</a>');
  ...
  it('drops TrustedUrl values placed on non-URL attributes at runtime', () => {
    ...
    expect(
      html(
        jsx('button', {
          formaction: trustedUrl('javascript:alert(1)', {
            reason: 'framework server rendering test fixture',
          }),
          children: 'go',
        }),
