...
describe('app diagnostics — prefetch guard gate (bugs-1 F36 / KV419)', () => {
    ...
    const app = createApp({
      routes: [
        route('/dashboard', {
          ...
          prefetch: 'moderate',
          page: () =>
            trustedHtml('<main>dashboard</main>', {
              reason: 'framework server rendering test fixture',
            }),
        }),
      ],
  ...
  it('retains blocking route diagnostics under late Array filter/map replacement', () => {
    ...
    const parameter = route('/products/:id', {
      page: () =>
        trustedHtml('<main>parameter</main>', {
          reason: 'framework server rendering test fixture',
        }),
    });
    const fixed = route('/products/new', {
      page: () =>
        trustedHtml('<main>fixed</main>', { reason: 'framework server rendering test fixture' }),
    });
    ...
    const app = createApp({
      routes: [
        ...
        route('/feed', {
          prefetch: 'moderate',
          page: () =>
            trustedHtml('<main>public feed</main>', {
              reason: 'framework server rendering test fixture',
            }),
        }),
      ],
