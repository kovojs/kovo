...
function snapshotQueryDefinition(
  ...
  if (readDescriptor !== undefined && 'value' in readDescriptor && readDescriptor.value !== undefined) {
...
function runtimeCacheInfluenceClosesPublic<Request>(
  ...
  if (lifecycleRequest !== null && typeof lifecycleRequest === 'object') {
    ...
    if (session !== undefined && (!('value' in session) || session.value !== null && session.value !== undefined)) {
      return true;
    }
...
function queryRequestHeader(value: unknown, name: string): string | undefined {
  if (isNativeRequest(value)) return requestHeader(value, name) ?? undefined;
  if (value === null || typeof value !== 'object') return undefined;
  const descriptor = witnessGetOwnPropertyDescriptor(value, 'headers');
  if (descriptor === undefined || !('value' in descriptor)) return undefined;
  const headers = descriptor.value;
  return isHeaderSource(headers) ? readHeader(headers, name) : undefined;
}
