...
// while `src/auth.tsx` reads `process.env.JISO_CSRF_SECRET` and fails closed if it is
// missing or still the placeholder.
export const csrfSecretEnvVar = 'JISO_CSRF_SECRET';
...
function renderEnvFile(secret: string): string {
  return [
    '# Local development environment for this Jiso app.',
    '# Generated by create-jiso. This file is gitignored; do NOT commit real secrets.',
    '',
    '# CSRF HMAC key. SECURITY: keep this secret and unique per deployment',
    '# (regenerate with `openssl rand -base64 32`). src/auth.tsx fails closed if it is',
