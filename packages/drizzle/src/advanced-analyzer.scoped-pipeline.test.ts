    ...
    const files = [
      pgDatabaseTypes(['select(value?: unknown): { from(table: unknown): Promise<unknown[]> };']),
      {
        ...
        source: [
          ...
          '}, kovo((columns) => ({ domain: "question", key: columns.id })));',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '    return {',
          '      items: await context.db.select({ id: questions.id, score: questions.score, answerCount: questions.answerCount }).from(questions).where(eq(questions.sessionId, sessionId)),',
          '    };',
          '  },',
          '});',
          '',
          ...
          '    const sessionId = context.request?.session?.id;',
          '    if (!sessionId) throw new Error("auth required");',
          '    const items = await context.db.select({ id: questions.id, score: questions.score, answerCount: questions.answerCount }).from(questions).where(eq(questions.sessionId, sessionId));',
          '    const scoreRows = await context.db.select({ value: sum(questions.score) }).from(questions).where(eq(questions.sessionId, sessionId));',
          '    return { items: items, totalScore: Number(scoreRows[0]?.value ?? 0) };',
          '  },',
          '});',
          '',
          'export const postAnswer = mutation("postAnswer", {',
          '  async handler({ targetId }: { targetId: string }, request: MutationRequest) {',
          '    const sessionId = request.session?.id;',
          '    if (!sessionId) throw new Error("auth required");',
          '    await request.db.update(questions).set({ answerCount: questions.answerCount + 1 }).where(and(eq(questions.sessionId, sessionId), eq(questions.id, targetId)));',
          '  },',
          '});',
          '',
          ...
          '    const sessionId = request.session?.id;',
          '    if (!sessionId) throw new Error("auth required");',
          '    await request.db.update(questions).set({ score: questions.score + 1 }).where(and(eq(questions.sessionId, sessionId), eq(questions.id, targetId)));',
          '  },',
          '});',
        ].join('\n'),
      },
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '',
          'interface MutationRequest { db: PgAsyncDatabase<any, any>; tenant: { id: string } }',
          'interface QueryContext { db: PgAsyncDatabase<any, any>; request: { tenant: { id: string } } }',
          ...
          '',
          'function contextTenantId(context: { request: { tenant: { id: string } } }) {',
          '  return context.request.tenant.id;',
          '}',
          'function requestTenantId(request: { tenant: { id: string } }) {',
          '  return request.tenant.id;',
          '}',
          'kovoAnalyzerSummary(contextTenantId, { returns: { kind: "tenant", path: "id" } });',
          ...
          '    return {',
          '      items: await context.db.select({ id: tickets.id, status: tickets.status }).from(tickets).where(and(eq(tickets.tenantId, tenantId), eq(tickets.status, "open"))),',
          '    };',
          '  },',
          '});',
          '',
          ...
          '    await request.db.update(tickets).set({ status: "closed" }).where(and(eq(tickets.tenantId, tenantId), eq(tickets.id, targetId)));',
          '  },',
          '});',
        ].join('\n'),
      },
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          'import { mutation, query } from "@kovojs/server";',
          '',
          'interface MutationRequest { db: PgAsyncDatabase<any, any>; guard: { owner: { id: string } } }',
          'interface QueryContext { db: PgAsyncDatabase<any, any>; request: { guard: { owner: { id: string } } } }',
          'export const documents = pgTable("documents", {',
          '  guardId: text("guard_id").notNull(),',
          '  id: text("id").notNull(),',
          '  status: text("status").notNull(),',
          '}, kovo((columns) => ({ domain: "document", key: [columns.guardId, columns.id] })));',
          '',
          'function contextGuardId(context: { request: { guard: { owner: { id: string } } } }) {',
          '  return context.request.guard.owner.id;',
          '}',
          'function requestGuardId(request: { guard: { owner: { id: string } } }) {',
          '  return request.guard.owner.id;',
          '}',
          'kovoAnalyzerSummary(contextGuardId, { returns: { kind: "guard", path: "owner.id" } });',
          'kovoAnalyzerSummary(requestGuardId, { returns: { kind: "guard", path: "owner.id" } });',
          '',
          'export const openDocuments = query("openDocuments", {',
          '  async load(_input: {}, context: QueryContext) {',
          '    const guardId = contextGuardId(context);',
          '    return {',
          '      items: await context.db.select({ id: documents.id, status: documents.status }).from(documents).where(and(eq(documents.guardId, guardId), eq(documents.status, "open"))),',
          '    };',
          '  },',
          '});',
          '',
          'export const archiveDocument = mutation("archiveDocument", {',
          '  async handler({ targetId }: { targetId: string }, request: MutationRequest) {',
          '    const guardId = requestGuardId(request);',
          '    await request.db.update(documents).set({ status: "archived" }).where(and(eq(documents.guardId, guardId), eq(documents.id, targetId)));',
          '  },',
          '});',
        ].join('\n'),
      },
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          'import { mutation, query } from "@kovojs/server";',
          '',
          'interface MutationRequest { db: PgAsyncDatabase<any, any>; guard: { userId: string } }',
          'interface QueryContext { db: PgAsyncDatabase<any, any>; request: { guard: { userId: string } } }',
          'export const caseFiles = pgTable("case_files", {',
          '  userId: text("user_id").notNull(),',
          '  id: text("id").notNull(),',
          '  title: text("title").notNull(),',
          '  status: text("status").notNull(),',
          '}, kovo((columns) => ({ domain: "case-file", key: [columns.userId, columns.id], owner: columns.userId })));',
          '',
          'function contextGuardUser(context: { request: { guard: { userId: string } } }) {',
          '  return context.request.guard.userId;',
          '}',
          'function requestGuardUser(request: { guard: { userId: string } }) {',
          '  return request.guard.userId;',
          '}',
          'kovoAnalyzerSummary(contextGuardUser, { returns: { kind: "guard", path: "userId" } });',
          ...
          '    return {',
          '      items: await context.db.select({ id: caseFiles.id, title: caseFiles.title, status: caseFiles.status }).from(caseFiles).where(and(eq(caseFiles.userId, userId), eq(caseFiles.status, "active"))),',
          '    };',
          '  },',
          '});',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '  status: text("status").notNull(),',
          '  title: text("title").notNull(),',
          '}, kovo((columns) => ({ domain: "case", key: [columns.userId, columns.id], owner: columns.userId })));',
          '',
          'function contextGuardUser(context: { request: { guard: { userId: string; actorId: string } } }) {',
          '  return context.request.guard.userId;',
          '}',
          'function requestGuardUser(request: { guard: { userId: string; actorId: string } }) {',
          '  return request.guard.userId;',
          '}',
          ...
          '    return {',
          '      items: await context.db.select({ id: cases.id, title: cases.title, status: cases.status }).from(cases).where(and(eq(cases.userId, ownerId), eq(cases.status, "open"))),',
          '    };',
          '  },',
          '});',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          'import { mutation, query } from "@kovojs/server";',
          '',
          'interface MutationRequest { db: PgAsyncDatabase<any, any>; session: { cartId: string } }',
          'interface QueryContext { db: PgAsyncDatabase<any, any>; request: { session: { cartId: string } } }',
          'export const cartItems = pgTable("cart_items", {',
          '  cartId: text("cart_id").notNull(),',
          '  productId: text("product_id").notNull(),',
          '  quantity: integer("quantity").notNull(),',
          '}, kovo((columns) => ({ domain: "cart-item", key: [columns.cartId, columns.productId] })));',
          '',
          'function contextCartId(context: { request: { session: { cartId: string } } }) {',
          '  return context.request.session.cartId;',
          '}',
          'function requestCartId(request: { session: { cartId: string } }) {',
          '  return request.session.cartId;',
          '}',
          'kovoAnalyzerSummary(contextCartId, { returns: { kind: "session", path: "cartId" } });',
          'kovoAnalyzerSummary(requestCartId, { returns: { kind: "session", path: "cartId" } });',
          '',
          'export const cartSummary = query("cartSummary", {',
          '  async load(_input: {}, context: QueryContext) {',
          '    const cartId = contextCartId(context);',
          '    const items = await context.db.select({ productId: cartItems.productId, quantity: cartItems.quantity }).from(cartItems).where(eq(cartItems.cartId, cartId));',
          '    const totalRows = await context.db.select({ value: sum(cartItems.quantity) }).from(cartItems).where(eq(cartItems.cartId, cartId));',
          '    const countRows = await context.db.select({ value: count() }).from(cartItems).where(eq(cartItems.cartId, cartId));',
          '    return {',
          '      items: items,',
          ...
          '});',
          '',
          'export const updateQuantity = mutation("updateQuantity", {',
          '  async handler({ productId, nextQuantity }: { productId: string; nextQuantity: number }, request: MutationRequest) {',
          '    const cartId = requestCartId(request);',
          '    await request.db.update(cartItems).set({ quantity: nextQuantity }).where(and(eq(cartItems.cartId, cartId), eq(cartItems.productId, productId)));',
          '  },',
          '});',
          '',
          'export const removeLine = mutation("removeLine", {',
          '  async handler({ productId }: { productId: string }, request: MutationRequest) {',
          '    const cartId = requestCartId(request);',
          '    await request.db.delete(cartItems).where(and(eq(cartItems.cartId, cartId), eq(cartItems.productId, productId)));',
          '  },',
          '});',
        ].join('\n'),
      },
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '  title: text("title").notNull(),',
          '  status: text("status").notNull(),',
          '}, kovo((columns) => ({ domain: "task", key: columns.id })));',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          'import { mutation, query } from "@kovojs/server";',
          '',
          'interface MutationRequest { db: PgAsyncDatabase<any, any>; session: { id: string } }',
          'interface QueryContext { db: PgAsyncDatabase<any, any>; request: { session: { id: string } } }',
          'export const invoices = pgTable("invoices", {',
          '  sessionId: text("session_id").notNull(),',
          '  id: text("id").notNull(),',
          '  status: text("status").notNull(),',
          '}, kovo((columns) => ({ domain: "invoice", key: [columns.sessionId, columns.id] })));',
          '',
          'function contextSessionId(context: { request: { session: { id: string } } }) {',
          '  return context.request.session.id;',
          '}',
          'function requestSessionId(request: { session: { id: string } }) {',
          '  return request.session.id;',
          '}',
          'kovoAnalyzerSummary(contextSessionId, { returns: { kind: "session", path: "id" } });',
          'kovoAnalyzerSummary(requestSessionId, { returns: { kind: "session", path: "id" } });',
          '',
          ...
          '    return {',
          '      items: await context.db.select({ id: invoices.id, status: invoices.status }).from(invoices).where(eq(invoices.sessionId, sessionId)),',
          '    };',
          '  },',
          '});',
          '',
          'export const markPaid = mutation("markPaid", {',
          '  async handler({ targetId }: { targetId: string }, request: MutationRequest) {',
          '    const sessionId = requestSessionId(request);',
          '    await request.db.update(invoices).set({ status: "paid" }).where(and(eq(invoices.sessionId, sessionId), eq(invoices.id, targetId)));',
          '  },',
          '});',
          '',
