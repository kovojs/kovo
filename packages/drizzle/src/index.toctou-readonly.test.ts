...
describe('KV429 TOCTOU lost-update gate', () => {
  it('flags an unguarded self-referential atomic decrement', () => {
    const result = toctou(
      ATOMIC_SCHEMA,
      buy('  await db.update(products).set({ stock: sql`${products.stock} - ${input.qty}` }).where(eq(products.id, input.id));'),
    );
  ...
  it('discharges with a compare-and-set guard on the atomic column', () => {
    const result = toctou(
      ATOMIC_SCHEMA,
      buy('  await db.update(products).set({ stock: sql`${products.stock} - ${input.qty}` }).where(and(eq(products.id, input.id), eq(products.stock, input.prevStock)));'),
    );
    expect(result).toEqual([]);
  });
  ...
  it('discharges with a version-column guard (optimistic concurrency)', () => {
    const result = toctou(
      ATOMIC_SCHEMA,
      buy('  await db.update(products).set({ stock: sql`${products.stock} - ${input.qty}` }).where(and(eq(products.id, input.id), eq(products.ver, input.prevVer)));'),
    );
    expect(result).toEqual([]);
  });
  ...
  it('does not flag a non-self-referential atomic write (absolute set)', () => {
    const result = toctou(
      ATOMIC_SCHEMA,
      buy('  await db.update(products).set({ stock: input.qty }).where(eq(products.id, input.id));'),
    );
    expect(result).toEqual([]);
  });
  ...
  it('honest ceiling: an opaque (multi-row/range) match is not flagged', () => {
    const result = toctou(
      ATOMIC_SCHEMA,
      buy('  await db.update(products).set({ stock: sql`${products.stock} - ${input.qty}` }).where(sql`${products.stock} > 0`);'),
    );
    expect(result).toEqual([]);
  });
});
...
describe('KV433 read-only query handle (Stage 2: static no-write-reachable)', () => {
  function reach(loaderSource: string) {
    return extractQueryWriteReachabilityFromProject({
      files: [
        writeDbTypes,
        pgDatabaseTypes(['delete(table: unknown): { where(value: unknown): Promise<void> }; insert(table: unknown): { values(value: unknown): Promise<void> };']),
        {
          fileName: 'schema.ts',
          source: 'export const logs = pgTable("logs", { id: text("id").primaryKey() }, kovo({ domain: "log" }));',
        },
