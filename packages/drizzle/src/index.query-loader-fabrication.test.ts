      ...
      files: [
        {
          ...
          source: [
            'export const auditLog = pgTable("audit_log", { message: text("message").notNull() }, kovo((columns) => ({ domain: "audit", key: columns.id })));',
            '',
    ...
    const first = extractTouchGraphFromProject({
      files: [
        ...
        ]),
        {
          fileName: 'schema.ts',
          source: `
            export const items = pgTable("cart_items", {}, kovo((columns) => ({ domain: "cart", key: columns.id })));
          `,
        },
        {
          fileName: 'domain.ts',
