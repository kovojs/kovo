...
describe('@kovojs/drizzle touch graph helpers', () => {
    ...
    const facts = extractQueryFactsFromProject({
      files: [
        pgDatabaseTypes(['select(value?: unknown): { from(table: unknown): Promise<void> };']),
        {
          ...
          source: [
            'import { pgTable as table, text as pgText, integer as pgInteger, type PgAsyncDatabase } from "drizzle-orm/pg-core";',
            '',
    ...
    const files = [
      ...
      },
      {
        ...
        source: [
          ...
          '}, kovo((columns) => ({ domain: "fake", key: columns.id })));',
          '',
      ...
      files: [
        ...
        ]),
        {
          ...
          source: [
            ...
            '',
            'export const auditLogs = pgTable("audit_logs", {}, kovo((columns) => ({ domain: "audit", key: columns.id })));',
            'export const products = pgTable("products", {}, kovo((columns) => ({ domain: "product", key: columns.id })));',
            '',
            'export async function syncCatalog(context: { db: PgAsyncDatabase<any, any>; nested: { tx: PgAsyncDatabase<any, any> }; tuple: [PgAsyncDatabase<any, any>] }, fake: { db: FakeDb }) {',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '}, kovo((columns) => ({ domain: "product", key: columns.id })));',
          '',
          'function readProducts(_input: unknown, db: PgAsyncDatabase<any, any>) {',
          '  return db.select({ id: products.id }).from(products);',
          '}',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '  [loadKey](_input: unknown, db: PgAsyncDatabase<any, any>) {',
          '    return db.select({ id: products.id }).from(products);',
          '  },',
          '});',
        ].join('\n'),
  ...
  it('propagates project static class member local helpers and KV406 surfaces', () => {
    const files = [
      pgDatabaseTypes([
  ...
  it('marks unresolved computed query loaders and domain actions as KV406', () => {
    const files = [
      pgDatabaseTypes([
        'select(value?: unknown): { from(table: unknown): Promise<void> };',
        'update(table: unknown): { set(value: unknown): Promise<void> };',
      ]),
      {
        ...
        source: [
          ...
          '',
          'function addItem(db: PgAsyncDatabase<any, any>) {',
  ...
  it('marks typed project query/domain factories as KV406 when callbacks are not statically visible', () => {
    const files = [
      pgDatabaseTypes([
        'select(value?: unknown): { from(table: unknown): Promise<void> };',
        'update(table: unknown): { set(value: unknown): Promise<void> };',
  ...
  it('keeps wrapped opaque project domain actions visible as KV406', () => {
    const graph = extractTouchGraphFromProject({
      files: [
        {
          fileName: 'product.domain.ts',
  ...
  it('marks project unresolved relational query API table names as KV406', () => {
    const graph = extractTouchGraphFromProject({
      files: [
        pgDatabaseTypes(['query: any;']),
        {
          ...
          source: [
            'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
            '',
            'export async function loadUsers(db: PgAsyncDatabase<any, any>, tableName: string) {',
  ...
  it('marks project aliases with unresolved bases as KV406', () => {
    const graph = extractTouchGraphFromProject({
      files: [
