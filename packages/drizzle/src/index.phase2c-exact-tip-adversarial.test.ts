...
function ownerSource(body: readonly string[]): string {
  return [
    ...
    'import { guards, query, s } from "@kovojs/server"; import { serverValue } from "@kovojs/server/write-safety";',
    ...
    'export const docs = pgTable("docs", { userId: text("user_id").notNull(), id: text("id").notNull() }, kovo((columns) => ({ domain: "doc", key: [columns.userId, columns.id], owner: columns.userId })));',
    ...
    files: [
      ...
      },
      {
        ...
        source: [
          ...
          'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
          'import { kovoAnalyzerSummary } from "@kovojs/drizzle";',
          'import { mutation, s } from "@kovojs/server"; import { serverValue } from "@kovojs/server/write-safety";',
  ...
  it('admits distinct body-verified helpers with the same exact boolean principal', () => {
    const result = ownerVerdict(
      [
        ...
        'export const docs = pgTable("docs", { ownerFlag: boolean("owner_flag").notNull(), id: text("id").notNull() }, kovo((columns) => ({ domain: "doc", key: columns.id, owner: columns.ownerFlag })));',
