...
function ownerSource(body: readonly string[]): string {
  return [
    ...
    'export const docs = pgTable("docs", { userId: text("user_id").notNull(), id: text("id").notNull() }, kovo((columns) => ({ domain: "doc", key: "userId,id", owner: "userId" })));',
  ...
  it('admits distinct body-verified helpers with the same exact boolean principal', () => {
    const result = ownerVerdict(
      [
        ...
        'export const docs = pgTable("docs", { ownerFlag: boolean("owner_flag").notNull(), id: text("id").notNull() }, kovo((columns) => ({ domain: "doc", key: "id", owner: "ownerFlag" })));',
