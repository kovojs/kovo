  ...
  function ordersQueryFile(importLine: string, callee: string, setupLine = ''): SourceFileInput {
    return {
      ...
      source: [
        ...
        '}, kovo((columns) => ({ domain: "order", key: "id", owner: "ownerId", secret: ["secretToken"] })));',
        '',
      ...
      files: [
        pgDatabaseTypes(['query: { posts: { findMany(value?: unknown): Promise<unknown[]> } };']),
        {
          ...
          source: [
            ...
            'export const comments = pgTable("comments", { id: text("id").primaryKey(), postId: text("post_id").notNull(), authorId: text("author_id").notNull() }, kovo((columns) => ({ domain: "comment", key: "id", owner: "authorId" })));',
        ...
        files: [
          ...
          ]),
          {
            ...
            source: [
              ...
              '  id: text("id").primaryKey(),',
              '}, kovo((columns) => ({ domain: "project", key: "id" })));',
              'export const projectNotes = pgTable("project_notes", {',
              '  id: text("id").primaryKey(),',
              '  projectId: text("project_id").notNull(),',
              '}, kovo((columns) => ({ domain: "project-note", key: "id" })));',
