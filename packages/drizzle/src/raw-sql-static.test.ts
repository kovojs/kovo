...
const RAW_SQL_WRITE_SINK_CASES = [
  ...
  {
    dbType: 'SQLiteAsyncDatabase<any, any, any>',
    importLine: 'import type { SQLiteAsyncDatabase } from "drizzle-orm/sqlite-core";',
    method: 'run',
    name: 'sqlite run',
...
function rawWriteSource({
  ...
  return [
    importLine,
    '',
    `export const orders = ${tableFactory}("orders", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "order", key: columns.id, owner: columns.userId })));`,
    '',
...
function rawMutationSource({
  ...
  return [
    importLine,
    '',
    `export const rawOwners = ${tableFactory}("raw_owners", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "raw-owner", key: columns.id, owner: columns.userId })));`,
    '',
      ...
      files: [
        sqliteDatabaseTypes(['run(query: unknown): Promise<void>;']),
        {
          ...
          source: [
            ...
            '',
            'export const orders = sqliteTable("orders", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "order", key: columns.id, owner: columns.userId })));',
            '',
            'export const order = domain({',
        ...
        files: [
          RAW_EXECUTE_DB,
          {
            ...
            source: [
              'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
              '',
              'export const orders = pgTable("orders", { id: text("id").primaryKey(), status: text("status").notNull() }, kovo((columns) => ({ domain: "order", key: columns.id, governed: [columns.status] })));',
              '',
              'export const order = domain({',
        ...
        files: [
          RAW_EXECUTE_DB,
          {
            ...
            source: [
              'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
              '',
              'export const rawOrders = pgTable("raw_orders", { id: text("id").primaryKey(), status: text("status").notNull() }, kovo((columns) => ({ domain: "raw-order", key: columns.id, governed: [columns.status] })));',
              '',
        ...
        files: [
          RAW_EXECUTE_DB,
          {
            ...
            source: [
              'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
              '',
              'export const rawOwners = pgTable("raw_owners", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "raw-owner", key: columns.id, owner: columns.userId })));',
              '',
              'export const addContact = mutation({',
              '  registry: { tables: ["raw_owners"] },',
              '  async handler(input: { id: string }, request: { db: PgAsyncDatabase<any, any> }) {',
