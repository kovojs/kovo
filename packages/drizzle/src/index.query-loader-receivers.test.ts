    ...
    files: [
      ...
      ...(options.files ?? []),
      {
        ...
        source: [
          ...
          '}, kovo((columns) => ({ domain: "reader-proof", key: columns.id })));',
          '',
      ...
      files: [
        {
          ...
          source: [
            ...
            '  passwordHash: text("password_hash").notNull(),',
            '}, kovo((columns) => ({ domain: "account", key: columns.id, owner: columns.ownerId, secret: [columns.passwordHash] })));',
            '',
      ...
      files: [
        {
          ...
          source: [
            ...
            'export const accounts = pgTable("accounts", {',
            '  id: text("id").primaryKey(),',
            '}, kovo((columns) => ({ domain: "account", key: columns.id })));',
      ...
      files: [
        queryReceiverTypes,
        {
          ...
          source: [
            ...
            '}, kovo((columns) => ({ domain: "todo", key: columns.id })));',
            '',
    ...
    const files = [
      ...
      queryReceiverTypes,
      {
        ...
        source: [
          ...
          '  title: text("title").notNull(),',
          '}, kovo((columns) => ({ domain: "note", key: columns.id, owner: columns.userId })));',
          '',
    ...
    const files = [
      kovoServerQueryTypes,
      {
        ...
        source: [
          ...
          '  ownerId: text("owner_id").notNull(),',
          '  title: text("title").notNull(),',
          '}, kovo((columns) => ({ domain: "note", key: columns.id, owner: columns.ownerId })));',
          '',
    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          '',
          'type QueryContext = { db: PgAsyncDatabase<any, any>; request: { guard: { userId: string; actorId: string } } };',
          'function currentGuardUser(context: QueryContext) { return context.request.guard.userId; }',
          'function currentActor(context: QueryContext) { return context.request.guard.actorId; }',
          ...
          'const currentGuardAlias = currentGuardUser;',
          'const computedKey = "current";',
          '',
          'export const directGuardOrders = query("directGuardOrders", {',
          '  guard: guards.all(currentGuardUser),',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.select({ id: orders.id, status: orders.status }).from(orders).where(eq(orders.userId, currentGuardUser(context)));',
          '  },',
          '});',
          '',
          'export const aliasGuardOrders = query("aliasGuardOrders", {',
          '  guard: guards.all(currentGuardAlias),',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.select({ id: orders.id }).from(orders).where(eq(orders.userId, currentGuardAlias(context)));',
          '  },',
          '});',
          '',
          ...
          '  guard: guards.all(currentGuardUser),',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.query.orders.findMany({ columns: { id: true, status: true }, where: (order, { eq }) => eq(order.userId, currentGuardUser(context)) });',
          '  },',
          '});',
          '',
          ...
          '  guard: guards.all(currentGuardUser),',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.query.orders.findMany({ columns: { id: true }, where: (order, { eq, inArray, or }) => or(eq(order.userId, currentGuardUser(context)), inArray(order.userId, [currentGuardUser(context)])) });',
          '  },',
          '});',
          '',
          'export const relationalActorOrders = query("relationalActorOrders", {',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.query.orders.findMany({ columns: { id: true }, where: (order, { eq }) => eq(order.userId, currentActor(context)) });',
          '  },',
          '});',
          '',
          'export const relationalComputedGuardOrders = query("relationalComputedGuardOrders", {',
          '  load(_input: unknown, context: QueryContext) {',
          '    return context.db.query.orders.findMany({ columns: { id: true }, where: (order, { eq }) => eq(order.userId, guardHelpers[computedKey](context)) });',
          '  },',
          '});',
          '',
          'export const relationalClientBranchOrders = query("relationalClientBranchOrders", {',
          '  load(input: { userId: string }, context: QueryContext) {',
          '    return context.db.query.orders.findMany({ columns: { id: true }, where: (order, { eq, or }) => or(eq(order.userId, currentGuardUser(context)), eq(order.userId, input.userId)) });',
          '  },',
          '});',
        ].join('\n'),
      },
