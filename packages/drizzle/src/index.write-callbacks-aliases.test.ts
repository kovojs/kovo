...
describe('@kovojs/drizzle touch graph helpers', () => {
      ...
      files: [
        ...
        ]),
        {
          ...
          source: `
            ...
            const writeAudit = async (db: PgAsyncDatabase<any, any>, productId: string) => {
              await db.insert(auditLog).values({ productId });
            };

            export const cart = domain({
              addItem: write({ touches: [cartItems] }, async (db: PgAsyncDatabase<any, any>, productId: string) => {
      ...
      files: [
        ...
        ]),
        {
          ...
          source: `
            ...
            export async function addItem(writer: PgAsyncDatabase<any, any>, db: FakeDb, productId: string) {
      ...
      files: [
        pgDatabaseTypes(['execute(query: unknown): Promise<void>;']),
        {
          ...
          source: `
            ...
            export async function reconcile(writer: PgAsyncDatabase<any, any>, client: FakeDb) {
  ...
  it('does not promote project receivers from PgAsyncDatabase-like type names', () => {
    const graph = extractTouchGraphFromProject({
      files: [
        pgDatabaseTypes(['execute(query: unknown): Promise<void>;']),
        {
          ...
          source: [
            ...
            'interface RealContext { db: PgAsyncDatabase<any, any> }',
            '',
      ...
      files: [
        ...
        ]),
        {
          ...
          source: [
            ...
            '}',
            '',
            'export const cartItems = pgTable("cart_items", {}, kovo((columns) => ({ domain: "cart", key: "id" })));',
            '',
            'declare const sendAudit: (receiver: unknown) => Promise<void>;',
            '',
            'export async function reconcile(db: PgAsyncDatabase<any, any>, fake: FakeDb) {',
      ...
      files: [
        pgDatabaseTypes(['execute(query: unknown): Promise<void>;']),
        {
          ...
          source: [
            ...
            '',
            'export async function sync(db: PgAsyncDatabase<any, any>, fake: FakeDb, method: string) {',
