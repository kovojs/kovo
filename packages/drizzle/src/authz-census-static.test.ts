    ...
    try {
      ...
      diagnostics = censusDiagnostics(
        [
          ...
          '',
          'export const payroll = pgTable("payroll", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "payroll", key: columns.id })));',
          '',
        ...
        files: [
          DB_TYPES,
          {
            ...
            source: [
              ...
              'import { pgTable, text } from "drizzle-orm/pg-core";',
              '',
              'export const auditLogs = pgTable("audit_logs", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "auditLog", key: columns.id })));',
              '',
    ...
    try {
      ...
      diagnostics = censusDiagnostics(
        [
          ...
          'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
          '',
          'export const drafts = pgTable("drafts", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "draft", key: columns.id })));',
          '',
  ...
  it('requires exactly one DEC-K classification for each request-reachable table', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
        '',
        'export const users = pgTable("users", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "user", key: columns.id, reference: true })));',
        'export const teams = pgTable("teams", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "team", key: columns.id, public: true, reference: true })));',
        '',
  ...
  it('accepts owned, ownedVia, authzPolicy, public, and reference classifications', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        'export const orderItems = pgTable("order_items", { id: text("id").primaryKey(), orderId: text("order_id").notNull() }, kovo((columns) => ({ domain: "orderItem", key: columns.id, ownerVia: { parent: orders, fk: columns.orderId, parentKey: orders.id } })));',
        'export const shares = pgTable("shares", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "share", key: columns.id, authzPolicy: sql`owner_id = current_setting(\\\'kovo.principal\\\', true)` })));',
        'export const posts = pgTable("posts", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "post", key: columns.id, public: true })));',
        '',
  ...
  it('flags a request-reachable FK child table when ownerVia is missing', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        '',
        'export const orders = pgTable("orders", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "order", key: columns.id, owner: columns.userId })));',
        'export const orderItems = pgTable("order_items", { id: text("id").primaryKey(), orderId: text("order_id").notNull().references(() => orders.id) }, kovo((columns) => ({ domain: "orderItem", key: columns.id })));',
        '',
