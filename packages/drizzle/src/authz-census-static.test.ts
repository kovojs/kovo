    ...
    try {
      ...
      diagnostics = censusDiagnostics(
        [
          ...
          '',
          'export const payroll = pgTable("payroll", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "payroll", key: "id" })));',
          '',
        ...
        files: [
          DB_TYPES,
          {
            ...
            source: [
              ...
              'import { pgTable, text } from "drizzle-orm/pg-core";',
              '',
              'export const auditLogs = pgTable("audit_logs", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "auditLog", key: "id" })));',
              '',
    ...
    try {
      ...
      diagnostics = censusDiagnostics(
        [
          ...
          'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
          '',
          'export const drafts = pgTable("drafts", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "draft", key: "id" })));',
          '',
  ...
  it('requires exactly one DEC-K classification for each request-reachable table', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
        '',
        'export const users = pgTable("users", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "user", key: "id", reference: true })));',
        'export const teams = pgTable("teams", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "team", key: "id", public: true, reference: true })));',
        '',
  ...
  it('accepts owned, ownedVia, authzPolicy, public, and reference classifications', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        'export const orderItems = pgTable("order_items", { id: text("id").primaryKey(), orderId: text("order_id").notNull() }, kovo((columns) => ({ domain: "orderItem", key: "id", ownerVia: { parent: orders, fk: "orderId", parentKey: "id" } })));',
        'export const shares = pgTable("shares", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "share", key: "id", authzPolicy: sql`owner_id = current_setting(\\\'kovo.principal\\\', true)` })));',
        'export const posts = pgTable("posts", { id: text("id").primaryKey() }, kovo((columns) => ({ domain: "post", key: "id", public: true })));',
        '',
  ...
  it('flags a request-reachable FK child table when ownerVia is missing', () => {
    const diagnostics = censusDiagnostics(
      [
        ...
        '',
        'export const orders = pgTable("orders", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo((columns) => ({ domain: "order", key: "id", owner: "userId" })));',
        'export const orderItems = pgTable("order_items", { id: text("id").primaryKey(), orderId: text("order_id").notNull().references(() => orders.id) }, kovo((columns) => ({ domain: "orderItem", key: "id" })));',
        '',
