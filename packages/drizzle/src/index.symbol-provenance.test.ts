      ...
      files: [
        ...
        ]),
        {
          ...
          source: [
            ...
            '',
            'export const accounts = pgTable("accounts", {}, kovo((columns) => ({ domain: "account", key: columns.id })));',
            'const accountTable = accounts;',
            '',
            'export async function saveAccount(db: PgAsyncDatabase<any, any>, input: { id: string; ownerId: string }) {',
            '  const id = input.id;',
            '  const { ownerId } = input;',
    ...
    const effects = extractSymbolicEffectsFromProject({
      files: [
        ...
        },
        {
          ...
          source: [
            'import { eq } from "drizzle-orm";',
            'import { alias, type PgAsyncDatabase } from "drizzle-orm/pg-core";',
      ...
      files: [
        pgDatabaseTypes(['update(table: unknown): { set(value: unknown): Promise<void> };']),
        {
          ...
          source: [
            ...
            '}',
            '',
            'export async function saveAccount(db: PgAsyncDatabase<any, any>, input: { ownerId: string }) {',
