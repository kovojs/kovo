    ...
    const files = [
      ...
      ]),
      {
        ...
        source: [
          ...
          'export const cartItems = pgTable("cart_items", {}, kovo((columns) => ({ domain: cartDomain, key: "id" })));',
          'export const invoices = pgTable("invoices", {}, kovo((columns) => ({ domain: billing, key: "id" })));',
          '',
      ...
      files: [
        pgDatabaseTypes(['update(table: unknown): { set(value: unknown): Promise<void> };']),
        {
          ...
          source: [
            ...
            'export const products = pgTable("products", {',
            '  id: text("id").primaryKey(),',
            '}, kovo((columns) => ({ domain: "product", key: "id", fans: [{ via: "price_rules", domain: "price", when: "update" }] })));',
            '',
            'export const renameProduct = async (db: PgAsyncDatabase<any, any>) => {',
            '  await db.update(products).set({ id: "p2" });',
            '};',
      ...
      files: [
        pgDatabaseTypes(['insert(table: unknown): { values(value: unknown): Promise<void> };']),
        {
          ...
          source: [
            ...
            'export const contacts = pgTable("contacts", {}, kovo((columns) => ({ domain: "contact", key: "id" })));',
            '',
