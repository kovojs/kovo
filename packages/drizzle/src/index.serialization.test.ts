    ...
    const files = [
      {
        ...
        source: [
          ...
          '  export class SQLiteAsyncDatabase<TResultKind extends "sync" | "async" = "async", TRunResult = unknown, TRelations = unknown> {}',
          '  export class LibSQLDatabase<TRelations = unknown> {}',
          '  export class BetterSQLite3Database<TRelations = unknown> {}',
          '  export class SQLJsDatabase<TRelations = unknown> {}',
          '  export class BunSQLiteDatabase<TRelations = unknown> {}',
          '}',
          'declare module "drizzle-orm/mysql-core" {',
          '  export class MySqlAsyncDatabase<TQueryResult = unknown, TRelations = unknown> {}',
          '}',
        ].join('\n'),
      },
      {
        ...
        source: [
          ...
          'import { sqliteTable, text, type SQLiteAsyncDatabase, type BetterSQLite3Database, type BunSQLiteDatabase, type LibSQLDatabase, type SQLJsDatabase } from "drizzle-orm/sqlite-core";',
          '',
          'export const products = sqliteTable("products", {',
          '  id: text("id").primaryKey(),',
          '}, kovo({ domain: "product", key: "id" }));',
          '',
          'export async function writeSqlite(db: SQLiteAsyncDatabase, productId: string) {',
          ...
          '  load(_input, db: MySqlAsyncDatabase) {',
    ...
    expect(
      extractQueryFactsFromProject({
        files: [
          ...
          {
            ...sourceFile,
            source: sourceFile.source.replace(
              'load(_input, db)',
