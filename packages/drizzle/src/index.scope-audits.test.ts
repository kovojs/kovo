        ...
        files: [
          pgDatabaseTypes(WRITE_DB_METHODS),
          {
            ...
            source: [
              ...
              '}',
              '',
              'export async function closeMineArrayOfSpreadAlias(db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '  const principals = [req.session.userId] as const;',
              '  await db.update(orders).set({ status: "closed" }).where(inArray(orders.userId, Array.of(...principals)));',
              '}',
              '',
              'export async function closeMineArrayFrom(db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '  await db.update(orders).set({ status: "closed" }).where(inArray(orders.userId, Array.from([req.session.userId] as const)));',
              '}',
              '',
              'export async function closeMineArrayFromAlias(db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '  const principals = [req.session.userId] as const;',
              '  await db.update(orders).set({ status: "closed" }).where(inArray(orders.userId, Array.from(principals)));',
              '}',
              '',
              'export async function closeMineConcatEmpty(db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '  await db.update(orders).set({ status: "closed" }).where(inArray(orders.userId, ([] as string[]).concat(req.session.userId)));',
              '}',
              '',
              'export async function closeMineConcatReceiver(db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '  await db.update(orders).set({ status: "closed" }).where(inArray(orders.userId, ([req.session.userId] as const).concat()));',
              '}',
              '',
        ...
        files: [
          ...
          ]),
          {
            ...
            source: [
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.query.orders.findMany({ columns: { id: true }, where: (order, { eq }) => { const owner = order.userId; const equals = eq; return equals(owner, req.session.userId); } });',
              '  },',
              '});',
              '',
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.query.orders.findMany({ columns: { id: true }, where: (order, { eq }) => { const owner = { userId: order.userId } as const; return eq(owner.userId, req.session.userId); } });',
              '  },',
              '});',
              '',
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.query.orders.findMany({ columns: { id: true }, where: (order, { eq }) => { const owner = order.id; return eq(owner, req.session.userId); } });',
              '  },',
              '});',
              '',
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.query.orders.findMany({ columns: { id: true }, where: (order, ops) => { const equals = ops.eq; return equals(order.userId, req.session.userId); } });',
              '  },',
              '});',
              '',
        ...
        files: [
          ...
          ]),
          {
            ...
            source: [
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.select({ id: orders.id }).from(orders).where(inArray(orders.userId, Array.from([req.session.userId] as const)));',
              '  },',
              '});',
              '',
              ...
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    const principals = [req.session.userId] as const;',
              '    return db.select({ id: orders.id }).from(orders).where(inArray(orders.userId, Array.from(principals)));',
              '  },',
              '});',
              '',
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.select({ id: orders.id }).from(orders).where(inArray(orders.userId, ([] as string[]).concat(req.session.userId)));',
              '  },',
              '});',
              '',
              ...
              '  output: s.object({ id: s.string() }),',
              '  async load(_input: unknown, db: PgAsyncDatabase<any, any>, req: { session: { userId: string } }) {',
              '    return db.select({ id: orders.id }).from(orders).where(inArray(orders.userId, ([req.session.userId] as const).concat()));',
              '  },',
              '});',
              '',
        ...
        files: [
          ...
          ]),
          {
            ...
            source: [
              'import { eq } from "drizzle-orm";',
              'import type { PgAsyncDatabase } from "drizzle-orm/pg-core";',
              'import { guards as serverGuards } from "@kovojs/server";',
              'import { kovoAnalyzerSummary } from "@kovojs/drizzle";',
              '',
              'export const orders = pgTable("orders", { id: text("id").primaryKey(), userId: text("user_id").notNull() }, kovo({ domain: "order", key: (t) => t.id, owner: (t) => t.userId }));',
              'function currentGuardUser(ctx: { guard?: { userId?: string } | null }) { return ctx.guard?.userId; }',
