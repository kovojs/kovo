...
function explainLine(output, prefix) {
  ...
  assert.ok(line, `Missing kovo explain line: ${prefix}`);
...
const cartQuery = kovoExplain(['query', 'cart']);
...
const cartPage = kovoExplain(['page', '/cart']);
assert.equal(explainLine(cartPage, 'prefetch: '), 'false');
assert.match(explainLine(cartPage, 'meta: '), /title=Kovo Starter Cart/);
