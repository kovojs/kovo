...
export async function createStarterServeServer({
  host = '127.0.0.1',
...
if (isMainModule()) {
  const served = await createStarterServeServer(parseCliOptions(process.argv.slice(2)));
  const origin = `http://${served.host}:${served.port}`;

  process.stdout.write(['starter-serve/v1', origin, ''].join('\n'));
...
function parseCliOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--host') {
      ...
      if (!host) throw new Error('Missing value for starter serve option --host.');
      options.host = host;
      index += 1;
      continue;
    }
    ...
    if (arg === '--port') {
      ...
      if (!rawPort) throw new Error('Missing value for starter serve option --port.');
      const port = Number(rawPort);
      if (!Number.isInteger(port) || port < 0 || port > 65_535) {
        throw new Error(`Invalid starter serve port '${rawPort}'.`);
      }
      options.port = port;
      index += 1;
      continue;
    ...
    throw new Error(`Unknown starter serve option '${arg}'.`);
  }
...
function actualPort(server) {
  ...
  throw new Error('Starter serve server did not expose a TCP port.');
}
