...
describe('create-kovo starter (build integration: adversarial production artifact sweep)', () => {
  ...
  it.each([...dialectIndependentCompilerGateCases])(
    'M1:safe-secret-wire permits public-only projections in the %s production artifact',
    async (_label: string, dialect: CreateKovoDialect | undefined) => {
...
function addBugz25ToctouProof(root: string): void {
  ...
  const schema = readFileSync(schemaPath, 'utf8')
    .replace(
      ...
      [
        ...
        'const retainedConcurrencyOutput = {',
        '  concurrency: mutableConcurrency,',
        '  parse(_input: unknown) { return { atomic: mutableConcurrency.atomic }; },',
        '};',
        "export const bugz25RetainedConcurrencyProof = query('bugz25-retained-concurrency', {",
        '  output: retainedConcurrencyOutput,',
        '  load() { return { atomic: mutableConcurrency.atomic }; },',
        '});',
      ].join('\n'),
    )
    .replace(
  ...
  const mutableTable = [
    ...
    '  kovo((columns) => ({',
    '    atomic: columns[mutableConcurrency.atomic],',
    '    domain: "bugz25-mutable",',
    '    key: columns.id,',
    '  })),',
    ');',
    '',
...
function addBugz31AssimilationProof(root: string): void {
  writeFileSync(
    ...
    [
      ...
      '  run() {',
      '    const Reflect = {',
      "      get<T extends { value: unknown }>(target: T, key: 'value'): T['value'] {",
      '        const observed: unknown = globalThis.Reflect.get(target, key);',
      "        if (observed !== target.value) throw new Error('unexpected reflected value');",
      '        return target.value;',
      '      },',
      '    };',
      ...
      '  input: s.object({}),',
      '  run() {',
      '    class Bugz31PromiseCallbackDeferred {',
      '      static then<TResult1 = { ok: true }, TResult2 = never>(',
      '        resolve?: ((value: { ok: true }) => TResult1 | PromiseLike<TResult1>) | null,',
      '        _reject?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null,',
      '      ): never {',
      '        if (resolve) void resolve({ ok: true });',
      "        queueMicrotask(() => { void fetch('https://example.test/late'); });",
      "        throw new Error('bugz-31 settled thenable');",
      '      }',
      '    }',
      '    return Promise.resolve(1).then(() => Bugz31PromiseCallbackDeferred);',
      '  },',
      '});',
      '',
    ].join('\n'),
...
function addBugz31TrustedInputProvenanceProof(root: string): void {
  writeFileSync(
    ...
    [
      ...
      '}',
      '',
      'class Bugz31ConcatCarrier {',
      "  static value = 'safe';",
      '}',
      "Object.defineProperty(Bugz31ConcatCarrier, 'value', { value: Bugz31InputDeferred });",
      '',
      ...
      '  run(input) {',
      '    return input.values.map(() => Bugz31InputDeferred)[0];',
      '  },',
      '});',
      '',
      ...
      '  input: s.object({ values: s.array(s.string()) }),',
      '  run(input) {',
      '    return input.values.concat([Bugz31ConcatCarrier.value])[0];',
      '  },',
      '});',
      '',
      ...
      '  input: s.object({ value: s.string() }),',
      '  run(input) {',
      '    class Bugz31ReplaceProtocol extends RegExp {}',
      '    Object.defineProperty(Bugz31ReplaceProtocol.prototype, Symbol.replace, {',
      '      value() { return Bugz31InputDeferred; },',
      '    });',
      "    const protocol = new Bugz31ReplaceProtocol('safe', 'u');",
      "    return input.value.replace(protocol, 'safe');",
      '  },',
      '});',
      '',
    ].join('\n'),
...
function addBugz31GlobalMemberCarrierProof(
  ...
  const poisonedMembers = [
    "const scheduleBugz31LateEffect = () => { void fetch('https://example.test/late'); };",
    'const poisonedPromiseResolve = new Proxy(Promise.resolve, {',
    '  apply(target, thisArg, args) {',
    '    queueMicrotask(scheduleBugz31LateEffect);',
    '    return Reflect.apply(target, thisArg, args);',
    '  },',
    '});',
    'const poisonedResponseJson = new Proxy(Response.json, {',
    '  apply(target, thisArg, args) {',
    '    queueMicrotask(scheduleBugz31LateEffect);',
    '    return Reflect.apply(target, thisArg, args);',
    '  },',
    '});',
    'const poisonedArrayIsArray = new Proxy(Array.isArray, {',
    '  apply(target, thisArg, args) {',
    '    queueMicrotask(scheduleBugz31LateEffect);',
    '    return Reflect.apply(target, thisArg, args);',
    '  },',
    '});',
    'const poisonedJsonStringify = new Proxy(JSON.stringify, {',
    '  apply(target, thisArg, args) {',
    '    queueMicrotask(scheduleBugz31LateEffect);',
    '    return Reflect.apply(target, thisArg, args);',
    '  },',
  ...
  switch (proof) {
    case 'ordinary-carriers':
      poison = [
        ...
        "Object.defineProperty(promiseHolder.value, 'resolve', { value: poisonedPromiseResolve });",
        'const responseHolder = { value: Response };',
        "Object.defineProperty(responseHolder, 'value', { get: () => Response });",
        "Object.defineProperty(responseHolder.value, 'json', { value: poisonedResponseJson });",
        'const arrayHolder = { value: Array };',
        'Object.assign(arrayHolder, { value: Array });',
        "Reflect.set(arrayHolder.value, 'isArray', poisonedArrayIsArray);",
        'const jsonHolder = { value: JSON };',
        'Reflect.setPrototypeOf(jsonHolder, { value: JSON });',
        "Object.defineProperty(jsonHolder.value, 'stringify', { value: poisonedJsonStringify });",
      ];
      break;
    case 'projection-carriers':
      poison = [
        ...
        'promiseHolder[namespaceKey][memberKey] = poisonedPromiseResolve;',
        'const { value: responseNamespace } = { value: Response };',
        'const { defineProperty: replaceMember } = Object;',
        "replaceMember(responseNamespace, 'json', { value: poisonedResponseJson });",
        'const arrayCarrier: [[ArrayConstructor]] = [[Array]];',
        'arrayCarrier[0][0].isArray = poisonedArrayIsArray;',
        '({ stringify: JSON.stringify } = {',
        '  stringify: poisonedJsonStringify,',
        '});',
      ];
      break;
    case 'array-result-carriers':
      poison = [
        ...deferredClass,
        ...poisonedMembers,
        'const promiseTarget = [Promise].filter(() => true).at(0);',
        "if (promiseTarget === undefined) throw new Error('missing Promise carrier');",
        "Object.defineProperty(promiseTarget, 'resolve', {",
        '  value: poisonedPromiseResolve,',
        '});',
        "Object.defineProperty([Response, Response].reduce((_accumulator, value) => value), 'json', {",
        '  value: poisonedResponseJson,',
        '});',
        "Object.defineProperty([Array, Array].reduceRight((_accumulator, value) => value), 'isArray', {",
        '  value: poisonedArrayIsArray,',
        '});',
        'const jsonTarget = [JSON].findLast(() => true);',
        "if (jsonTarget === undefined) throw new Error('missing JSON carrier');",
        "Object.defineProperty(jsonTarget, 'stringify', {",
        '  value: poisonedJsonStringify,',
        '});',
      ];
      break;
    case 'iterable-binding-carriers':
      poison = [
        ...deferredClass,
        ...poisonedMembers,
        'const promiseTarget = Array.of(Promise).at(0);',
        "if (promiseTarget === undefined) throw new Error('missing Promise iterable carrier');",
        "Object.defineProperty(promiseTarget, 'resolve', {",
        '  value: poisonedPromiseResolve,',
        '});',
        'const [responseNamespace] = new Set([Response]);',
        "if (responseNamespace === undefined) throw new Error('missing Response iterable carrier');",
        "Object.defineProperty(responseNamespace, 'json', { value: poisonedResponseJson });",
        'function replaceArray(...targets: [ArrayConstructor]): void {',
        "  Object.defineProperty(targets[0], 'isArray', { value: poisonedArrayIsArray });",
        '}',
        'replaceArray(Array);',
        'function replaceJson({ target: alias }: { target: JSON }): void {',
        "  Object.defineProperty(alias, 'stringify', { value: poisonedJsonStringify });",
        '}',
    ...
    case 'assignment-targets':
      poison = [
        ...deferredClass,
        ...poisonedMembers,
        '[Promise.resolve] = [poisonedPromiseResolve];',
        '({ nested: { json: Response.json } } = {',
        '  nested: {',
        '    json: poisonedResponseJson,',
        '  },',
        '});',
        'for (Array.isArray of [poisonedArrayIsArray]) { break; }',
        'for (JSON.stringify of [poisonedJsonStringify]) { break; }',
      ];
      break;
    case 'loop-and-exhaustion-targets': {
      ...
      poison = [
        ...
        ...aliases,
        "Object.defineProperty(jsonAlias40, 'stringify', { value: poisonedJsonStringify });",
      ];
      promisePrelude = 'for await (Promise.resolve of [Promise.resolve]) { break; }';
      arrayPrelude =
        'const arrayLoopValues: [[typeof Array.isArray]] = [[Array.isArray]]; for await ([Array.isArray] of arrayLoopValues) { break; }';
      break;
    }
  }
