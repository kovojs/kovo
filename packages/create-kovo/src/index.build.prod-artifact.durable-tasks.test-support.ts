...
export function addDurableTaskProofs(root: string): void {
  ...
  const app = readFileSync(appPath, 'utf8')
    ...
    .replace(
      ...
      'mutations: [addContact, scheduleTaskProof, scheduleCancelledTaskProof, scheduleDelayedTaskProof, scheduleFlakyTaskProof, scheduleReplacementTaskProof, scheduleRollbackTaskProof, scheduleSelfRescheduleTaskProof, signInMutation, signOutMutation],',
    )
    .replace(
...
function writeDurableTaskProofModules(root: string): void {
  writeFileSync(
    ...
    [
      ...
      "import type { MutationHandlerRequest } from '@kovojs/server/data';",
      '',
      ...
      "type MutationDb = MutationHandlerRequest<{ db: AppDb }>['db'];",
      '',
      'async function insertTaskProof(db: MutationDb, eventId: string, proofId: string) {',
