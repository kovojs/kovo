...
import {
  ...
  PRODUCTION_ARTIFACT_TEST_TIMEOUT_MS as TIMEOUT,
} from './index.build.test-support.js';

describe('create-kovo starter (build integration: production runtime contract artifacts)', () => {
  it(
    'serves query warnings and upload MIME sniffing through the production build artifact',
    async () => {
      ...
      try {
        ...
        expect(mutationRefresh.headers.get('kovo-warn')).toContain(
          'QUERY_LIST_LIMIT $.rows;limit=2',
        );
