...
export function addStarterMutationDbScopeProof(
  ...
  const rawWriteMethod = readFileSync(join(root, 'src/schema.ts'), 'utf8').includes('sqliteTable(')
    ? 'run'
    : 'execute';
    ...
    [
      ...
      ...(staticStructured
        ...
        : [
            ...
            '});',
            '',
            '// SPEC §10.3: keep this authorization-bearing transition inside the exact finite fragment;',
            '// omitting registry.tables is the independent runtime table-scope choke under proof.',
  ...
  writeFileSync(
    ...
    [
      ...
      '  render: () => (',
      '    <div hidden>',
      ...proofMutations.flatMap((name) => [
        `      <form {...mutationFormAttributes(${name})}>`,
        '        <input name="marker" />',
        '      </form>',
      ]),
      '    </div>',
...
import { webhook${includeWebhookTransactionProof ? ', createMemoryWebhookReplayStore, webhookReplayIdentity' : ''} } from '@kovojs/server/webhooks';`,
      ...
      "const publicProof = app.publicAccess('public production mutation safety regression proof');",
      "const txProof = domain('tx_proof');",
      'type RuntimeSafetyWriteDb = Parameters<Parameters<typeof app.mutation>[0]["handler"]>[1]["db"];',
      ...
      ...(includeReadonlyRuntimeChokeProbe
        ? [
            'export const readonlyRuntimeChokeProbe = app.query({',
            '  access: publicProof,',
      ...
      ...(includeManagedWriteEscapeAttempt
        ? [
            'export const managedWriteEscapeAttempt = app.mutation({',
            '  access: publicProof,',
            '  input: s.object({ id: s.string() }),',
            "  registry: { tables: ['tx_proofs'], touches: [txProof] },",
            '  async handler(input, request) {',
            "    if ('$client' in request.db && typeof request.db.$client === 'object' && request.db.$client !== null && 'close' in request.db.$client && typeof request.db.$client.close === 'function') {",
            '      const closeRawClient = request.db.$client.close;',
      ...
      ...(includeRawTableDrift
        ? [
            ...
            '  async run(db: RuntimeSafetyWriteDb, input: { id: string; label: string }) {',
        ...
        : []),
      'export const failAfterWrite = app.mutation({',
      '  access: publicProof,',
      '  input: s.object({ id: s.string() }),',
      ...(includeReadonlyRuntimeChokeProbe
        ? ["  optimistic: [readonlyRuntimeChokeProbe.optimistic('await-fragment')],"]
        : []),
      "  registry: { tables: ['tx_proofs'], touches: [txProof] },",
      '  async handler(input, request) {',
      `    await request.db.insert(txProofs).values({ id: trustedAssign(input.id, { evidence: { digest: 'sha256:50bdcf18fca1a51200dab11f42ace64b20ae8d42194762c00cd14f9e0596ad73', kind: 'test', reference: 'starter-tests/transaction-proof-id' }, invariant: 'governed-write.authorized-principal', why: { kind: 'policy', policy: 'starter.transaction-proof-id/v1' } }) });`,
      "    throw new Error('rollback proof');",
      '  },',
      '});',
      '',
      'export const writeTxProof = app.mutation({',
      '  access: publicProof,',
      '  input: s.object({ id: s.string() }),',
      ...(includeReadonlyRuntimeChokeProbe
        ? ["  optimistic: [readonlyRuntimeChokeProbe.optimistic('await-fragment')],"]
      ...
      ...(includeWebhookTransactionProof
        ? [
            "export const txProofWebhook = app.endpoint(webhook('/webhooks/tx-proof', {",
            '  access: publicProof,',
      ...
      ...(includeWebhookTxEscapeAttempt
        ? [
            "export const webhookTxEscapeAttempt = app.endpoint(webhook('/webhooks/tx-escape', {",
            '  access: publicProof,',
            '  input: webhookTxProofInput,',
            "  verify: 'none',",
            "  verifyJustification: 'local production webhook transaction proof fixture',",
      ...
      ...(includeRawTableDrift
        ? [
            'export const rawTableDrift = app.mutation({',
            '  access: publicProof,',
      ...
      ...(includeSqliteAuthorizerTriggerDrift
        ? [
            ...
            '}',
            '',
            'export const sqliteAuthorizerTriggerDrift = app.mutation({',
            '  access: publicProof,',
            '  errors: { RUNTIME_TABLE_DRIFT: runtimeTableDriftError },',
            '  input: s.object({ id: s.string(), label: s.string() }),',
        ...
        : []),
      "export const txProofCountEndpoint = app.endpoint('/api/tx-proof-count', {",
      '  access: publicProof,',
      ...
      '});',
      '',
      "export const rawRuntimeDriftCountEndpoint = app.endpoint('/api/raw-runtime-drift-count', {",
      '  access: publicProof,',
      ...
      ...(includeReadonlyMutationAttempt
        ? [
            ...
            '}',
            '',
            'function readonlyProperty(value: unknown, property: string): unknown {',
            "  if ((typeof value !== 'object' && typeof value !== 'function') || value === null) {",
            '    return undefined;',
            '  }',
            "  if (property === 'execute' && 'execute' in value) return value.execute;",
            "  if (property === 'run' && 'run' in value) return value.run;",
            "  if (property === 'all' && 'all' in value) return value.all;",
            "  if (property === 'get' && 'get' in value) return value.get;",
            "  if (property === 'values' && 'values' in value) return value.values;",
            "  if (property === 'transaction' && 'transaction' in value) return value.transaction;",
            "  if (property === '$client' && '$client' in value) return value.$client;",
            "  if (property === 'session' && 'session' in value) return value.session;",
            "  if (property === 'futureStatement' && 'futureStatement' in value) return value.futureStatement;",
            '  return undefined;',
            '}',
            '',
            'async function attemptReadonlySqlMethod(readonlyDb: unknown, method: string): Promise<ReadonlyAttemptResult> {',
            '  try {',
            '    const sqlMethod = readonlyProperty(readonlyDb, method);',
            "    if (typeof sqlMethod !== 'function') {",
            '      throw new TypeError(`read-only handle does not expose ${method}`);',
            '    }',
            ...
            '}',
            '',
            'async function attemptReadonlyTransaction(readonlyDb: unknown): Promise<ReadonlyAttemptResult> {',
            '  try {',
            "    const transaction = readonlyProperty(readonlyDb, 'transaction');",
            "    if (typeof transaction !== 'function') {",
            "      throw new TypeError('read-only handle does not expose transaction');",
            '    }',
            ...
            '}',
            '',
            'async function attemptReadonlyDeniedProperty(readonlyDb: unknown, method: string): Promise<ReadonlyAttemptResult> {',
            '  try {',
            '    const denied = readonlyProperty(readonlyDb, method);',
            ...
            '}',
            '',
            "export const readonlyMutationAttemptEndpoint = app.endpoint('/api/readonly-mutation-attempt', {",
            '  access: publicProof,',
            "  auth: { justification: 'public readonly handle mutation proof', kind: 'none' },",
            '  csrf: false,',
            "  csrfJustification: 'GET endpoint proves the app-scoped read handle cannot mutate',",
            '  db: true,',
            '  async handler(_request, context) {',
            "    const scoped = await context.actAs('public-production-readonly-handle-proof');",
            '    const readonlyDb = scoped.db.read;',
            '    const results = [',
            `      await attemptReadonlySqlMethod(readonlyDb, '${rawRuntimeDriftMethod}'),`,
            "      await attemptReadonlySqlMethod(readonlyDb, 'all'),",
            "      await attemptReadonlySqlMethod(readonlyDb, 'get'),",
            "      await attemptReadonlySqlMethod(readonlyDb, 'values'),",
            '      await attemptReadonlyTransaction(readonlyDb),',
            "      await attemptReadonlyDeniedProperty(readonlyDb, '$client'),",
            "      await attemptReadonlyDeniedProperty(readonlyDb, 'session'),",
            "      await attemptReadonlyDeniedProperty(readonlyDb, 'futureStatement'),",
            '    ];',
  ...
  const app = readFileSync(appPath, 'utf8')
    ...
    .replace(
      ...
      `  endpoints: [${runtimeSafetyEndpoints.join(', ')}],`,
    )
    .replace(
      '  mutations: [addContact, signInMutation, signOutMutation],',
    ...
    .replace(
      'const assembledApp = app.assemble({',
      [
        "const runtimeSafetyProofFormsRoute = app.route('/runtime-safety-proof-forms', {",
        "  access: app.publicAccess('public runtime safety CSRF regression proof'),",
        '  page() {',
    ...
    .replace(
      '  routes: [homeRoute, loginRoute],',
      '  routes: [homeRoute, loginRoute, runtimeSafetyProofFormsRoute],',
    );
  writeFileSync(appPath, app, 'utf8');
  formatGeneratedSources(root, [
    'src/_kovo/app-runtime-db.ts',
    'src/app.tsx',
    'src/runtime-safety-proof-forms.tsx',
    'src/runtime-safety-proofs.ts',
    'src/schema.ts',
  ]);
}
...
export function addRuntimeSecretBoundaryProof(root: string): void {
  ...
  queries = [
    ...
    [
      ...
      '}',
      '',
      'const runtimeSecretIdLabelResultSchema = s.object({',
      '  items: s.array(s.object({ id: s.string(), label: s.string() })),',
      '});',
      'const runtimeSecretIdLeakedResultSchema = s.object({',
      '  items: s.array(s.object({ id: s.string(), leaked: s.string() })),',
      '});',
      'const runtimeSecretIdLabelLeakedResultSchema = s.object({',
      '  items: s.array(s.object({ id: s.string(), label: s.string(), leaked: s.string() })),',
      '});',
      '// These migration-seeded read-only proof tables have no app mutation invalidation domain, so KV407 requires empty query reads.',
      'export const runtimeSecretColumnEngineDenialQuery = app.query({',
      '  access: [app.authenticated],',
      '  output: runtimeSecretIdLeakedResultSchema,',
      '  reads: [],',
      ...
      '  access: [app.authenticated],',
      '  output: runtimeSecretIdLabelLeakedResultSchema,',
      '  reads: [],',
      '  async load(_input, context) {',
      '    const db = context?.db;',
      '    const items = await db',
      ...
      '  access: [app.authenticated],',
      '  output: runtimeSecretIdLabelResultSchema,',
      '  reads: [],',
      '  async load(_input, context) {',
      '    const db = context?.db;',
      '    const items = await db',
      ...
      '    return {',
      '      items: [{',
      "        id: 'runtime-secret-box',",
      '        // @ts-expect-error SPEC §6.6: this negative fixture deliberately exercises the runtime Secret wire boundary.',
      '        leaked: boxed,',
      '      }],',
      '    };',
      '  },',
...
export function addSqliteRuntimeSecretProvenanceProof(root: string): void {
  ...
  queries = [
    ...
    [
      ...
      '    });',
      '    const items = await context.db.rawRead<SqliteSecretRows["items"][number]>(statement, { reads: ["runtime_secret_proof"] });',
      '    return { items };',
      '  },',
      '});',
      '',
...
export function addParanoidPhase5WriteBoundaryProof(root: string): void {
  writeFileSync(
    join(root, 'src/paranoid-phase5-capability-closure-proof.tsx'),
    [
      '/** @jsxImportSource @kovojs/server */',
      "import { route } from '@kovojs/server';",
      '',
      'let mutableFactory = route;',
      "export const mutableParanoidRoute = mutableFactory('/paranoid-phase5-mutable-root', {",
      '  page() {',
      '    return <p>closed</p>;',
      '  },',
      '});',
  ...
  writeFileSync(
    ...
    [
      ...
      "import { contacts } from './schema.js';",
      '',
      "const publicProof = app.publicAccess('public phase 5.1 write boundary proof');",
      ...
      'const proofInput = s.object({ marker: s.string() });',
      'type Phase5WriteDb = Parameters<Parameters<typeof app.mutation>[0]["handler"]>[1]["db"];',
      "const sqliteMaster = sqliteTable('sqlite_master', {",
      "  name: text('name'),",
      "  type: text('type'),",
      '});',
      "const phase5UntypedContacts = sqliteTable('contacts', {",
      "  company: text('company').$type<unknown>(),",
      "  email: text('email').notNull(),",
      "  id: text('id').primaryKey(),",
      "  name: text('name').notNull(),",
      '});',
      '',
      'async function phase5DdlWriteRun(db: Phase5WriteDb) {',
      '    await db.run(',
      '      trustedSql(',
      ...
      '}',
      '',
      'export const phase5DdlWriteProof = app.mutation({',
      '  access: publicProof,',
      '  input: proofInput,',
      ...
      '  input: { marker: string },',
      ') {',
      '    await db.insert(phase5UntypedContacts).values({',
      "      company: secret('phase5-builder-secret'),",
      ...
      '}',
      '',
      'export const phase5BoxedSecretBuilderWriteProof = app.mutation({',
      '  access: publicProof,',
      '  input: proofInput,',
      ...
      '    await db.run(',
      '      trustedSql(',
      "        sql`insert into \"contacts\" (id, name, email, company) values (${serverValue(`${input.marker}-raw-secret`, 'phase 5.1 boxed secret raw id')}, ${'blocked boxed secret raw write'}, ${`${input.marker}-raw-secret@example.com`}, ${secret('phase5-raw-secret')})`,",
      "        { justification: 'phase 5.1 boxed secret raw write rejection proof' },",
      '      ),',
      '    );',
      '}',
      '',
      'export const phase5BoxedSecretRawWriteProof = app.mutation({',
      '  access: publicProof,',
      '  input: proofInput,',
      "  optimistic: [contactsQuery.optimistic('await-fragment')],",
      "  registry: { tables: ['contacts'], touches: [contact] },",
      ...
      '}',
      '',
      'export const phase5GovernedMassAssignmentProof = app.mutation({',
      '  access: publicProof,',
      '  input: proofInput,',
      "  optimistic: [contactsQuery.optimistic('await-fragment')],",
      "  registry: { tables: ['contacts'], touches: [contact] },",
      ...
      '});',
      '',
      "export const phase5WriteBoundaryStatusEndpoint = app.endpoint('/api/phase5-write-boundary-proof', {",
      '  access: publicProof,',
      ...
      '  db: true,',
      '  async handler(_request, context) {',
      "    const scoped = await context.actAs('public-phase5-write-boundary-proof');",
  ...
  formatGeneratedSources(root, [
    'src/app.tsx',
    'src/paranoid-phase5-capability-closure-proof.tsx',
    'src/paranoid-phase5-write-boundary-proof.ts',
    'src/paranoid-phase5-write-proof-forms.tsx',
  ]);
}
...
export function addParanoidPhase5AuthorizationProof(root: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '',
      "const publicProof = app.publicAccess('public phase 5.1 authorization dogfood proof');",
      ...
      '});',
      '',
      'export const phase5AuthzBuilderQuery = app.query({',
      '  access: [app.authenticated],',
      ...
      '});',
      '',
      'export const phase5AuthzAliasQuery = app.query({',
      '  access: [app.authenticated],',
      '  output: authzRowsSchema,',
      '  reads: [phase5AuthzOrderDomain],',
      '  async load(_input, context) {',
      ...
      '});',
      '',
      'export const phase5AuthzCompoundQuery = app.query({',
      '  access: [app.authenticated],',
      '  output: authzRowsSchema,',
      '  reads: [phase5AuthzOrderDomain],',
      '  async load(_input, context) {',
      ...
      '});',
      '',
      'export const phase5AuthzChildQuery = app.query({',
      '  access: [app.authenticated],',
      '  output: authzRowsSchema,',
      ...
      '      .from(phase5AuthzItems);',
      '    return { items: rows };',
      '  },',
      '});',
      '',
      'export const phase5AuthzGraphProof = app.mutation({',
      '  access: publicProof,',
      '  input: proofInput,',
      '  optimistic: [',
      "    phase5AuthzAliasQuery.optimistic('await-fragment'),",
      "    phase5AuthzBuilderQuery.optimistic('await-fragment'),",
      "    phase5AuthzCompoundQuery.optimistic('await-fragment'),",
      "    phase5AuthzChildQuery.optimistic('await-fragment'),",
      '  ],',
      ...
      '});',
      '',
      "export const phase5AuthzEndpoint = app.endpoint('/api/phase5-authz-endpoint', {",
      '  access: publicProof,',
      ...
      '      .select({ id: phase5AuthzOrders.id, label: phase5AuthzOrders.label })',
      '      .from(phase5AuthzOrders);',
      '    const childRows = await scoped.db.read',
      '      .select({ id: phase5AuthzItems.id, label: phase5AuthzItems.label })',
      '      .from(phase5AuthzItems);',
      ...
      '});',
      '',
      "export const phase5AuthzStatusEndpoint = app.endpoint('/api/phase5-authz-status', {",
      '  access: publicProof,',
      ...
      '    try {',
      '      await db.rawRead<{ classified: string }>(',
  ...
  formatGeneratedSources(root, [
    'src/_kovo/app-runtime-db.ts',
    'src/app.tsx',
    'src/paranoid-phase5-authz-proof.ts',
    'src/schema.ts',
  ]);
}
...
function formatGeneratedSourcesWithPinnedFormatter(root: string, files: readonly string[]): void {
  ...
  // starter format options whose source anchor was checked above. Keep Oxfmt's normal nested-config
  // semantics because the project-quality gate uses the same semantics; otherwise generated source
  // can be formatted into a different fixed point than the immediately following build accepts.
  const formatter = resolveVitePlusQualityBin('oxfmt');
  const formatterConfigName = `.kovo-fixture-formatter-${randomBytes(8).toString('hex')}.oxfmtrc.json`;
  const formatterConfigPath = join(root, formatterConfigName);
  writeFileSync(
    formatterConfigPath,
    `${JSON.stringify({
      ...generatedStarterFormatConfig,
      ignorePatterns: [formatterConfigName],
    })}\n`,
    'utf8',
  );
  try {
    // Oxfmt 0.52 can split a long fluent expression on its first pass and then compact that
    // normalized expression on its second pass. Project quality checks the latter fixed point, so
    // fixture generation must converge before handing source to `kovo build`.
    for (let pass = 0; pass < 2; pass += 1) {
      execFileSync(
        process.execPath,
        [formatter.executable, '--config', formatterConfigPath, '--threads=1', '--write', ...files],
        {
          cwd: root,
          env: {
            ...process.env,
