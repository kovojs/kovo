...
export function addPostgresParanoidPhase5DogfoodProof(root: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '    const dbQueryRows = await (db as any).query.phase5PgOrders.findMany({ columns: { id: true, label: true } });',
      '    const owned = alias(phase5PgOrders, "phase5_pg_endpoint_alias");',
      '    const aliasRows = await db.select({ id: owned.id, label: owned.label }).from(owned);',
      '    const childRows = await db.select({ id: phase5PgItems.id, label: phase5PgItems.label }).from(phase5PgItems);',
