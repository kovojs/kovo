...
export function addPostgresParanoidPhase5DogfoodProof(root: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      'const proofInput = s.object({ marker: s.string() });',
      'const eventInput = s.object({ label: s.string() });',
      ...
      '  return (await (db as any).rawRead(',
      ...
      "  key: 'phase5-pg/event-write', tables: ['phase5_pg_events'], touches: [eventDomain],",
      '  async run(db: AppRequest["db"], input: { label: string }) {',
      '    await db.insert(phase5PgEvents).values({ id: serverValue("phase5-pg-task-event", "phase 5 postgres event id"), label: input.label }).onConflictDoNothing();',
      '  },',
      '});',
      '',
      'export const phase5PgTaskRecord = mutation({',
      '  access: publicProof, csrf: false, input: eventInput,',
      "  registry: { tables: ['phase5_pg_events'], touches: phase5PgEventWrite.touches },",
      '  async handler(input: { label: string }, request: AppRequest) {',
      '    await phase5PgEventWrite.run(request.db, input);',
      '    return { ok: true };',
      '  },',
      '});',
      '',
      ...
      '    await context.actAs("demo-user").runMutation(phase5PgTaskRecord, { label: result.items.map((row) => row.label).join(",") });',
      '  },',
      '});',
      '',
      ...
      '    return Response.json({ aliasRows, childRows, rows, viewRows }, { headers: { "Cache-Control": "no-store" } });',
      '  },',
      ...
      '    try {',
      '      await (scoped.db.write as any).insert(phase5PgOrders).values({ classified: "own-secret", id: serverValue("phase5-pg-endpoint-own", "phase 5 postgres endpoint own write id"), label: "own-write-visible", userId: "demo-user" }).onConflictDoNothing();',
      '      ownWrite = true;',
      '    } catch {}',
      '    try {',
      '      await (scoped.db.write as any).insert(phase5PgOrders).values({ classified: "cross-secret", id: serverValue("phase5-pg-endpoint-cross", "phase 5 postgres endpoint cross write id"), label: "cross-owner-write", userId: "other-user" });',
      '    } catch { crossOwnerDenied = true; }',
      '    try {',
      '      await (scoped.db.write as any).insert(verification).values({ id: serverValue("phase5-pg-endpoint-verification", "phase 5 postgres endpoint verification write id"), identifier: "phase5", value: "blocked", expiresAt: new Date(), createdAt: new Date(), updatedAt: new Date() });',
      '    } catch { verificationDenied = true; }',
      '    return Response.json({ crossOwnerDenied, ownWrite, verificationDenied }, { headers: { "Cache-Control": "no-store" } });',
      '  },',
      "  method: 'POST', reason: 'phase 5 postgres write boundary proof', response: { appOwnedSafety: true, body: 'json', cache: 'no-store' },",
      '});',
      '',
      ...
      '    const events = await readonlyAppDb.select({ id: phase5PgEvents.id, label: phase5PgEvents.label }).from(phase5PgEvents).where(eq(phase5PgEvents.id, "phase5-pg-task-event"));',
      '    let secretReadBlocked = false;',
      '    let verificationDenied = false;',
      '    try {',
      ...
      '    } catch { verificationDenied = true; }',
      '    return Response.json({ events, readonlyRows, secretReadBlocked, verificationDenied }, { headers: { "Cache-Control": "no-store" } });',
      '  },',
      ...
      '  async handler(input, context) {',
      '    await context.actAs("demo-user").runMutation(phase5PgTaskRecord, { label: "owner-visible" });',
      '    return { ok: true };',
      '  },',
      '});',
      '',
  ...
  let app = readFileSync(appPath, 'utf8');
  const importLine =
    "import { phase5PgAliasQuery, phase5PgBuilderQuery, phase5PgDbQueryQuery, phase5PgEndpoint, phase5PgOwnerViaQuery, phase5PgRawQuery, phase5PgReadTask, phase5PgScheduleTask, phase5PgStatusEndpoint, phase5PgSubqueryQuery, phase5PgUnionQuery, phase5PgViewQuery, phase5PgWebhook, phase5PgWriteBoundaryEndpoint } from './paranoid-phase5-postgres-proof.js';";
  app = replaceRequired(
    app,
    "import { appTheme } from './theme.js';",
  ...
  for (const mutationEntry of ['phase5PgScheduleTask']) {
