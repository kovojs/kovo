...
export function addSqliteRuntimeSecretProvenanceProof(root: string): void {
  ...
  queries = [
    ...
    [
      ...
      '}',
      '',
      'const sqliteSecretRowsSchema = s.json<SqliteSecretRows>();',
      '',
      ...
      '    });',
      '    const items = await db.rawRead<SqliteSecretRows["items"][number]>(statement, { reads: ["runtime_secret_proof"] });',
      '    return { items };',
      '  },',
      '});',
      '',
  ...
  mutations = replaceRequired(
    ...
    '  optimistic: [\n    sqliteSecretMixedChunkBuilderQuery.optimistic(addContactInput, (value) => ({ ...value, items: [...value.items] })),\n    contactsQuery.optimistic(addContactInput, (value, input) => {',
