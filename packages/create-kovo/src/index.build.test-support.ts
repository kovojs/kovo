...
export function addParanoidPhase5AuthorizationProof(root: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '',
      'function requirePhase5AuthzDb(context?: Phase5AuthzQueryContext): Reader<AppDb> {',
      '  const db = context?.db;',
      '  if (!db) throw new Error("missing phase 5.1 authorization query db");',
      '  return db;',
      '}',
      '',
      ...
      '    const db = requirePhase5AuthzDb(context);',
      '    const rows = (await db',
...
export function addPostgresParanoidPhase5DogfoodProof(root: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '',
      'function requireDb(context?: Phase5PgQueryContext): Reader<AppDb> {',
      '  const db = context?.db;',
      '  if (!db) throw new Error("missing phase 5 postgres query db");',
      '  return db;',
      '}',
      '',
      ...
      '      return Response.json({ rows }, { headers: { "Cache-Control": "no-store" } });',
      '    } else if (family === "subquery-in-from") {',
      ...
      '    try {',
      '      await readonlyAppDb.rawRead<{ id: string }>(trustedSql(sql.raw("select id from verification"), { justification: "phase 5 postgres status auth-table denial proof" }), { reads: ["verification"] });',
      '    } catch { verificationDenied = true; }',
...
export function addPostgresParanoidFollowup8Shapes(root: string): void {
  ...
  app = replaceRequired(
    app,
    "import { phase5PgAliasQuery, phase5PgBuilderQuery, phase5PgCrossOwnerOrderWriteProof, phase5PgCteQuery, phase5PgDbQueryQuery, phase5PgEndpoint, phase5PgFunctionBoundaryEndpoint, phase5PgFunctionQuery, phase5PgJoinQuery, phase5PgOwnerViaQuery, phase5PgOwnOrderWriteProof, phase5PgRawCrossOwnerOrderWriteProof, phase5PgRawQuery, phase5PgReadTask, phase5PgScheduleTask, phase5PgStatusEndpoint, phase5PgSubqueryQuery, phase5PgUnionQuery, phase5PgUnknownRelationEndpoint, phase5PgViewQuery, phase5PgWebhook, phase5PgWriteBoundaryEndpoint } from './paranoid-phase5-postgres-proof.js';",
    "import { phase5PgAliasQuery, phase5PgBuilderQuery, phase5PgCrossOwnerOrderWriteProof, phase5PgCteQuery, phase5PgDbQueryQuery, phase5PgEndpoint, phase5PgFunctionBoundaryEndpoint, phase5PgFunctionQuery, phase5PgJoinQuery, phase5PgOwnerViaQuery, phase5PgOwnOrderWriteProof, phase5PgRawCrossOwnerOrderWriteProof, phase5PgRawQuery, phase5PgReadTask, phase5PgReferenceMembershipEndpoint, phase5PgScheduleTask, phase5PgStatusEndpoint, phase5PgSubqueryQuery, phase5PgUnionQuery, phase5PgUnknownRelationEndpoint, phase5PgViewQuery, phase5PgWebhook, phase5PgWriteBoundaryEndpoint } from './paranoid-phase5-postgres-proof.js';",
