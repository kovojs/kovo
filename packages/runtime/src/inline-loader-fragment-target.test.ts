...
// response-apply pass and fell back to a fresh native form submit with no FW-Idem dedup. The guard
...
describe('inline loader fragment-target selector guard (M10)', () => {
  const sources: readonly [string, string][] = [
    ['readable build source', inlineJisoLoaderInstallerReadableSource],
    ['freshly minified build source', buildInlineJisoLoaderInstallerSource()],
    ['generated bootstrap source', createInlineJisoLoaderSource('globalThis.__jisoInlineImport')],
  ];
  ...
  it.each(sources)('guards the fragment-target querySelector in the %s', (_name, source) => {
    ...
    expect(source).toContain('[fw-c="');
    // ...the `[fw-c="` lookup is opened inside a `try` block (var names may be minified)...
    expect(/try\s*\{[\s\S]{0,200}\[fw-c="/.test(source)).toBe(true);
    // ...and a `catch` closes the guard so a malformed selector cannot throw out of `ft`.
    expect(/\[fw-c="[\s\S]{0,600}?catch/.test(source)).toBe(true);
  });
});
