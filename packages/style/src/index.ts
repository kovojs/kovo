...
/** Explicit raw inline style payload used only through `style.raw(...)`. */
...
/** Opaque compiled style object that may be passed to `style.attrs` or `style.props`. */
export type Style = CompiledStyle | null | false | undefined;

/** A style argument accepted by `attrs`/`props`, including arrays and raw inline tuples. */
...
/** Structured result for compiler callers that need both style records and extracted CSS. */
...
/** React-shaped merge result returned by `style.props`. */
...
/**
 * Compile static style namespaces and return both the opaque style records and
 * the extracted CSS rules. Compiler integration uses this structured result to
 * build manifests and attribution instead of scraping strings (SPEC.md §13.1).
 */
...
/**
 * Merge compiled style records into React-shaped props. Later style arguments win
 * at the CSS-property key, matching StyleX's styleq merge contract while keeping
 * raw inline style explicit and rare (SPEC.md §4.7 and §13.1).
 */
export function props(...styles: readonly StyleInput[]): PropsResult {
...
export function attrs(...styles: readonly StyleInput[]): AttrsResult {
  ...
  if (result.className) attrsResult.class = result.className;
  if (result[STYLE_SRC]) attrsResult[STYLE_SRC] = result[STYLE_SRC];
  if (result.style) attrsResult.style = serializeInlineStyle(result.style);
  return attrsResult;
}
...
/**
 * Explicit raw inline-style escape hatch for truly dynamic values. Keep this rare:
 * the normal `style` prop is for typed atomic style objects, per the Phase 0
