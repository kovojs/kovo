export interface KovoCheckDiagnosticFact {
  code: string;
  message: string;
...
export interface KovoCheckCoverageFact {
  properties: Record<string, string>;
  raw: string;
}
...
export interface KovoCheckResultLike {
  exitCode: number;
  output: string;
}

export interface KovoCheckResultFact extends KovoCheckOutput {
  exitCode: number;
}
...
export interface KovoCheckOkAssertionFact {
  exitCode: 0;
  issueCount: 0;
  status: 'ok';
  version: KovoCheckOutput['version'];
...
export interface KovoCheckUnguardedAuditBehaviorFact {
  coverage: KovoCheckCoverageAssertionFact[];
  diagnostics: KovoCheckDiagnosticAssertionFact[];
  exitCode: number;
  status: KovoCheckOutput['status'];
...
export interface KovoCheckUnguardedAuditGraph {
...
export type KovoCheckCommand = (graph: KovoCheckUnguardedAuditGraph) => KovoCheckResultLike;

export function parseKovoCheckOutput(output: string): KovoCheckOutput {
  const lines = output.trimEnd().split('\n');
  const version = lines[0];
  if (version !== 'kovo-check/v1') {
    throw new Error(`kovo check output starts with kovo-check/v1: ${version ?? ''}`);
  }
...
export function kovoCheckResultFact(result: KovoCheckResultLike): KovoCheckResultFact {
  return { ...parseKovoCheckOutput(result.output), exitCode: result.exitCode };
}

export function kovoCheckAssertionFact(result: KovoCheckResultLike): KovoCheckAssertionFact {
  const fact = kovoCheckResultFact(result);
...
export function kovoCheckOkAssertionFact(result: KovoCheckResultLike): KovoCheckOkAssertionFact {
  const fact = kovoCheckAssertionFact(result);
  const issueCount = fact.coverage.length + fact.diagnostics.length;
  if (fact.exitCode !== 0 || fact.status !== 'ok' || issueCount !== 0) {
    throw new Error(
      `kovo check expected OK: exitCode=${fact.exitCode} status=${fact.status} diagnostics=${fact.diagnostics.length} coverage=${fact.coverage.length}`,
    );
  }
...
export function kovoCheckUnguardedAuditBehaviorFact(options: {
  kovoCheck: KovoCheckCommand;
  ...
  const fact = kovoCheckAssertionFact(
    options.kovoCheck({
      mutations: [
...
export function kovoCheckDiagnosticFacts(output: string): KovoCheckDiagnosticFact[] {
  return parseKovoCheckOutput(output).diagnostics;
}

export function kovoCheckDiagnosticAssertionFacts(
  output: string,
): KovoCheckDiagnosticAssertionFact[] {
  return kovoCheckDiagnosticFacts(output).map(({ raw: _raw, ...fact }) => fact);
}

export function kovoCheckCoverageFacts(output: string): KovoCheckCoverageFact[] {
  return parseKovoCheckOutput(output).coverage;
}
...
function parseKeyValueFields(source: string): Record<string, string> {
  ...
  if (rest.length > 0) {
    throw new Error(`kovo check coverage fields are key=value entries: ${source}`);
  }
