...
describe('@kovojs/test package subpath exports', () => {
  ...
  it('keeps subpath-only helpers available through their owning modules', () => {
    ...
    expect(
      viteLoweredEventDiagnosticFact(
        [
          ...
          '',
          'KV201 routes/card.tsx:1:1 message.',
          '  help: Would lower to: on:click="/c/__v/1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd1234abcd/routes/card.client.js#Card$click"',
          '  help: Element params: -',
        ].join('\n'),
      ).loweredHandler,
    ).toEqual({
      ...
      versionShape: 'representation-digest-hex-64',
    });
    ...
    expect(
      generatedHandlerReferenceFact(
        '/c/__v/0a1b2c3d0a1b2c3d0a1b2c3d0a1b2c3d0a1b2c3d0a1b2c3d0a1b2c3d0a1b2c3d/cart.client.js#Cart$click',
      ),
    ).toMatchObject({
    ...
    expect(generatedWireResponseBodies['typed-read.http']).toEqual([
      '<kovo-query name="product" key="product:p1" href="/_q/product?id=p1">{"name":"Mug","stock":4}</kovo-query>\n',
    ]);
