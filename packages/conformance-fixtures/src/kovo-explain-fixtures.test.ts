...
describe('@kovojs/test kovo explain fixture seam', () => {
  it('turns kovo-explain/v1 output into structured field and record facts', () => {
    expect(parseKovoExplainOutput(fixture)).toEqual({
      fields: [
      ...
      records: [
        { key: 'OPTIMISTIC', raw: 'OPTIMISTIC cart await-fragment', value: 'cart await-fragment' },
        {
          ...
          raw: 'OPTIMISTIC-SUMMARY total=1 derived=0 hand-written=0 await-fragment=1 UNHANDLED=0 PUNTED=0',
          value: 'total=1 derived=0 hand-written=0 await-fragment=1 UNHANDLED=0 PUNTED=0',
        },
      ],
  ...
  it('exposes focused helpers for harness assertions without local output parsing', () => {
    expect(kovoExplainField(fixture, 'session')).toBe('commerceSession');
    expect(kovoExplainListField(fixture, 'input-fields')).toEqual(['productId', 'quantity']);
    expect(kovoExplainRecords(fixture, 'OPTIMISTIC')).toEqual(['cart await-fragment']);
    expect(kovoExplainOptimisticStatuses(fixture)).toEqual({ cart: 'await-fragment' });
    expect(kovoExplainSummary(fixture, 'OPTIMISTIC-SUMMARY')).toMatchObject({
      UNHANDLED: '0',
      'await-fragment': '1',
      total: '1',
    });
    expect(kovoExplainUpdateTargets(fixture)).toEqual([
      'cart->component:CartBadge,page:/cart',
      'product->page:/products',
    ]);
    expect(kovoExplainUpdateConsumers(fixture)).toEqual([
      { consumers: ['component:CartBadge', 'page:/cart'], query: 'cart' },
      { consumers: ['page:/products'], query: 'product' },
    ]);
    expect(Object.fromEntries(kovoExplainUpdateConsumerMap(fixture))).toEqual({
      cart: ['component:CartBadge', 'page:/cart'],
      product: ['page:/products'],
    });
    expect(
      kovoExplainListField('kovo-explain/v1\nMUTATION order/receipt\nupdates: -\n', 'updates'),
    ).toEqual([]);
    expect(
      kovoExplainUpdateTargets('kovo-explain/v1\nMUTATION order/receipt\nupdates: -\n'),
    ).toEqual([]);
    expect(
      kovoExplainUpdateConsumers('kovo-explain/v1\nMUTATION order/receipt\nupdates: -\n'),
    ).toEqual([]);
    expect(
      kovoExplainEndpointFacts(
        [
  ...
  it('exposes rawless assertion facts for query, mutation, and page explanations', () => {
    expect(kovoExplainMutationAssertionFact({ exitCode: 0, output: fixture })).toEqual({
      exitCode: 0,
      guards: ['authed'],
  ...
  it('exposes rawless assertion facts for endpoint and scope audit explanations', () => {
    const componentOutput = [
      ...
      'MERGE button attr=data-bind:hidden rule=single-binding-writer decision=diagnostic diagnostics=KV233',
      '',
    ].join('\n');

    expect(kovoExplainComponentHandlerFacts(componentOutput)).toEqual([
      {
    ...
    expect(kovoExplainComponentDeriveFacts(componentOutput)).toEqual([
      {
        inputs: ['cart'],
    ...
    expect(kovoExplainComponentTriggerFacts(componentOutput)).toEqual([
      {
        deps: ['cart'],
    ...
    expect(kovoExplainComponentMergeFacts(componentOutput)).toEqual([
      {
        attr: 'aria-expanded',
        decision: 'primitive',
        diagnostics: [],
    ...
    expect(kovoExplainComponentAssertionFact({ exitCode: 0, output: componentOutput })).toEqual({
      derives: [
        {
          inputs: ['cart'],
          name: 'CartBadge$isEmpty',
  ...
  it('derives mutation-query matrix facts from kovo-explain outputs', () => {
    const graph = {
  ...
  it('rejects malformed explain output at the fixture seam', () => {
    expect(() => parseKovoExplainOutput('kovo-check/v1\nOK\n')).toThrow(
      'kovo explain output starts with kovo-explain/v1: kovo-check/v1',
    );
    expect(() => parseKovoExplainOutput('kovo-explain/v1\n')).toThrow(
      'kovo explain output includes a subject line',
    );
    expect(() => kovoExplainField(fixture, 'missing')).toThrow(
      'kovo explain output includes missing:',
    );
    expect(() => kovoExplainSummary(fixture, 'MISSING')).toThrow(
      'kovo explain output includes MISSING',
    );
    expect(() =>
      kovoExplainOptimisticStatuses('kovo-explain/v1\nMUTATION cart/add\nOPTIMISTIC cart\n'),
    ).toThrow("kovo explain OPTIMISTIC record is '<query> <status>': cart");
    expect(() =>
      kovoExplainUpdateConsumers('kovo-explain/v1\nMUTATION cart/add\nupdates: cart\n'),
    ).toThrow("kovo explain update target is '<query>-><consumers>': cart");
    expect(() =>
      kovoExplainEndpointFacts('kovo-explain/v1\nENDPOINTS\nENDPOINT cart/add\n'),
    ).toThrow('kovo explain ENDPOINT record is');
    expect(() =>
      kovoExplainScopeAuditFacts('kovo-explain/v1\nUNSCOPED\nUNSCOPED cart\n', 'UNSCOPED'),
    ).toThrow('kovo explain UNSCOPED record is');
    expect(() =>
      kovoExplainComponentHandlerFacts('kovo-explain/v1\nCOMPONENT CartBadge\nHANDLER click\n'),
    ).toThrow('kovo explain HANDLER record is');
    expect(() =>
      kovoExplainComponentDeriveFacts('kovo-explain/v1\nCOMPONENT CartBadge\nDERIVE value\n'),
    ).toThrow('kovo explain DERIVE record is');
    expect(() =>
      kovoExplainComponentTriggerFacts('kovo-explain/v1\nCOMPONENT CartBadge\nTRIGGER visible\n'),
    ).toThrow('kovo explain TRIGGER record is');
    expect(() =>
      kovoExplainComponentMergeFacts('kovo-explain/v1\nCOMPONENT CartBadge\nMERGE button\n'),
    ).toThrow('kovo explain MERGE record is');
    expect(() => kovoExplainUnguardedFacts('kovo-explain/v1\nUNGUARDED\nQUERY\n')).toThrow(
      'kovo explain UNGUARDED record includes a target',
    );
    expect(() =>
      kovoExplainUnguardedFacts('kovo-explain/v1\nUNGUARDED\nQUERY cart guards\n'),
    ).toThrow('kovo explain record field is key=value');
    expect(() =>
      kovoExplainUnguardedAssertionFact({
        exitCode: 0,
        output: 'kovo-explain/v1\nENDPOINTS\nSUMMARY total=0\n',
      }),
    ).toThrow('kovo explain unguarded subject is UNGUARDED: ENDPOINTS');
    expect(() =>
      kovoExplainScopeAuditAssertionFact({
        exitCode: 0,
        output: 'kovo-explain/v1\nENDPOINTS\nSUMMARY total=0\n',
      }),
    ).toThrow('kovo explain scope audit subject is UNGUARDED or UNSCOPED: ENDPOINTS');
  });
});
