  ...
  it('guards the authoritative compiler-owned diagnostic code list', () => {
    ...
    expect(outOfScopeCompilerDiagnostics).toMatchInlineSnapshot(`
      [
        ...
        {
          "code": "KV436",
          "reason": "Security-heavy: kovo check consumes compiler-derived undecided access facts, while compileRouteModule emits access/legacy-guard ambiguity into the static build graph; the latter is covered by route-pages and kovo-build security fixtures rather than the component/registry matrix.",
        },
        {
          "code": "KV438",
  ...
  it('snapshots representative compiler-owned diagnostics with file, position, length, message, and help', () => {
    ...
    expect(diagnosticFacts).toMatchInlineSnapshot(`
      ...
      SPEC §4.8 query binding validation depends on one stable shape per query and one unambiguous alias-to-runtime identity per component; duplicate identities would otherwise silently last-write-wins during graph indexing or DOM update ownership.",
          "length": null,
          "message": "Duplicate or ambiguous query-plan identity. query="cart" sources=generated/queries/cart-refresh.shape.ts, generated/queries/cart.shape.ts",
          "severity": "error",
          "start": null,
        },
        {
