...
| --- | --- | --- | --- |
| `lowerStructuralJsx` | `structural-jsx.ts` | `jsx-ir-owner` | Owns structural element/tag/attribute/child rewrites: primitive spreads, primitive `asChild`/attrs composition, `Link` tag/attribute lowering, `viewTransitionName` style lowering, inline binding stamps, mixed-text wrapper insertion, and generated helper imports. |
| `platformBehaviorLowering` | `platform.ts` | `structural-debt` | Currently rewrites event attributes to platform attributes as source patches in the model-patch pass. It overlaps authored attributes and must migrate into JSX IR or be proven terminal before the next structural-IR closure item is checked. |
