...
`,
    ...
    expect(serverSource).toMatch(
      /on:idle="\/c\/__v\/[0-9a-f]{64}\/components\/search\.client\.js#Search\$search_index_idle"/,
    );
    expect(serverSource).toMatch(
      /on:visible="\/c\/__v\/[0-9a-f]{64}\/components\/search\.client\.js#Search\$sales_chart_visible"/,
    );
