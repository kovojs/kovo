 ...
 * framework-identity project. Model construction and compilation both execute under the entry's
 * exact app-contract resolutions; each transformed root is attached to the invocation-scoped
 * lookup in O(1), replacing the former N per-entry O(N) map registrations without persisting ASTs
 * beyond this synchronous source-proof call (SPEC §5.2 rules 6/9).
 */
export function compileComponentProjectEntries(
  ...
  const parsedProject = parseComponentProject(
    files,
    options.withEntryResolutions === undefined
      ? {}
      : { withEntryResolutions: options.withEntryResolutions },
  );
