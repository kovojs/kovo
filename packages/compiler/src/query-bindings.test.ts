    ...
    expect(result.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          fileName: 'cart-badge.tsx',
          message:
            'Duplicate or ambiguous query-plan identity. query="cart" sources=generated/queries/cart-refresh.shape.ts, generated/queries/cart.shape.ts',
          severity: 'error',
        }),
      ]),
    );
  ...
  it('reports KV240 when duplicate query-shape facts have the same shape', () => {
    ...
    expect(queryShapeFactDiagnostics('cart-badge.tsx', queryShapeFacts)).toMatchInlineSnapshot(`
      ...
      SPEC §4.8 query binding validation depends on one stable shape per query and one unambiguous alias-to-runtime identity per component; duplicate identities would otherwise silently last-write-wins during graph indexing or DOM update ownership.",
          "message": "Duplicate or ambiguous query-plan identity. query="cart" sources=generated/queries/cart-copy.shape.ts, generated/queries/cart.shape.ts",
          "severity": "error",
        },
      ]
    `);
