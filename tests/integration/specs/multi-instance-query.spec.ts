...
test('keeps parameterized query instances distinct on the wire and patches only the matching fragment', async ({
  page,
  kovoApp,
}) => {
  await page.goto('/');
  ...
  await expect(page.locator('[data-product-id="p2"] [data-bind="product.stock"]')).toHaveText('7');
