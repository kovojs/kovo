...
| 16.9 Authorization & data plane | `KOVO_PARANOID=1` Postgres generative dogfood proves zero cross-owner read/write + zero secret-column egress across every query shape; capability-closure boot audit refuses every unsafe reachable object; runtime holds no superuser/`BYPASSRLS`; no value/effect authz/confidentiality code is `build-only`; SQLite quarantined. (`rules/v1-acceptance.md` 16.9.) | Existing `check` security gates + `postgres-authz`/`sqlite-authz`/`authz-census-static`/`cross-owner-read-static` tests. `test:authz-paranoid` harness + closure-audit + runtime-least-priv + registry-invariant tests NOT yet built/wired; `plans/claude-bugz-32.md` B1–B4 are open HIGH fail-opens; fix drafted in `plans/fundamental-fixes-followup-7.md`. | blocked — open fail-opens + harness |
