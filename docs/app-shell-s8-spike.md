...
`conformance/app-shell-spike/src/index.test.ts` starts a real `node:http` server and exercises a prototype request shell through loopback HTTP. The shell is deliberately local to the conformance fixture: it imports no `@jiso/server` internals, so this spike proves the boundary shape rather than implementing R3.
...
- Add the normative SPEC §9.5 text for the request shell and diagnostics FW228/FW229.
