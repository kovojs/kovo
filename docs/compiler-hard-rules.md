...
3. Fixpoint invariant. Generated IR is authorable Jiso source, so `compile(compile(source))` must equal `compile(source)`, and the browser-free render-equivalence gate must prove `render(source)` equals `render(compile(source))`.
