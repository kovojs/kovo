...
| Optimism         | "For each changed query, is the optimistic status hand-written, await-fragment, or unhandled?" | An explicit status for each changed query, with no unhandled status counted as passing. | `kovo explain mutation <name> --optimistic` or `kovo check` optimistic output.               |
