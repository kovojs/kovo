...
**Durable replay storage is bounded and refuse-never-evict (normative).** Exact in-horizon replay, finite storage, and unlimited concurrent admission still require a database-enforced bound. The shipped Postgres store therefore gives mutation and webhook truth separate admission pools of 1,000 retained pending-or-committed keys each. Every admitted row owns a unique numbered slot, so concurrent replicas cannot race above the ceiling. At capacity, an existing in-horizon mutation key (or retained webhook key) still joins or replays its truth, while unseen work is refused before its handler runs (the mutation 429 / webhook retry outcome); no committed row is evicted merely to admit new work. Aborting a pre-commit reservation or the explicit generation-fenced operator reconciliation path releases its slot. Committed mutation truth MUST remain through the exact token `expiresAtMs` defined below and MAY be reclaimed only after that boundary, because the same exact token then fails before store access; webhook committed truth remains retained until an application-aware archival/migration. One committed snapshot is additionally limited to 1,048,576 UTF-16LE body bytes and 65,536 UTF-8 header bytes. Oversized settlement stores no oversized bytes and leaves the already-claimed key pending/fail-closed, so a retry cannot repeat a write whose application transaction may have committed; operator reconciliation must first establish the application outcome. The schema posture audit proves the exact non-deferrable `(surface, scope, idem)` primary key, slot column, 1..1,000 constraint, unique per-surface slot index, response-byte constraint, and exact replay-table ACL before production serves. Ordinary app/runtime roles MUST have neither table-level nor column-level replay privileges; only the isolated system role receives the exact `SELECT, INSERT, UPDATE, DELETE` set. Provisioning repairs a missing canonical primary key, revokes stray table/column grants, and fails closed if duplicate legacy truth prevents identity-key repair.
