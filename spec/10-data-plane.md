...
| HTTP response headers                  | `@kovojs/server/response-finalization` | own         | App-header classifiers plus final raw/structured adapter reconstruction                                                     | `pnpm run check:wire-output-boundary`       | `packages/server/src/response-posture.test.ts`                                                               |
...
| `Set-Cookie`                           | `@kovojs/server/cookies`               | own         | Typed cookie builder and serializer                                                                                         | `pnpm run check:wire-output-boundary`       | `packages/server/src/cookies.test.ts`                                                                        |
| Blob/file write                        | `@kovojs/core/storage`                 | own         | Storage-key validation and static-export path containment                                                                   | `pnpm run check:filesystem-boundary`        | `packages/core/src/storage.test.ts`; `packages/server/src/static-export-output.test.ts`                      |
| Durable-task payload                   | `@kovojs/server/task-runner`           | own         | Queue envelope and redaction-aware observability views                                                                      | `pnpm run check:security-test-builds`       | `packages/server/src/task-observability.test.ts`                                                             |
...
| Webhook payload                        | `@kovojs/server/webhook`               | own         | Verifier-before-parse plus replay-scoped dispatch                                                                           | `pnpm run check:security-test-builds`       | `packages/server/src/webhook.test.ts`                                                                        |
...
| Outbound egress request                | `@kovojs/server/egress`                | own         | Declared-origin, per-hop DNS/address classification and selected-address transport choke                                    | `pnpm run check:egress-boundary`            | `packages/server/src/egress.test.ts`; `packages/server/src/egress-redirect.test.ts`                          |
| Authorization principal/data access    | `@kovojs/server/postgres-authz`        | own         | Pinned principal plus least-privilege Postgres role/RLS/effective-privilege-graph closure                                   | `pnpm run test:authz-paranoid`              | `packages/server/src/postgres-authz.test.ts`; served paranoid production-artifact matrix                     |
| Better Auth credential/non-egress      | `@kovojs/better-auth/credential-gate`  | own         | Exact registered consumer, validated result, and same-consumer one-shot result opening                                      | `pnpm run check:security-classifier-corpus` | `packages/better-auth/src/internal.trusted-plaintext.test.ts`                                                |
| Dynamic module/process execution       | `@kovojs/compiler/capability-closure`  | own         | Compiler-owned immutable client-module registry plus reviewed build/runtime capability doors                                | `pnpm run check:sink-policy`                | `packages/browser/src/handlers.test.ts`; `packages/compiler/src/conformance-compat.test.ts`                  |
