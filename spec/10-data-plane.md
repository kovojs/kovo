...
| Request method/authority/scheme     | `@kovojs/server/request-ingress`       | reconstruct          | Transport-owned source snapshot plus one finite classifier and reconstructed Request decision | `pnpm run check:security-classifier-corpus` | `packages/server/src/request-ingress-policy.test.ts`; real HTTP/2/live/generated parity     |
