...
| HTTP response headers | framework-owned door | Finalization owns names/repetition/control chars and rejects app-authored framing or hop-by-hop fields before adapter mutation. | `packages/server/src/response-posture.test.ts`; `packages/server/src/node.test.ts`; `packages/server/src/build.test.ts`.            |
