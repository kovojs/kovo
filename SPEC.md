...
- **bfcache and loader budget stay load-bearing.** Enhanced navigation must not add `unload` handlers or global session heaps that block bfcache. Its code counts against the inline loader's gzip budget (the enforced ceiling lives in `inlineKovoLoaderGzipByteBudget`). The budget was raised from 8KB to keep reactive attribute writes behind the same XSS sink policy as the rest of the framework, then to route the always-on `insertAdjacentHTML`/`innerHTML` fragment-apply sinks through the framework `kovo` Trusted Types policy (the inlined `trustedHtml` shim) — the comparable XSS-sink-policy justification that lets Trusted Types ship default-on without bricking Chromium hydration — and then to admit the §9.3 `mode="prepend"` wire capability (the `p` closure's keyed-dedup insert-at-START branch and scroll-anchor guarantee for "load older" feeds). Future budget increases require comparable XSS-sink or first-class-wire-capability evidence.
