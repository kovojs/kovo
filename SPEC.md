...
The local invariant is default-deny plus freshness proof. Queries, mutations, endpoints, and routes need explicit access posture; owner/secret/governed facts flow from schema to wire eligibility and invalidation; raw or opaque SQL must declare the facts the analyzer cannot prove and is then runtime-verified.
