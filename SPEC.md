...
**Client-wire confidentiality (KV435).** The client-visible query wire, island `kovo-state`, typed mutation failure payloads, and emitted client modules are the confidentiality dual of the unsafe output contexts above. A value classified by schema/query facts as `secret` MUST NOT reach those sinks by default, and an opaque projection from a table carrying secret columns fails closed unless the projection is explicit non-secret data. The only declassification path is `trustedReveal(value, { justification })` in a statically analyzed projection; such reveals are recorded for `kovo explain --revealed`.
...
10. **Output safety is contextual and default-on.** The server renderer and the client update plan MUST contextually encode every interpolated query/state value for its sink — escaped text for text content, attribute-value escaping for attributes, the §9.1 script-data encoding for JSON islands — and MUST encode identically (bound by render-equivalence, rule #3). Plain bindings may reach only safe contexts; the unsafe output contexts and the URL-scheme allowlist are defined in §4.8 and gated by **KV236**. The only suppression is the typed trusted-HTML escape hatch (§4.8); there is no raw-string ejection. A sink renderer or any other app-authored presentation layer that consumes streamed/model output is bound by the same obligation (§9.1). The same default-deny rule applies on the confidentiality axis: a client-visible query, state, failure, or client-module sink may not receive a `secret`-classified value unless the source contains a statically audited `trustedReveal` (§4.8, §10.2, KV435).
...
| Confidentiality       | `secret` facts + client-bound sink classifiers         | secret-classified values are ineligible for query/state/failure/client-module sinks unless declassified through statically-audited `trustedReveal`; opaque secret-table projections fail closed (KV435)                                                                      |
| Access completeness   | explicit `access:` declarations                        | every query, mutation, route/page, endpoint, and webhook has an explicit access decision; omission is KV436; `public(reason)` and `access: verified` are explicit reviewed inhabitants, not defaults                                                                         |
| Write provenance      | `owner`/`governed` schema facts + write analysis       | primary keys and owner columns are governed, `governed: true` extends the set, client input cannot write governed columns without audited `adminAssign`, non-input overrides use `serverValue`, and query loaders cannot write without `query.elevated` (KV433/KV437)        |
...
**Outbound egress floor (normative).** Public/external egress is unrestricted. Private, loopback, link-local, special-use, and cloud metadata destinations are denied by default and may be reached only through exact `egress.allowInternal` `host:port` entries, except that cloud metadata endpoints are never allowlisted. Metadata access is available only inside framework-owned cloud credential providers (`awsCredential`, `gcpCredential`, `azureCredential`) that enter a private capability frame; raw SDK refreshes outside that frame fail closed. The floor runs at both undici dispatch and Node `http`/`https`/`net` hooks, and every redirect hop is rechecked after resolution and normalization. This is a runtime defense-in-depth floor, not a by-construction proof; broad CIDR exceptions are permitted only as diagnostic-visible KV438 infrastructure exceptions.
...
The same schema fact also carries security classification. `secret` marks columns ineligible for client-bound sinks without a statically audited reveal (KV435). `owner` columns and primary keys are governed by default, and `governed: true` marks additional columns whose writes may not be sourced from client input without the audited `adminAssign` escape (KV437). `atomic` and `version` mark contended values for optimistic-concurrency primitives and the KV429 read-then-write gate; plain mutation transactions are not themselves an atomicity proof. Database constraints (`CHECK`, `UNIQUE`, foreign keys, exclusion constraints, and equivalent engine constraints) remain the fail-closed backstop for invariants the database can enforce directly; multi-row or aggregate invariants require explicit row locks, serializable isolation, or a database-level constraint that represents the whole invariant.
...
Every query, mutation, route/page, endpoint, and webhook MUST carry an explicit `access:` decision. There is no implicit public or authenticated default: omission is KV436. `public(reason)` is the reviewed public inhabitant, guard chains are the executable session/authorization inhabitant, and `access: verified` is reserved for machine-auth surfaces whose verifier is declared separately.
...
| KV428 | error    | Uploaded attacker-controlled bytes would be rendered inline without a verified-safe content type or framework re-encode step; serve as attachment with `nosniff`, re-encode through a safe framework transform, or use an audited inline-upload escape surfaced in `kovo explain --capabilities` (§6.6, §9.1)                                                                                                                                                     |
| KV429 | error    | A mutation reads and then writes a declared atomic/versioned value without compare-and-set, version conflict handling, a row lock, or serializable-transaction proof; plain READ COMMITTED mutation transactions are not an atomicity proof (§10.3, §11.1)                                                                                                                                                                                                        |
| KV430 | lint     | A wire-reachable schema admits recursive depth, array length, record breadth, or total node count without an explicit budget; runtime defaults still fail closed, but `.max(...)`/budget declarations make capacity decisions reviewable (§6.6, §9.1)                                                                                                                                                                                                             |
| KV431 | error    | A route document references a client module absent from the static/export client-module manifest; the route cannot be replayed as immutable same-origin JavaScript under the CSP/export manifest (§6.6, §9.5)                                                                                                                                                                                                                                                     |
| KV432 | error    | A session/auth-reachable cookie weakens the class-derived HttpOnly, Secure, or SameSite floor without a recorded `unsafeCookie` justification visible in `kovo explain --cookies` (§6.5, §9.1)                                                                                                                                                                                                                                                                    |
| KV433 | error    | A query loader reaches a write-capable database operation without the explicit `query.elevated` capability; read surfaces lack write authority by default, with elevated reads audited in `kovo explain --capabilities` (§10.2, §11.1)                                                                                                                                                                                                                            |
| KV434 | error    | `s.string().pattern(...)` is dynamic or structurally non-linear-safe; use a blessed linear validator, a compile-visible safe literal, or the audited `unsafeRegex(re, justification)` escape (§6.6, §9.1)                                                                                                                                                                                                                                                         |
...
| KV437 | error    | Client input reaches a governed column write; derive the value from server provenance, use `serverValue` for non-input values, or use audited `adminAssign` for intentional administrator writes (§10.3, §11.1)                                                                                                                                                                                                                                                   |
| KV438 | warn     | Broad `egress.allowInternal` CIDR grants provenance-blind private-network reachability; prefer exact `host:port` entries, or keep the CIDR only as an explicit infrastructure exception surfaced through app diagnostics (§6.6, §9.5, §11.3)                                                                                                                                                                                                                      |
...
`kovo explain --capabilities` is the stable least-authority audit. Its diffable rows include confidentiality reveals, `egress.allowInternal` holes, `query.elevated`, `adminAssign`, `serverValue`, `unsafeCookie`, inline upload escapes, CSP/Trusted Types exceptions, signed capability URL mints, and other framework-defined authority grants. Each row carries the capability owner, source/sink labels where applicable, and the author justification or reason text that made the escape reviewable.

`kovo explain --trust` remains the source/sink trust-provenance view. It answers which dangerous sinks and escape hatches were reached, what provenance was proven, and which trust assertions discharged a diagnostic. It complements `--capabilities`: `--trust` is the low-level sink/provenance ledger, while `--capabilities` is the higher-level authority and escape-hatch review table.
