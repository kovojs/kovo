...
Raw HTTP integrations use declared `endpoint()` entries, not ad-hoc server escape hatches. An endpoint is registry-visible, receives `Request -> Response`, may opt out of CSRF with a named justification, and is enrolled in the unguarded audit with the same auth metadata as routes, queries, and mutations. OAuth/SAML callbacks, webhooks, and exports belong here; browser credential forms should still prefer typed `mutation()` flows so they keep schema validation, no-JS behavior, and the normal response vocabulary.
