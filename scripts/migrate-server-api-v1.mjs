 ...
 * refuses namespace/default/dynamic access and implementation carriers because syntax alone
 * cannot prove which authority or application-owned replacement the caller intended.
 */
export function analyzeServerApiV1Migration({ fileName, source }) {
  ...
  if (node.attributes || node.assertClause) {
    refusals.push(
      refusal(
        ...
        node,
        sourceFile,
        'An attributed server import may carry loader semantics that a split rewrite cannot preserve.',
      ),
    );
    return;
  }
  ...
  if (clause.name) {
    refusals.push(
      refusal(
        ...
        clause.name,
        sourceFile,
        'The server root has no mechanical default-import split across task entrypoints.',
      ),
    );
  }
  ...
  if (ts.isNamespaceImport(clause.namedBindings)) {
    refusals.push(
      refusal(
        ...
        sourceFile,
        'A server namespace may read moved or internalized members dynamically and cannot be split safely.',
      ),
    );
    return;
  }
  ...
  if (
    ...
    refusals.push(
      refusal(
        ...
        node,
        sourceFile,
        'A wildcard, namespace, or attributed server re-export cannot be split without changing downstream bindings.',
      ),
    );
    return;
  }
...
function dynamicRefusal(node, sourceFile, spelling) {
  return refusal(
    ...
    node,
    sourceFile,
    `A ${spelling} has namespace semantics that cannot be proven safe across task entrypoints.`,
  );
}
...
function uniqueEdits(edits) {
  ...
  for (let index = 1; index < ordered.length; index += 1) {
    if (ordered[index].start < ordered[index - 1].end) {
      throw new Error('server API migration produced overlapping edits');
    }
  }
  return ordered;
}
...
export function runServerApiV1Migration({ cwd = process.cwd(), mode, sourcePaths = [] }) {
  ...
  for (const file of files) {
    ...
    const analysis = analyzeServerApiV1Migration({ fileName: file.path, source });
...
function replaceRegularFile(filePath, source, expectedSource, before) {
  const tempPath = resolve(
    dirname(filePath),
    `.${basename(filePath)}.kovo-server-api-v1-${String(process.pid)}-${Date.now().toString(36)}`,
  );
  let descriptor;
  try {
...
function main(args) {
  ...
  if (modeArg !== '--check' && modeArg !== '--write') {
    process.stderr.write(
      'usage: node scripts/migrate-server-api-v1.mjs --check|--write [source-or-directory ...]\n',
    );
    return 2;
  }
  try {
    ...
    process.stderr.write(
      `migrate-server-api-v1: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    return 1;
  }
}

if (isDirectScriptInvocation(import.meta.url, process.argv[1], 'migrate-server-api-v1.mjs')) {
