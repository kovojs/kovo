# Agent setup for `kovo`

This is a deepsec scanning workspace. Project `kovo` was just registered
(target: `..`). Setup is incomplete — `data/kovo/INFO.md`
...
2. **Fill in `data/kovo/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id kovo
pnpm deepsec process --project-id kovo
```
