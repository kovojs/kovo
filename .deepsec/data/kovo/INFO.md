...
demos. Because this is a framework, an insecure default propagates to _every_
...
- `guards` object (`packages/server/src/guards.ts`): `guards.authed()`,
  ...
  _before_ the handler; order matters (auth must precede rate-limit).
...
- **New output sinks that skip escaping.** Text interpolation is escaped by the
  _compiler_ (`escapeText` in `packages/server/src/html.ts`) and by `@kovojs/ui`
...
- **Mutation idempotency/replay** (`mutation.ts`/`replay.ts`): reservation must
  happen _before_ the handler runs, not after.
...
- `jsx()` returning a raw `string` is a load-bearing contract — composed
  component output is intentionally _not_ branded/escaped at runtime; escaping
