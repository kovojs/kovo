# Agent setup for `jiso`

This is a deepsec scanning workspace. Project `jiso` was just registered
(target: `..`). Setup is incomplete — `data/jiso/INFO.md`
...
2. **Fill in `data/jiso/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id jiso
pnpm deepsec process --project-id jiso
```
