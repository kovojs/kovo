...
// file; scripts/materialize-demo-css.mjs folds it into a content-hashed
// `/assets/styles.<hash>.css` asset for deployment (SPEC §6.1.1, §13.1). Re-run
// after upgrading @kovojs/ui.
...
const outPath = resolve(soRoot, 'src/generated/kovo-ui.css');
const tempRoot = mkdtempSync(resolve(tmpdir(), 'kovo-so-ui-css-'));
...
try {
  const output = execFileSync(
    ...
    [
      ...
      '@kovojs/ui',
      '--entry',
      resolve(soRoot, 'src/app.ts'),
      '--out',
      componentCssPath,
    ],
    { cwd: soRoot, encoding: 'utf8' },
  );
