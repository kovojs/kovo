  ...
  render: (_queries: Record<string, never>, state: GalleryAutocompleteDemoState) => {
    ...
    return (
      ...
      >
        ...
        {/* T6 (UX): wrap input + list in a position:relative anchor so the
            suggestion list drops flush below the input instead of overlaying it. */}
          ...
          >
            <AutocompleteOption
              ...
              onClick={() => {
                ...
                if (!result) return;
                // B5 (UX): selecting any option closes the list, even when the
                // value is unchanged (re-selecting the current suggestion).
                state.open = result.open.open;
                if (result.value.changed) {
                  state.inputValue = result.inputValue.inputValue;
                  state.value = result.value.value ?? state.value;
