...
export const crmViteConfig = defineConfig({
  ...
  // The multi-tenant demo server installs its own per-session request dispatch, so it drops the
  // singleton app-shell route plugin and enables the compiler pre-plugin for SPEC §4.8/§5.2 output
  // escaping. Source-mode dev/tests keep their existing app-shell plugin path.
  plugins: [
    ...(process.env.KOVO_DEMO_MULTITENANT
      ? [exampleKovoCompilerPlugin({ include: ['src/components', 'src/interactive-app.tsx'] })]
      : []),
