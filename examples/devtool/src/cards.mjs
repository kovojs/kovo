...
export function buildCard(node, bundle) {
  ...
  const mutsWriting = (domains) =>
    bundle.nodes.filter((n) => n.kind === 'mutation' && (n.data.writes ?? []).some((d) => domains.includes(d)));
  const optStatus = (m, queryName) => {
    const o = (m.data.optimistic ?? []).find((x) => x.query === queryName);
    return o ? { status: o.status, reason: o.derivation?.status === 'PUNTED' ? o.derivation.reason : undefined } : null;
  };
  ...
  const card = {
    ...
    label: node.label,
    endpoint:
      node.kind === 'mutation' ? `POST /_m/${node.name}` :
      node.kind === 'query' ? `GET /_q/${node.name}` :
      node.kind === 'page' ? `GET ${node.name}` : undefined,
  ...
  if (node.kind === 'component') {
    const queries = edges.filter((e) => e.kind === 'feeds' && e.to === node.id).map((e) => byId.get(e.from));
    const mutations = edges.filter((e) => e.kind === 'emits' && e.from === node.id).map((e) => byId.get(e.to));
    card.domName = node.data.domName;
    card.fragments = node.data.fragments ?? [];
    S.queriesIn = queries.map((q) => ({
      ...ref(q),
      domains: q.data.domains ?? [],
      invalidators: mutsWriting(q.data.domains ?? []).map((m) => ({ ...ref(m), optimistic: optStatus(m, q.name) })),
    }));
    S.mutationsOut = mutations.map((m) => ({ ...ref(m), fields: m.data.inputFields ?? [] }));
  } else if (node.kind === 'mutation') {
    const domains = node.data.writes ?? [];
    const queries = bundle.nodes.filter((n) => n.kind === 'query' && (n.data.domains ?? []).some((d) => domains.includes(d)));
    S.writes = domains.map((d) => ref(byId.get(`domain:${d}`) ?? { id: `domain:${d}`, kind: 'domain', label: d, name: d }));
    S.invalidates = queries.map((q) => ({ ...ref(q), optimistic: optStatus(node, q.name) }));
    S.inputs = node.data.inputFields ?? [];
    S.touchSites = (node.source?.touches ?? []).map((t) => ({ via: t.via, domain: t.domain, keys: t.keys ?? null, site: t.site }));
  } else if (node.kind === 'query') {
    const domains = node.data.domains ?? [];
    S.reads = domains.map((d) => ref(byId.get(`domain:${d}`) ?? { id: `domain:${d}`, kind: 'domain', label: d, name: d }));
    S.feeds = edges.filter((e) => e.kind === 'feeds' && e.from === node.id).map((e) => ref(byId.get(e.to)));
    S.invalidatedBy = mutsWriting(domains).map((m) => ({ ...ref(m), optimistic: optStatus(m, node.name) }));
  } else if (node.kind === 'domain') {
    S.backs = bundle.nodes.filter((n) => n.kind === 'query' && (n.data.domains ?? []).includes(node.name)).map(ref);
    S.writtenBy = bundle.nodes.filter((n) => n.kind === 'mutation' && (n.data.writes ?? []).includes(node.name)).map(ref);
  } else if (node.kind === 'page') {
    card.meta = node.data.meta ?? {};
    S.renders = edges.filter((e) => e.kind === 'renders' && e.from === node.id).map((e) => ref(byId.get(e.to)));
  }
  return card;
}

const opt = (o) => (o ? `[${o.status}${o.reason?.code ? `:${o.reason.code}` : ''}]` : '[no-transform]');
...
export function cardToText(card) {
  ...
  if (S.queriesIn) {
    ...
    L.push(list(S.queriesIn, (q) => `  ${q.label}  reads ${q.domains.join(', ') || '—'}` +
  ...
  if (S.writes) { L.push(`\nWRITES DOMAINS (${S.writes.length})`); L.push(list(S.writes, (d) => `  ${d.label}`)); }
  if (S.invalidates) {
  ...
  if (S.touchSites?.length) {
    ...
    L.push(list(S.touchSites, (t) => `  ${t.via} → ${t.domain}${t.keys ? ` (${t.keys})` : ''}  ${t.site}`));
  }
  if (S.reads) { L.push(`\nREADS DOMAINS: ${S.reads.map((d) => d.label).join(', ') || '—'}`); }
  if (S.feeds) { L.push(`\nFEEDS COMPONENTS (${S.feeds.length})`); L.push(list(S.feeds, (c) => `  ${c.label}`)); }
  if (S.invalidatedBy) {
    L.push(`\nINVALIDATED BY (${S.invalidatedBy.length})`);
    L.push(list(S.invalidatedBy, (m) => `  ${m.label}  ${opt(m.optimistic)}`));
  }
  if (S.backs) { L.push(`\nBACKS QUERIES (${S.backs.length})`); L.push(list(S.backs, (q) => `  ${q.label}`)); }
  if (S.writtenBy) { L.push(`\nWRITTEN BY (${S.writtenBy.length})`); L.push(list(S.writtenBy, (m) => `  ${m.label}`)); }
  if (S.renders) { L.push(`\nRENDERS (${S.renders.length})`); L.push(list(S.renders, (c) => `  ${c.label}`)); }
  if (card.source) {
    L.push(`\nSOURCE  ${card.source.file}:${card.source.startLine}-${card.source.endLine}`);
    L.push(card.source.code.split('\n').map((l) => `  ${l}`).join('\n'));
  }
  return L.join('\n');
}
