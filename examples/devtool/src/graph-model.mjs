...
export function buildDataflowGraph(raw) {
  ...
  for (const p of raw.pages ?? []) {
    ...
    const compExports = [];
    for (const seg of p.navigationSegments ?? []) for (const c of seg.components ?? []) compExports.push(c);
    for (const exp of compExports) {
      const match = [...nodes.values()].find((n) => n.kind === 'component' && (n.data.exportName === exp || n.label === exp));
...
function buildIndex(nodes, edges, byId) {
  ...
  const componentInflow = (cid) => {
    ...
    const detail = queries.map((q) => {
      const domains = (inc.get(q.id) ?? []).filter((e) => e.kind === 'backs').map((e) => byId[e.from]);
      const invalidators = nodes
        .filter((n) => n.kind === 'mutation' && n.data.writes.some((d) => q.data.domains.includes(d)))
  ...
  const componentOutflow = (cid) => {
    const mutations = (out.get(cid) ?? []).filter((e) => e.kind === 'emits').map((e) => byId[e.from === cid ? e.to : e.from]);
  ...
  const mutationEffects = (m) => {
    const queries = nodes.filter((n) => n.kind === 'query' && n.data.domains.some((d) => m.data.writes.includes(d)));
  ...
  const traceFrom = (nid) => {
    ...
    const walk = (cur, dir) => {
      ...
      for (const e of list ?? []) {
        ...
        edgeSet.add(e.id);
        if (!set.has(next)) { set.add(next); walk(next, dir); }
      }
    };
    walk(nid, 'down');
    walk(nid, 'up');
...
function cardText(n, byId, index) {
  ...
  if (n.kind === 'component') {
    ...
    for (const mf of d.mutationForms ?? []) parts.push('mutation', 'form', mf.mutation, ...(mf.fields ?? []));
  } else if (n.kind === 'query') {
    parts.push('query', 'read', ...(d.domains ?? []), ...(d.guards ?? []));
  } else if (n.kind === 'mutation') {
    parts.push('mutation', 'write', ...(d.writes ?? []), ...(d.inputFields ?? []), ...(d.guards ?? []));
    for (const o of d.optimistic ?? []) parts.push(o.query, o.status, o.derivation?.reason?.code ?? '');
