...
- **t6-2 — "a transaction whose callback throws is an unauthenticated DoS / production crash"** — refuted: not reproducible across ~1300 HTTP requests / ~390 transaction throws; the error path is handled (a 500), not a crash. (The genuine defect in this area is the *non-atomic default handler*, `bugz-21` B3.)
