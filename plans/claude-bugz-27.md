...
**Meta-theme — the round-5 _instances_ (bugz-26 B1–B4) are individually fixed, but the _class_ is not closed.**
...
- **J.2** ("the read-only floor rejects any statement not positively proven read-only") was NOT implemented. The
  ...
  DDL _cases_ to `writeTablesForStatement`; the next write-shaped statement the classifier maps to `[]`
...
the syntactic classifier over an _enumerated_ corpus — it is NOT wired as the runtime enforcement twin (M10). The
