...
app and **(b)** a StackOverflow-style Q&A app on Kovo. Findings were produced by *building* the
...
- **No hard blockers** to a working build — but the apps are not *safe or complete* out of the box.
...
- [ ] **Data-plane & optimism safety gates are not enforced in the example/scaffold build path.**
  ...
  *Fix:* run `kovo compile drizzle-static` + `kovo check` in the example build, or make
  `exampleDrizzleRegistryPlugin` throw on error-severity diagnostics.
...
- [ ] **`trustedHtml(queryData)` is a silent stored-XSS sink (the user-content path is not
  ...
  is `safeRichHtml()` (a runtime sanitizer *floor*, documented as **not** a by-construction claim), but
  the example trains the opposite reflex. *Fix:* require a `reason` on `trustedHtml` (or warn when its
...
- [ ] **CSRF audience foot-gun bricks hand-authored auth forms.** Dispatch validates
  ...
  **422**s; `examples/reference` auth tests are **red on HEAD**. Compiler-emitted `<form mutation>`
  auto-binds and works. *Fix:* auto-bind audience from the targeted mutation in the helpers; add a no-JS
...
- [ ] **`owns()` cannot read the validated key → latent IDOR.** `guards.owns` reads `keyOf(request)`
  ...
  sugar (SPEC §10.3:1157). *Fix:* merge args/params before `runGuard`; add a production-path `owns()`
...
- [ ] **KV429 lost-update lowering footgun.** `set({score: sql\`${col} + 1\`})` (bare literal) silently
  **escapes** KV429, while `+ ${1}` (interpolated operand) trips it — trivially-equivalent SQL gets
  different verdicts (`packages/drizzle/src/index.toctou-readonly.test.ts`). *Fix:* lower `+ 1` and
  `+ ${1}` identically. (Caveat: a pure counter increment is atomic and correctly un-gated;
...
- [ ] **Both shipped example suites are red at HEAD** (so they can't be trusted as references):
  ...
  fail on the CSRF foot-gun above. *Fix:* the reserved import + the CSRF audiences, then keep them green.
...
- [ ] **No durable background jobs / scheduled tasks / job queue.** Explicitly deferred (`SPEC.md:728`);
  ...
  *Workaround:* `endpoint()` + external cron, `webhook()` for inbound callbacks, inline mutation
  side-effects, or the §10.1 outbox/exempt-table pattern. *Fix:* add a `job()` primitive (durable retry +
  scheduling).
- [ ] **No first-class email-verification flow.** `@kovojs/better-auth` exports only sign-in/up/out; the
  `verification` table is schema-bridge-exempt only (`internal/contracts.ts:229`). *Workaround:*
  better-auth `emailVerification` via `mount()`, or hand-write `mutation()`s over `auth.api`. *Fix:* ship
...
- [ ] **2FA is mis-handled, not just missing.** The credential mutation classifies better-auth's
  ...
  sign-in hard-fails silently; there is no challenge/verify flow. *Workaround:* `mount()` better-auth's
  2FA endpoints + hand-author the UI. *Fix:* return a typed two-factor-required outcome + a verify mutation.
- [ ] **No first-class markdown / rich-text primitive.** Kovo ships only the HTML-sanitizer floor
  ...
  Q&A site). *Workaround:* `safeRichHtml(markdownToHtml(body))` with code emitted unescaped. *Fix:* ship
...
- [ ] **Instance-keyed query optimism is unauthorable inline.** The inline `mutation({optimistic})` map
  keys by query *name* only — no `keys` field (`packages/compiler/src/scan/optimistic-inline.ts`) — so
  ...
  but isn't wired from `mutation.optimistic`. *Fix:* add a `keys` map to the inline form, or wire + document
...
- [ ] **No read-side cursor-append ("load more") primitive.** `@kovojs/server` exports only `stream`;
  ...
  *Fix:* a read-side `paginate`/append emitting compiler-derived keyed `mode='append'` rows that upsert
  into one client collection.
- [ ] **No `mode='prepend'` (chat "load older messages").** Only `append`|`replace` (`wire-html.ts:89`);
  append writes to the DOM end. *Workaround:* `flex-direction: column-reverse` + newest-first + append
  (Slack-style; undocumented). *Fix:* document the idiom or add `mode='prepend'` with a scroll-anchor contract.
- [ ] **No social sign-in helper.** No `signInSocial` anywhere; `mount()` (`mount.ts:35`, SPEC §871) is the
  ...
  (POST init + GET callback need two mounts). `oidcProvider` metadata is KV406-degraded. *Fix:*
...
- [ ] **No password-reset helper.** §6.6 names it only as an example anonymous-CSRF form. Achievable via
  ...
  internal-only. *Fix:* add `betterAuthRequestPasswordReset`/`ResetPassword`, or at least re-export the union.
- [ ] **`guards.rateLimit` lacks `per:'ip'`** though SPEC §9.5:935 says it admits it (`guards.ts:265`); no
  request-IP accessor. (Coarse pre-dispatch per-IP shedding *does* exist in `app-load-shed.ts`.) *Fix:* add
...
- [ ] **`refetchOnFocus` / `live` not declarable on the query.** Focus-refetch works but its opt-out is a
  ...
  exists though SPEC §9.3:905 specs it. *Fix:* add `refetchOnFocus?: false` and `live?: true` to
  `QueryDefinition`.
- [ ] **Mutation `redirectTo` is untyped.** A plain string (`mutation/definition.ts:219`): no PathParams
  ...
  pages, `route.ts:1211`). Renaming `/chat/:id` would not flag the create-then-navigate redirect. *Fix:*
...
- [ ] **Compiler-emitted registries aren't in the app `tsc` program.** KV310/`OptimisticFor` only bite
  ...
  produces them; they're just not wired into the example's tsc.) *Fix:* fold emitted registries into the
  app tsc program.
...
- [ ] **No public optimism test helper.** The runtime primitives (`OptimisticRebaser`, `createQueryStore`)
  ...
  internal imports. Minor. *Fix:* a small public transform-soundness helper.
...
- [ ] **Live markdown preview is effectively blocked client-side.** A md→HTML transform in a render
  ...
  `textContent` in an L1 island (no client raw-HTML sink). *Workaround:* render the preview via a
  server-fragment round-trip (1 RTT — not instant). *Fix:* document preview-as-server-fragment, or provide a
...
- [ ] **Per-user vote state** requires threading `myVote` into the query result *and* a client-held pre-image
...
- [ ] **`<kovo-live>` SSE live queries unimplemented.** No `text/event-stream` subscriber in any package
  ...
  notifications. Roadmap `plans/data-layer-roadmap.md:28`; SPEC §9.3:905. *Workaround:* poll `/_q/` on a
  timer + BroadcastChannel (same-user only). *Also:* emit a compiler diagnostic for the inert tag.
...
- [ ] **Stream resume-after-reload** — `streaming.ts` is a single-pass iterator over one POST; no job
  ...
  *Workaround:* persist assistant output incrementally (a real chat persists messages anyway).
...
- [ ] **Presence / typing indicators** — depend on the unshipped SSE subscriber (SPEC §7 L4); no primitive,
  no interval-poll helper. *Workaround:* degraded `/_q/` heartbeat poll.
...
- [x] **Postgres FTS via `sql<>` opaque projection** — KV410 (`output`) + KV422 (params-not-text) fire
  correctly *when the gates are run*; ran on PGlite returning ranked rows. Plus a zero-config ILIKE fallback.
...
- [x] **Tags (M2M), comments, accept-answer (`owns()`), reputation (SUM)** — all type-check; only the *example*
