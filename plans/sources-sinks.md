...
- [x] Add a general "unregistered sink" diagnostic for app source.
  - Evidence: `pnpm exec vitest --run packages/cli/src/index.kovo-check.test.ts packages/cli/src/index.kovo-explain.test.ts packages/cli/src/commands-manifest.test.ts` verifies `kovo check` turns structured `unregisteredSinks` graph facts and generic KV423 source spans into error-level teaching diagnostics with a safe-surface hint.
