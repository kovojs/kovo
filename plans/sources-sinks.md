...
- [x] Header/cookie corpus.
  ...
  - Payloads: CR/LF/NUL/DEL/control chars, multi-cookie injection attempts, semicolon cookie value attempts, quoted filename breakouts, Unicode bidi filename spoofing, bad header names, reserved `Kovo-*` app writes, cache header overrides on private data, and raw `Set-Cookie` forwarding from session providers.
...
- [x] File/storage/static-export corpus.
  ...
  - Payloads: traversal in params/filenames/storage keys, dot segments, backslashes, absolute paths, symlinks where relevant, unsafe MIME/SVG/HTML inline, content-disposition injection, Unicode bidi filename spoofing, oversized uploads, metadata control chars, Vite manifest path escapes, generated-output temporary-name tampering, and static export links to reserved dynamic endpoints.
...
- [x] Allocate source/sink diagnostic codes after checking `diagnosticDefinitions`.
  - Evidence: `pnpm exec vitest run packages/core/src/diagnostics.test.ts`, `pnpm run check:api-surface`, and `git diff --check` verify KV423-KV426 are allocated in `diagnosticDefinitions`, the inline registry snapshot, SPEC §11.3, and the diagnostics guide without widening the public API baseline.
...
- [x] Add a general "unregistered sink" diagnostic for app source.
  - Evidence: `pnpm exec vitest --run packages/cli/src/index.kovo-check.test.ts packages/cli/src/index.kovo-explain.test.ts packages/cli/src/commands-manifest.test.ts` verifies `kovo check` turns structured `unregisteredSinks` graph facts and generic KV424 source spans into error-level teaching diagnostics with a safe-surface hint.
...
- [x] Add `kovo explain --trust`.
  - Evidence: `pnpm exec vitest --run packages/cli/src/index.kovo-explain.test.ts packages/cli/src/commands-manifest.test.ts` verifies `kovo explain --trust` lists trusted HTML/URL-style escapes, webhook `verify: none`, and static export path overrides with source, owner, safe path, site, and justification fields; `pnpm run check:api-surface` verified no new public API-surface debt.
...
- `pnpm exec vitest run packages/core/src/diagnostics.test.ts` verified source/sink diagnostic allocation KV423-KV426 in `diagnosticDefinitions` and snapshots.
...
- `node packages/cli/src/bin.ts check sources-sinks` verified current-repo drift scan output: `DRIFT-SCAN roots=packages|examples|site|tests files=3548 hits=1937 findings=581 unregistered=0 status=accounted`.
...
- `pnpm exec vitest --run packages/cli/src/index.kovo-check.test.ts packages/cli/src/index.kovo-explain.test.ts packages/cli/src/commands-manifest.test.ts` verified KV424 unregistered app-authored sink diagnostics.
