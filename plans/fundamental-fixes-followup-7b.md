...
- [x] **A1 — When the runtime resolves the `pglite` driver AND is serving production traffic, refuse to boot.** In the boot/config path (`resolveConfig`/`createPostgresAppRuntime`, keyed off the same production signal the rest of the runtime uses — e.g. `NODE_ENV==='production'` / the built prod artifact, not `KOVO_PARANOID`), fail closed with: `KV433: production requires a least-privilege external Postgres via KOVO_DATABASE_URL; PGlite is dev/test-only and runs in-process as superuser (SPEC §10.3).` Dev/test PGlite (default no-env path) is unchanged.
  ...
  - Evidence: `packages/server/src/postgres-runtime.test.ts` `refuses production boot on in-process PGlite`; `packages/create-kovo/src/index.build.prod-artifact.postgres-external.test.ts` proves the executable exits before opening its Node listener.
- [x] **A2 — Keep the followup-7 A1 boot invariant (`runtimeConnectionLeastPrivilegeIssue`, `postgres-runtime.ts:1390-1414`, `KV433_RUNTIME_ROLE`) as the belt-and-suspenders on the managed path.** With DEC-A1 in place, the two together make "in-process superuser + real tenants" unreachable: dev is PGlite (single-tenant), prod is a proven least-priv external role.
  ...
  - Evidence: `packages/server/src/postgres-runtime.test.ts` (97 tests) covers runtime-login closure and elevated-role refusal; the external production-artifact test exercises the least-privilege URL end to end.

### DEC-B — Demote B4-on-PGlite to a documented, dev-only, defense-in-depth residual

- [x] **B1 — Record in SPEC §10.3 that PGlite is a dev/test-only single-tenant database that runs in-process as superuser; the owner-scoping/confidentiality guarantee is provided by the least-privilege managed runtime role (A1) and the closure audit (followup-7 DEC-C), NOT by the in-process dev DB; a deliberately raw-opened PGlite handle in dev is superuser by nature and is caught only by the defense-in-depth lint (followup-7 A3).**
  ...
  - Evidence: `SPEC.md` §10 and `spec/10-data-plane.md` §10.3 state the boundary; `security/TCB.md` enrolls `assertProductionRuntimeDriver`; KV433 remains `runtime-choke` in `packages/core/src/internal/security-markers.ts`.

### DEC-C — Amend followup-7 to reflect the resolution

- [x] **C1 — In `plans/fundamental-fixes-followup-7.md`: rewrite DEC-A2 from "make raw PGlite least-priv" (infeasible) to "production forbids in-process PGlite; the B4 residual is dev-only, single-tenant, lint-warned, prod-harmless via A1 — see followup-7b." Mark A2 `[x]` resolved-by-relocation, and close the §6 probe (line 100) with the same disposition.**
...
- [x] Prod artifact, no `KOVO_DATABASE_URL` → boot refused with KV433 (new test).
- [x] Prod artifact, least-priv `KOVO_DATABASE_URL` → serves; raw `new Client(url)` reconnect reads 0 owner rows
