...
- [x] **A1 — A fresh app cannot deploy any SPEC §7 client island via ANY documented path: once an island lowers to a versioned `/c/__v/` module, `build:prod` fails `KV417` (the §14 deploy-skew retention floor) on every built-in preset (node/vercel/cloudflare), and no config field, CLI flag, env var, or preset option lets an author declare retention.** (high, framework; found by `t1-island-deploy`; next wall after super-7 A1)
  ...
  - Fixed 2026-06-29: built-in `node()`, `vercel()`, and `cloudflare()` presets accept `retention: { hours, immutableClientModules: 'retained', priorTokenQueryReads: 'retained' }`; KV417 still fires by default and below 24 hours, but a retention-declaring preset satisfies the SPEC §14 floor.
  - Evidence: `pnpm exec vitest run packages/server/src/build.test.ts` proves the default KV417 failure, under-floor rejection, and green retention-declaring built-ins; `pnpm exec vitest run packages/cli/src/index.kovo-build.test.ts packages/cli/src/index.kovo-build-browser.test.ts` proves a retention-declaring node preset builds a client-island app and hydrates the prod artifact.
...
- [x] **C1 — `kovo export <app> --out dist` silently ships an unstyled site: route documents `<link>` to `/assets/*.css` that the exporter never writes and never warns about (no `--manifest`).** (med, framework; found by `t4-static-export`)
  - Evidence: `pnpm exec vitest run packages/cli/src/index.kovo-export.test.ts` proves bare export now copies referenced `/assets/styles.css` when present and emits KV229 before writing output when the referenced asset is missing.
...
- [x] **C2 — A `publicAccess()` route that renders a per-form CSRF token passes the export route plan as an exportable target, then aborts mid-render with a `Set-Cookie` KV229 — a late, non-actionable, all-or-nothing failure without `--skip-non-exportable`.** (low, dev-tooling; found by `t4-static-export`)
  - Evidence: `pnpm exec vitest run packages/server/src/static-export-route-plan.test.ts` proves publicAccess routes in an app with checked mutations and CSRF now get an upfront KV229 route-plan diagnostic naming anonymous CSRF `Set-Cookie`, while all-`csrf:false` mutation apps still export public routes.
...
- [x] **D1 — Deferred `<Defer>` regions are buffered, not streamed: the slowest region blocks the entire TTFB and the shell never paints first.** (high, framework; found by `t5-streaming`)
  ...
  - Fix evidence: `pnpm exec vitest run packages/server/src/deferred-stream.test.ts packages/server/src/deferred-region.test.ts packages/server/src/wire-fixtures.test.ts packages/server/src/document.test.ts packages/server/src/app-document.test.ts --run --reporter=dot` proves route documents now return a `ReadableStream`, flush the shell/fallback before a slow deferred region resolves, then stream each chunk as it settles.
...
- [x] **D3 — A hung / never-resolving `<Defer>` region hangs the whole request indefinitely — no per-region timeout, and the shell is never delivered.** (med, framework; found by `t5-streaming`)
  ...
  - Fix evidence: `pnpm exec vitest run packages/server/src/deferred-stream.test.ts packages/server/src/deferred-region.test.ts packages/server/src/wire-fixtures.test.ts packages/server/src/document.test.ts packages/server/src/app-document.test.ts --run --reporter=dot` proves a never-resolving region no longer blocks the shell, and after `timeoutMs` it streams a per-region `state="error"` fallback chunk.
