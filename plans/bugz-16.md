...
- **B1 (self-verified, clean `create-kovo` default scaffold, prod build):** add-contact success response `Content-Type: text/vnd.kovo.fragment+html`, `Kovo-Changes` set, **body empty**, DOM unchanged; 422 path emits a fragment. Root-cause chain confirmed in source: `build-export.ts:2078-2083` (registry plugin before kovo), `compiler/src/vite.ts:489-492` (reentry guard matches `__kovoAssignDerived`/`componentLiveTargetRenderer`), `mutation.ts:551-614` (empty query+fragment chunks). Shared with `papercuts-super-7` A1.
