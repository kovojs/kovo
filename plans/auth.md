...
- **B5 — `mount()` for browser-redirect protocols.** The better-auth handler mounted via A4 `endpoint()` for the flows that are irreducibly redirect-based: OAuth callbacks, SAML ACS, magic-link verification. This is the *only* place the mounted handler is the surface; credential flows stay on B4.
