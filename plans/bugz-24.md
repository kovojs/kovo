...
- [x] **A1 - The generated Better Auth system DB handle can be routed through app code to read secret auth columns as plain strings.** (high, template/framework security; found by `track-secret-egress-postgres`, independently verified)
  ...
  - **Evidence:** the current starter keeps the raw system DB, Better Auth instance, and adapter inside
    `_kovo/app-runtime-db`, exposes only sanitized bindings, and the focused `KOVO_PARANOID` scaffold
    regression rejects both `$context` credential reads and factory re-export.

- [x] **A2 - Mutable separated SQL carriers can pass managed write validation with one SQL text and execute another.** (high, framework security/soundness; found by `track-raw-sql-runtime`, independently verified)
  ...
  - **Evidence:** managed DB now passes a frozen `ManagedSqlStatement` snapshot to drivers, refuses
    accessor-backed carriers before execution, and executes the first descriptor snapshot for proxy
    carriers; all three focused regressions pass.
...
- `pnpm exec vitest run packages/create-kovo/src/index.test.ts packages/server/src/managed-db.test.ts --testNamePattern "confines the generated Better Auth instance|fails closed on getter-backed separated carriers|executes the first proxy snapshot|passes a frozen ManagedSqlStatement snapshot" --reporter=dot`: 4 focused regressions passed.
