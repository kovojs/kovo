...
- [ ] **Custom 404/500 error shells bypass document security/header defaults.** (med, framework; found by `request-shell-errors`)
...
- `pnpm run dev -- --host 127.0.0.1 --port 41789` in `request-shell-errors` plus verifier `curl` probes confirmed the 404/500 header gap and hardened 403 control; the server was killed after the run.
- Main-thread self-check repeated the curl probes on port `41801` with the same result.
