...
- [x] **A1 — Multi-component state modules validate later components against the first component's state shape.** (high, framework; found by `t1-state-islands`, verifier `Ptolemy`)
  ...
  - Fixed evidence: `pnpm exec vitest --run packages/compiler/src/state-bindings.test.ts packages/compiler/src/query-bindings.test.ts --reporter=dot` passes 52 tests, including valid multi-component state bindings and a later-component missing-key KV302 negative.

- [x] **A2 — Build graph derivation still drops Drizzle output-schema facts for direct computed-field bindings.** (high, framework deploy-gate regression; found by `t3-drizzle-optimistic`, verifier `Zeno`; regression of `plans/papercuts-16.md` C1 / `plans/papercuts-super-5.md` A1)
  ...
  - Fixed evidence: `pnpm exec vitest run packages/cli/src/index.kovo-build.test.ts packages/server/src/vite-data-plane-gate.test.ts --reporter=dot` passes 50 tests / 1 skipped, including graph-derivation declared-output direct-binding coverage.

### B. Storage capability route method dispatch

- [x] **B1 — Signed `HEAD` storage capability URLs 404 before reaching the framework download handler.** (med, framework; found by `t2-files-capabilities`, verifier `Helmholtz`)
  ...
  - Fixed evidence: `pnpm exec vitest run packages/server/src/shell.test.ts packages/server/src/app-dispatch.test.ts packages/server/src/capability-route.test.ts packages/server/src/endpoint.test.ts packages/server/src/app.test.ts --reporter=dot` passes 120 tests, including mounted signed HEAD dispatch through the app shell.
...
- Fix pass: state/compiler, build/data-plane, and endpoint/storage focused tests listed under the completed issues passed after integration.
