...
- [x] **C6** ✅ real-Postgres declared-version CAS race (`postgres-external-probe.test.ts`) + ✅ derived-optimism (`optimistic-success` — prediction 2→6, reconcile →8 via a `<kovo-query>` wire element so the store settles to server truth) + ✅ multi-transform rebase (`optimistic-rebase.spec.ts`). The former PGlite in-DB arithmetic race was retired because it could not witness a lost update. Queue mechanism exists in `optimism.ts` and is exercised via the rebase/pending flow.
