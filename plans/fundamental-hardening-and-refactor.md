...
- Independent hvr fan-out (no dependency on the above):
  ...
  - [ ] **U2** shared `isActivationKey` in `lib/keyboard-navigation.ts` (add legacy `'Spacebar'` to `select.ts:945`; siblings dropdown/context/menubar have it). S · low.
  - [ ] **U3** shared active-descendant/`describedBy` helpers in `lib/active-descendant.ts` (byte-identical at `combobox.ts:1007-1052` / `autocomplete.ts:1147-1191`). M · low.
  - [ ] **U4** thread `now` through `selectKeyDown` (`select.ts:980` hard-codes `Date.now()`; siblings inject `now`; `selectTypeahead` already accepts it). S · low.
