...
- [x] **Q.5 secret-material lifecycle** — `console.*` / logger / `reportServerError` / `createDurableTaskStatus` /
      at-rest columns scrub secret-tagged values via the DEC5 provenance tag.
  - Evidence: `pnpm exec vitest --run packages/server/src/build.test.ts packages/server/src/logging.test.ts packages/server/src/diagnostics.test.ts packages/server/src/task-queue.test.ts packages/server/src/task-observability.test.ts packages/server/src/task-runtime.test.ts scripts/check-sink-policy-gate.test.mjs`, `pnpm run check:sink-policy`, `pnpm run check:api-surface`, touched-file `vp check`, and `git diff --check origin/main..HEAD` passed after logger/console diagnostics, `reportServerError`, durable task status, and `_kovo_jobs` args/last_error writes scrubbed runtime secret-tagged values.
...
- [x] **Q.7 DoS / rate-limit** — per-principal bucket keyed on a proven identity, not a shared `unknown` bucket.
  - Evidence: `pnpm exec vitest --run packages/server/src/guards.test.ts packages/server/src/guards-rate-limit-ip.test.ts packages/server/src/app.test.ts`, `pnpm run check:security-brands`, touched-file `vp check`, and `git diff --check` passed after guard-level session rate limits keyed through `provenPrincipalFromRequest()` and unresolved/anonymous principal carriers failed closed instead of sharing an implicit unknown bucket.
- [x] **Q.8 timing side-channel** — constant-time compare on secret-tagged material.
  - Evidence: `pnpm exec vitest --run packages/core/src/secret.test.ts packages/core/src/verifier.test.ts packages/server/src/keyring.test.ts packages/server/src/csrf.test.ts packages/server/src/capability-url.test.ts packages/server/src/webhook.test.ts packages/server/src/mutation-wire.test.ts`, structural `rg` over secret/signature comparison paths, touched-file `vp check`, and `git diff --check` passed after framework signing and live-target attestation comparisons moved malformed-length candidates onto fixed-digest `timingSafeEqual` paths.
...
- Independent hvr fan-out (no dependency on the above):
  ...
  - [x] **T3** one CLI arg-parsing framework: express check/audit/explain as `CommandArgvSpec`s (`graph-output.ts:1261` `parseFlaggedArgs` diverges from `commands-manifest.ts:708` `parseCommandArgv` on `--flag=value`/missing-value/unknown-option); delete `parseFlaggedArgs`. M · med. _(precedes T6)_
    - Evidence: `pnpm exec vitest --run packages/cli/src/commands-manifest.test.ts packages/cli/src/tooling.test.ts packages/cli/src/graph-output.args.test.ts packages/cli/src/commands/argv-helpers.test.ts packages/cli/src/index.kovo-check.test.ts packages/cli/src/commands/build-export.context.test.ts packages/cli/src/index.kovo-compile.test.ts packages/compiler/src/scan/optimistic-inline.test.ts`, touched-file `vp check`, and `git diff --check HEAD~1..HEAD` passed after replacing graph-output ad hoc flag parsing with shared command argv specs while preserving the O.2 KV310 fragment-only warning.
  - [x] **T4** shared `commandArgvError(name,error,usage)` + `requireSinglePositional(...)` (`build-export.ts:153,226` byte-identical; `compile.ts:587-822` repeats the mapper 7×). M · low.
