...
## Status (integrated to `main` 2026-06-25, merge `44a04f7da`)

**17 of 18 fixed and merged** (checked below). Each fix was built in its own worktree/branch, integrated
on `bugz-integration`, and verified by the full unit suite (4815 passing; only 2 unrelated **pre-existing**
failures remain — `site/scripts/api-examples-check` and `examples/gallery/.../interactive-gallery.artifacts`)
plus gates: `check:api-surface` (no API drift), `check:sink-policy`, `check:imports`,
`check:inline-loader`, `check:kovo` (28/28 — drizzle changes do not over-flag examples), and `check:vp`
(typecheck + format).

- **H6 upgraded during integration:** the trust-brand witness moved from a per-instance `WeakSet` to a
  process-global `Symbol.for` marker — still non-forgeable from wire JSON, but valid across module
  instances (an app loaded through a separate Vite SSR graph, the real TS static-export path, now renders
  its `trustedHtml` correctly instead of as escaped JSON).
- **M2 (double-escape) — DEFERRED, NOT fixed (left open below).** The fix branded `escapeText` as
  `RenderedHtml` so the runtime's second escape pass would be a no-op, but `escapeTextWithRenderedHtml`
  coerces any branded value to a deferred coerced-rendered-html marker, and the list-stamp /
  live-component server-render boundary never resolves that marker — it leaked `kovo-rendered-html:<rand>:N`
  into shipped example output (commerce/tutorial). Reverted to a plain escaped string: the double-escape
  returns (fail-safe over-escape, **never under-escapes → no XSS**), and the leak is gone. The correct fix
  resolves coerced-rendered-html at that emit boundary (or drops the compiler `escapeText(` injection and
  updates the §5.2 equivalence gate).

---

## HIGH

- [x] **H1 — Server JSX runtime emits attribute _names_ unescaped; untrusted spread keys break out of the tag (stored XSS).** `packages/server/src/jsx-runtime.ts:251-253`
...
- [x] **H2 — Egress private-network floor validates only `address[0]` of a multi-A DNS answer, then forwards the whole array (SSRF / DNS-rebind / cloud-metadata).** `packages/server/src/egress.ts:613-618` (mirror: `egress-undici.ts:114-116`)
...
- [x] **H3 — KV438 mass-assignment gate: an input field named `session`/`guard`/`tenant` is mis-classified as trusted server scope (cross-tenant write / privilege escalation).** `packages/drizzle/src/static/session-provenance.ts:454-456` _(shared root with H5)_
...
- [x] **H4 — KV422 SQL-safety analyzer recognizes Kovo `sql.raw`/`identifier` only by the literal receiver name `sql`; aliasing/namespace import defeats it (SQL injection via query-builder ORDER BY/WHERE).** `packages/drizzle/src/static.ts:766,835`
...
- [x] **H5 — KV414 owner-scope (IDOR) gate: a `.session.`/`.guard.` member access is trusted as the session principal even when rooted at client input.** `packages/drizzle/src/static/session-provenance.ts:454-456` _(same root function as H3)_
...
- [x] **H6 — Client URL/HTML trust brands (`__kovoTrustedUrl`/`__kovoTrustedHtml`) are structurally forgeable from wire/query JSON (`javascript:` href + SSR HTML injection).** `packages/browser/src/security-output.ts:120-126,155-161,199-220`
...
- [x] **H7 — ReDoS analyzer fails open: an unescaped `)` inside a character class hides the quantified group, accepting a catastrophic `pattern()`.** `packages/server/src/redos.ts:428-443` + `packages/compiler/src/validate/redos-pattern.ts:188` (identical copy)
...
- [x] **H8 — Webhook idempotency floor is keyed on `recordChange()`, not on actual writes; a tx-direct/outbox write double-executes on provider retry.** `packages/server/src/webhook.ts:407-421` (early return at :411; check at :357 runs _after_ commit)
...
- [x] **H9 — Webhook reserve-lost race falls through to execute instead of failing closed; concurrent redelivery double-executes with a durable store.** `packages/server/src/webhook.ts:328-356` (fall-through after :340)
...
- [x] **M1 — No-JS mutation submissions have no replay/idempotency protection; the handler double-executes on duplicate/concurrent submit.** `packages/server/src/mutation.ts:658-672, 781-787, 833, 895-939`
...
- [x] **M2 — Compiler-injected `escapeText` double-escapes every data-path text binding (`&`→`&amp;amp;`).** `packages/compiler/src/lower/structural-jsx.ts:1176` (and :1201,1224,1246,1268,1280,1310); interacts with `server/src/renderable.ts:39`
  ...
  - **Evidence:** `pnpm exec vitest run packages/server/src/m2-double-escape.test.ts packages/server/src/jsx-runtime.test.ts --reporter=dot` passed on 2026-06-29; `packages/server/src/m2-double-escape.test.ts` proves compiler-injected `escapeText(...)` values render single-escaped through `renderServerRenderable`, nested JSX, list-stamp, live-component, render-tree, and string-coercion paths.

- [x] **M3 — `verifyCredential` decoy digest is pinned at floor params, leaking account existence by login timing when the app hardens hashing.** `packages/server/src/password.ts:80-81, 142-156`
...
- [x] **M4 — Static export drops the strict CSP / X-Frame-Options / COOP / Permissions-Policy floor; every prerendered page is weaker than dynamic dispatch.** `packages/server/src/static-export-output.ts:242-263` (+ `static-export-output-targets.ts:48-88`, `build.ts:394-398`)
...
- [x] **L1 — Classless cookies fail _open_: an omitted `class` defaults to `app-data` (no HttpOnly/Secure), a default-allow the framework should fail closed.** `packages/server/src/cookies.ts:162-179, 200-206`
...
- [x] **L3 — List-stamp template renders item-relative _attribute_ bindings (`data-bind:href`) as escaped _text_, clobbering the element's label.** `packages/compiler/src/analyze/query-bindings.ts:127-168` → `compiler/src/emit/client.ts:311-336`
...
- [x] **L4 — Inline-loader URL scheme check (`uu`) uses a looser regex than the canonical primitive, rewriting valid relative URLs to `#`.** `packages/browser/src/inline-loader-build.ts:183-186` (+ regenerated `inline-loader.ts`)
...
- [x] **L5 — `sniffUploadBytes` marks ZIP/OOXML containers `inlineSafe`, contradicting its own download-only invariant.** `packages/server/src/upload-sniff.ts:108-117` (comment at :136-137)
