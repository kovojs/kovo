...
- **postgres-authz-policy**: `kovo.principal` provenance (`principalFromRequest`, the per-request
  ...
  SQLite-only string-justification `authzPolicy` variant and the fail-closed Postgres rejection;
  the coexisting `kovo_system_scope`/`kovo_admin_scope` policies (`crossOwnerRead`); reproducing scoped reads
