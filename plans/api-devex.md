...
TanStack Router/Start, shadcn/ui, Radix UI, and Base UI** — would they correctly _guess_ each
API's name, shape, and behavior, and is each one documented at the point of use? This is a
_familiarity/clarity_ audit, not the conservative "less surface is more" audit (that lens lives
...
is _post-verification_; where an adversarial verifier adjusted it, the original is noted
(`H→M`) with the reason.
...
The public surface is _mostly_ well-shaped — routing (`href`/`route`/`redirect`), the styling
...
`query` from `@kovojs/server`," and `session()` is a _schema declaration_ named like a runtime
...
`@kovojs/style#attrs` occupies StyleX's `props` slot while _reading_ like styled-components
...
`/generated` subpaths and reference them _structurally_ so they vanish from app-facing docs;
...
element queries; `defineVariants` _is_ cva and `cn` _is_ a clsx-style joiner, but both are hidden
...
tracking. These are defensible _architectural_ choices, but they are undiscoverable.
...
`@internal` on every member yet is the _only_ path to app-required token CSS (`kovoUiTokenSheetCss`,
...
(each primitive is its own package) and ships shared utilities as _public_ `@radix-ui/react-*`
...
gap before v1 — fix the code _or_ downgrade the spec, but don't leave a public surface that
...
| M    | `@kovojs/server#session`                                              | → `defineSession` / `sessionSchema`                                      | Auth.js `getSession()`/`auth()` _return_ the session                     |
...
> **⚠ withdrawn** failed verification and are recorded for completeness, _not_ recommended.
...
- **`Enhanced*Mutation*` family** (`mutation-fetch.ts:43`: keep `EnhancedMutationFetch`+`Options`
  as the `defaultEnhancedFetch`-wrapper type — _rename_ to `MutationFetch`; relocate
...
- **Verifier note (why weakened H→M):** several of these are _reachable_ from public option
...
- ~589 type-export rows (`command.ts:80` representative) show `named imports: 0, examples: 0`.
  ...
  with _no_ public Reason/Detail/Result types) and is almost entirely unused. It makes every
...
- The published `.` entry resolves to `export {}`. `import { … } from '@kovojs/headless-ui'` (the
  ...
  Radix avoids a root barrel by _not publishing_ `.` at all — an empty-but-present `.` is worse
...
- `documentQueryScriptBehaviorFact` (`html-fragment.ts:284`) bundles rendered query-script
  ...
  query-script placement. The unused `*Fact` _result_ interfaces (`HtmlDocumentFact`,
...
- Defaults to the framework-internal attribute `data-kovo-check-export` (`html-fragment.ts:208`)
  ...
  marker (a test uses `data-commerce-shell`), and it has a real internal caller, so it is _not_
...
- A deep structural validator (`app-guards.ts:9`), 1 import / 0 examples. **Verifier correction:**
  ...
  app-facing contract, _not_ a pure internal leak. The "app author never needs it" framing is
...
- The dynamic-import callback type (`handlers.ts:13`), exported from both `.` and `/client`, 0
  ...
  TanStack _do_ expose import-shaped option types in places, so "no React analogue" is too
...
- The intersected callback-carrier _return_ types of `kovo()` (`drizzle-surface.ts:50`); apps
...
- **Verifier correction:** `/internal` is a _deliberate_ framework-maintenance entrypoint used
  package-wide (`@kovojs/style/internal`, `@kovojs/cli/internal`, `@kovojs/core/internal/*`), and
  app source _cannot_ validly import `@kovojs/*/internal` (SPEC §442 + the authoring-surface
...
- The finding claimed the subpath duplicates 5 types already at the package root. **False
  ...
  If anything, consider surfacing those 4 from the root _and_ keeping or dropping the subpath —
...
- The published `./internal` subpath re-exports 46 `@internal` symbols, yet `kovoUiTokenSheetCss`
  ...
  app-required asset with no public path. Radix ships shared utilities as _public_ documented
...
- `Link(path, options) => { href }` (`index.ts:375`) — a non-JSX descriptor, the single most
  ...
  **Recommendation:** rename to `linkProps(path, options)` returning anchor-spreadable props
  ...
  prefetch" alternative _conflicts with SPEC §6.4_ — the lowered form is a plain anchor with "no
...
- Two public `query` exports: server `query(key, {load, reads, args})` (the loader) and core
  ...
  JSDoc documents _around_ the collision. **Recommendation:** rename the core handle to
...
- `session(schema)` (`guards.ts:276`) declares the session _shape_ (like `pgTable`/a zod schema),
  but collides with the ecosystem meaning (`useSession`/`auth().session`/`getSession` _return_ the
...
- `defineConfig(config: KovoConfig)` (`build.ts:93`) for `kovo.config.ts`, while every example's
  `vite.config.ts` imports a _different_ `defineConfig` from `vite-plus` — two same-named helpers
  in one project. **Verifier pushback:** the ecosystem norm is actually the _opposite_ — multiple
...
- `kovoTest(name, fn, options, runner?)` returns `{name, run}`, forcing a two-step
  ...
  _does_ include the manual `it(...)` re-registration — so the only true divergence is the
...
- Both public input types are `unknown` (`index.ts:3589/3598`), so the facade gives no
  compile-time help — every real call site imports the _typed_ `KovoExplainInput` from
...
- Returned by _both_ `kovoCheck` and `kovoExplain` (`index.ts:79`), so the check-specific name
...
- Two helpers for one job differing only by JSX-object vs HTML-string output, distinguished by a
  ...
  note:** the string variant _has_ real SSR-template call sites (2 in `examples/reference`), so it
...
- `guards.all(...items)` composes left-to-right, first-denial-wins (`guards.ts:201`). **Verifier
  ...
  name isn't as misleading as claimed; the real gap is that it returns a _guard composer_, not a
...
- `defineVariants` _is_ cva and `cn` is a clsx-style joiner (`variants.ts:57`/`class-names.ts:34`),
  both hidden behind `/internal` with 0 usage. **Verifier correction:** `cn` is **not**
  clsx+tailwind-merge — it's a clsx-style joiner with Set dedup, _no_ Tailwind conflict-merging;
...
- Toast adds an event-bus layer (`toastShowEventName='toast:show'`, `ToastShowPayload`,
  ...
  _additional_ layer), and `serverFactKeys` suggests the event names are an intentional
...
- No component accepts `className`/`class`; overrides go through `styles?: XStyleOverrides`
  ...
  biggest familiarity gap. **Recommendation:** keep the slot-map for per-part overrides but _also_
...
Content` compound set (`drawer.tsx:29`/`:48`). **Verifier correction:** `sheet.tsx` has the
  _same_ dual API — only `dialog.tsx` is compound-only — so "diverges from every other component"
...
- One callable named `form` carries `.get` for an unrelated concept (GET-route search forms vs POST
  ...
  _literal SPEC §6.3 spelling_ (SPEC.md:697) and is demonstrated in the routing guide — renaming it
...
> Each API below should _stay as-is_ but needs better docs — missing JSDoc, a usage example, a
...
| Conf | API                                                       | Doc gap → addition                                                                                                                                                                                                                                                                |
| ---- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M    | `s.boolean` coercion                                      | Empty string _and_ absent both become `false` (not a validation error) — surprising vs strict `z.boolean()`, hides bugs. Document the exact truthy/falsy token table and the absent-as-false rule (§6.3 form rationale).                                                          |
...
| M    | `csrfToken`/`csrfField`                                       | Bespoke and mostly auto-handled; docs don't say _when_ an app must call them (hand-written forms only). Document that framework-emitted forms include CSRF automatically; note the secret/sessionId requirement.                                                                |
...
| L    | Structured document primitives                                     | Resolved replacement for the former public `DocumentTemplate`/`DocumentParts`/`DocumentTemplateContext` surface. Keep docs centered on `Document`, `Head`, `BodyStart`, `BodyEnd`, and typed head primitives.        |
...
| M    | `runKovoCommand`                                | 0 inventory usage but it _is_ used by the create-kovo template (`export-static.mjs`). Document it as "generated-script-only; prefer the `kovo` bin," with a usage example.                             |
...
- **Rejected (1):** `@kovojs/server/app-shell/static-export` removal — false-premise (4 of 5
  types are _not_ duplicated at the root). Recorded under Remove as ⚠ withdrawn.
...
- **Factual corrections folded in:** `cn` is a clsx-style joiner _without_ tailwind-merge;
