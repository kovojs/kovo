...
All confirmed findings plus the promoted/contested items are implemented with red→green tests in worktree
`../kovo-bugs-part3` (branch `agent/bugs-part3`, off `main` `a817cb08`), built by 16 file-disjoint sub-agent
lanes merged + gated by the main thread. **Final gates (all green in the worktree):** `tsc --noEmit` 0
errors; full unit suite **3481 passed / 3 skipped / 0 failed (442 files)**; `api-surface-gate` clean
(1367 = baseline, 0 boundary violations — `resolveLifecycleRequest` and `emitQueryPlanBootstrapModule`
tagged `@internal`); duplicate-export check clean; `git diff --check` clean; inline-loader check clean.
(The 2 `import-boundary` warnings — `site/scripts` / `examples/stackoverflow` importing
`@kovojs/compiler/package-styles` — are pre-existing on `main`, exit 0, not introduced here.)

Lane → status (every box below is checked except the two noted):

- **Lane A (IDOR + invalidation):** A1 (write-side KV414 producer), A2 (`and()`/`or()` unwrap), A3
  (owner-column arg), A4 (list/aggregate rerun, canonical `name:keyValue` matcher), A5 (fold reads) — done.
  Contested L2-invalidation-2 folded into the A4 canonical-key rewrite.
...
- **Deferred-stream ordering:** L2-deferred-1 (NaN floor), L2-deferred-2 (author-order fragments within a
  ...
  and the co-location invariant the canonical fixture enforces already satisfies §9:769; reverted to per-chunk
  co-location. Left open; see Deferred.
...
> the fix**; tests that _codified_ the bug were inverted (`combobox.stylex.test.tsx.snap` activedescendant;
> the `domain:via:key` assertions in `change-record.test.ts`/`mutation.test.ts`; `meta.test.ts` meta-throw;
> the deferred-stream ordering goldens).
...
- [x] **A1 (critical) — KV414 IDOR audit never runs on writes; owner-table mutations keyed by a client arg are unenforced** `spec-impl-divergence` (L4-1)
...
- [x] **A2 (critical) — Query KV414 escapes via `and()`/`or()`-wrapped predicates (combinator-nested `eq` operands never extracted)** `impl-bug` (L4-2)
...
- [x] **A3 (high) — Owner-column key predicate with a client arg escapes KV414 because detection only matches the declared `key` column** `impl-bug` (L4-3)
...
- [x] **A4 (critical) — Keyed change-record never reruns a list/aggregate query reading the same domain → silent stale list** `impl-bug` (L2-invalidation-1)
...
- [x] **A5 (high, promoted) — `queryWithGeneratedReads` overwrites author-declared `reads:` instead of folding → silent under-invalidation** `spec-impl-divergence` (L12-2)
...
- [x] **B1 (critical) — Persistent compile cache survives any compiler upgrade: `compilerBuildId()` is a hardcoded constant** `impl-bug` (L8-1)
...
- [x] **B2 (high, promoted) — App client bootstrap collides query-update-plan exports across same-named components (duplicate import binding + plan clobber)** `impl-bug` (L3-1)
...
- [x] **B3 (low) — Persistent cache prune deletes content-addressed blobs shared by kept entries** `impl-bug` (L8-4)
...
- [x] **C1 (high) — `skip`-mode export silently drops every valid sibling page of a param route when one staticPath is non-exportable** `impl-bug` (L1-1)
...
- [x] **C2 (high) — Static export never prunes prior artifacts: a removed route keeps serving stale 200 HTML across rebuilds** `coverage-gap` (L1-2)
...
- [x] **D1 (high) — A dropped top-level non-collection field is silently retained; the KV416 round-trip fails** `spec-impl-divergence` (L2-query-delta-1)
...
- [x] **D2 (medium) — A malformed/non-object delta envelope is silently treated as a no-op apply and counted applied, instead of a delta-miss refetch** `impl-bug` (L2-query-delta-3)
...
- [x] **D3 (high, promoted) — BroadcastChannel rebroadcast applies delta chunks with NO build-token validation, defeating §9.1.1 base-version validation across a redeploy** `spec-impl-divergence` (L2-query-delta-2)
...
- [x] **E1 (high) — Mid-stream response error corrupts a 200 body with "Internal Server Error" instead of aborting the transfer** `impl-bug` (L11-1)
...
- [x] **E2 (high) — Node adapter throws on every HTTP/2 request: pseudo-headers (`:path`/`:method`/…) are copied into a web `Headers`** `impl-bug` (L11-2)
...
- [x] **E3 (medium) — Client disconnect is never propagated to the handler: `request.signal` is unset and there is no timeout** `coverage-gap` (L11-3)
...
- [x] **E4 (medium, promoted) — In-flight pending replay reservations bypass `maxEntries` and persist for the full TTL → DoS** `impl-bug` (L7-3)
...
- [x] **F1 (high) — Internal `//` runs are not collapsed: an empty path segment silently matches a param and changes the matched route** `impl-bug` (L2-route-matcher-1)
...
- [x] **F2 (high) — `resolveRouteMeta` throws on a missing query → hard 500 during head render; `document-core` never passes queries, so any `metaFromQuery` factory always throws** `impl-bug` (L2-early-hints-2)
...
- [x] **G1 (high) — Deferred-stream inline apply/cleanup `<script>` blocks are unhashed and omitted from `document.csp`, so a strict hash-CSP breaks deferred hydration** `impl-bug` (CSP-1)
...
- [x] **G2 (high, promoted) — `renderContentSecurityPolicy` omits `base-uri`/`object-src`/`frame-ancestors`/`form-action`, leaving the hash-locked `script-src` bypassable** `impl-bug` (CSP-2)
...
- [x] **H1 (high) — Duplicate mutation keys have no uniqueness diagnostic; invalidation registry is last-write-wins, server dispatch is first-match-wins** `coverage-gap` (L12-1)
...
- [x] **I1 (high) — Cookie re-emission silently drops `Partitioned` (CHIPS) and other unmodeled attributes, breaking embedded/cross-site login** `spec-impl-divergence` (L13-2)
...
- [x] **I3 (low) — `isSessionEstablishingSetCookie` claims `Expires`-in-past counts as deletion but only checks `Max-Age`** `spec-impl-divergence` (L13-4)
...
- [x] **J1 (high) — Tabs / Accordion / ToggleGroup keyboard nav defaults to `'both'` axes, contradicting their own rendered orientation** `spec-impl-divergence` (L9-1)
...
- [x] **J2 (medium) — Combobox synthesized `aria-activedescendant` points at an id no option ever renders** `impl-bug` (L9-2)
...
- [x] **J3 (low) — Accordion silently degrades a multiple-select accordion to single-select when `type` is omitted and the initial value is empty** `coverage-gap` (L9-3)
...
- [x] **K1 (high) — `cssLengthValue` appends spurious `px` to numeric CSS custom-property (`--var`) values** `impl-bug` (L14-1)
...
- [x] **K2 (medium) — `defineVars` numeric token values are px-ified by the served-CSS normalizer (distinct code path from K1)** `impl-bug` (L14-2)
...
- [x] **L1 (high) — A user key ending in the sidecar suffix `.kovo-storage.json` collides with another object's filesystem metadata sidecar (cross-object corruption + metadata disclosure)** `impl-bug` (L2-storage-1)
...
- [x] **L2 (medium) — The S3 adapter silently drops a caller-provided `etag` (honored by memory + filesystem) → cross-backend parity divergence** `impl-bug` (L2-storage-2)
