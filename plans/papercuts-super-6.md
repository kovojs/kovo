...
- [ ] **A2 — `vp dev` serves island client modules with an unresolvable bare import `@kovojs/browser/generated` and no import map, so no island hydrates in the dev loop (typing/clicks are silently dead despite correct SSR lowering).** (high, dev-tooling; found by `t4-islands`)
...
- **A1 (self-verified):** `t4-islands` `build:prod` green; only `/c/*.js` is `kovo-runtime.client.js`; `handler.mjs` has 0 `people-filter.client.js` refs and 11 raw un-lowered `onInput`.
- **A2 (self-verified):** `t4-islands` `vp dev` authed `/` carries lowered `on:input` → `.client.js#…`; that module's import is the bare `@kovojs/browser/generated`; document has no import map.
