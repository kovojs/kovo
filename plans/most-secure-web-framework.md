...
- [x] **§3 Sink-Token Brands** — the unified `Blessed<Sink>` substrate. See the dedicated section below;
      ...
      `pnpm run check:vp` passed. Optional-call constructor forms (`RegExp?.(...)`,
...
packages/drizzle/src/index.scope-audits.test.ts --run`, `git diff --check`, and `pnpm run check:vp` passed.
      ...
      scope-audit test and `git diff --check` passed. Negated range predicates such as `not(gt(...))`,
      `not(gte(...))`, and `not(between(...))` now feed the fail-closed arg-candidate path for read/write owner
      audits, while session/guard negated ranges remain excluded from exact `scope: session` proof; the focused
      ...
      `scope: unknown`; `pnpm exec vitest run packages/drizzle/src/index.scope-audits.test.ts --run`,
      `git diff --check`, and `pnpm run check:vp` passed.
      Latest spread-membership batch proves exact singleton owner predicates through unshadowed
      `Array.of(...tupleAlias)` when the spread source is a statically proven singleton tuple, while mixed tuple
      spreads remain `scope: unknown`; `pnpm exec vitest run packages/drizzle/src/index.scope-audits.test.ts --run`
      passed.
