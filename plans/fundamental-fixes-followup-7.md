...
- [x] **A2 — Resolve the in-process PGlite superuser residual by relocating the production boundary (followup-7b).** Framework-managed dev handles remain scoped and the internal raw handle remains capability-gated, but an independently opened raw PGlite handle cannot be made least-privilege. PGlite is therefore dev/test-only and single-tenant; production refuses it before listening and requires external Postgres whose runtime login passes A1. The B4 dev residual is app-author-authored, single-tenant, and lint-warned rather than part of Kovo's owner/confidentiality guarantee.
  - Evidence: `packages/create-kovo/src/index.build.prod-artifact.postgres-external.test.ts` proves the no-URL production artifact exits with KV433 before listening; `SPEC.md` §10 / `spec/10-data-plane.md` §10.3 records the relocated guarantee.
...
- [x] **DEC-A2 independent raw-PGlite handle:** probe closed by followup-7b. Direct PGlite opens as superuser and cannot authenticate a separate least-privilege connection, so production forbids PGlite; the raw-handle residual remains dev/test-only and defense-in-depth linted.
...
- **Make raw PGlite least-privilege vs relocate the boundary** — chose relocation (followup-7b). An independently opened in-process PGlite handle is necessarily superuser; production therefore requires external Postgres and rejects PGlite before listening, while dev keeps its zero-dependency single-tenant database and defense-in-depth lint.
