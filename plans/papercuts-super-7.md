...
- [ ] **A2 — Two stateful islands in one module: the SECOND island's valid local-state bindings fail KV302 (false positive), because state-shape validation resolves only the first component (the fail-closed state-side twin of `bugz-14` B3, which fixed the query half).** (med, framework; found by `t1-islands-runtime`)
...
- [ ] **B1 — Refetch-on-focus (SPEC §9.3) is structurally inert for the starter's server-fragment region: a freshly-loaded tab has no client query ledger to refresh, so returning to focus never re-runs the query over `/_q/`.** (low, docs/framework; found by `t2-liveness`)
...
- [ ] **C1 — `viewTransitionName` props lower to `view-transition-name` CSS but never animate: no cross-document `@view-transition { navigation: auto }` opt-in is emitted, and the loader's morph path never calls `startViewTransition()`.** (med, framework; found by `t3-navigation`)
...
- [ ] **C2 — The loader pins `history.scrollRestoration='manual'` unconditionally and only restores scroll on the enhanced path, so back/forward scroll position is lost on every full-GET fallback navigation.** (med, framework; found by `t3-navigation`)
...
- [ ] **C3 — An enhanced (JS) mutation form authored as a plain route-page helper (not `component()`) stamps `enhance` but never lowers `FormError` to a target anchor, so on the enhanced path the failure errors are silently swallowed — including the starter's own `LoginForm`.** (med, framework; found by `t3-navigation`)
...
- [ ] **D1 — `endpoint()` returns `404` (with the full HTML 404 shell, no `Allow` header) for a wrong-method request on an existing endpoint path, while routes and `/_q/` correctly return `405`.** (med, framework; found by `t4-endpoints`)
...
- [ ] **D2 — A content-negotiating `endpoint()` (vary body by `Accept`) is unrepresentable in the single-body response posture, and produces a dev/CI `500` (KV423) but a prod `200` — a posture audit that splits behavior for the same code.** (low, framework; found by `t4-endpoints`)
...
- [ ] **D3 — A thrown `endpoint()` handler returns the full HTML route-error shell (`text/html`, ~3.7 KB) even though the endpoint declared `body:'json'` — there is no endpoint-appropriate JSON 500 path.** (low, framework; found by `t4-endpoints`)
...
- [ ] **E2 — Keyboard focus is lost to `<body>` after a success-path mutation fragment morph when focus was on a non-input focusable (e.g. the submit button) — the starter's primary "Add contact" flow.** (med, framework; found by `t5-a11y-theme`)
...
- **A1 (self-verified):** authored island → `kovo build ./src/app.tsx` exit 1 (fail-closed); source confirmed at `build-export.ts:2078-2083` (registry plugin before kovo) and `compiler/vite.ts:489-492` (reentry guard matches `__kovoAssignDerived`). No app-accessible island-lowering flag exists.
- **bugz-16 B1 (self-verified, prod):** prod add-contact success body empty, DOM stale; shares A1's reentry-guard root cause.
- **E1 (self-verified):** `jsx-runtime.ts:275-278,333-335` applies boolean-presence to all attributes with no enumerated-ARIA carve-out.
- **A2, B1, C1–C3, D1–D3, E2–E3:** independently reproduced by the track verifiers and source-confirmed (`bindings.ts:188-199`, `inline-loader.ts`, `mutation-form.ts:35,76`, `shell.ts:165-181`, `endpoint.ts:11,23`, `app-request.ts:64-83`, `morph.ts:407-447`, `theme.ts:221,636-653`).
- Baseline: fresh default scaffold passes `pnpm run check`; islands hydrate in `vp dev` (super-6 A2 fix). Monorepo repaired; transitive deps resolve. Throwaway apps under `/Users/mini/kovo-dogfood-pg7-20260629/` (+ `-pg7-base`) safe to delete.
