...
- [x] **B1 — The production cookie floor renames Better Auth's session cookie to `__Host-…`, but Better Auth reads the unprefixed name, so production login is completely non-functional — every authenticated request bounces to `/login`.** (HIGH, framework; found by `t2-auth-lifecycle`)
  - Fixed: `runMutation` now exposes an internal forwarded-cookie sink that routes Better Auth credential cookies through the same name-preserving credential-floor normalizer used by session-refresh cookies; `forwardBetterAuthSetCookie` uses that sink when present, so the upstream Better Auth cookie name remains readable while `HttpOnly`/`Secure`/`SameSite` are still enforced.
  - Evidence: `pnpm exec vitest run packages/better-auth/src/index.credential-mutations.test.ts packages/server/src/cookies.test.ts --reporter=dot` passes and asserts a production Better Auth session cookie is re-emitted as `better-auth.session_token=...; Path=/; HttpOnly; Secure; SameSite=Lax` (no `__Host-` rename). A fresh default Postgres/PGlite starter at `/Users/mini/kovo-bugz15-prod-auth-20260629` built with `pnpm run build:prod`, run as `NODE_ENV=production PORT=3021 node dist/server/server.mjs`, then `POST /_m/auth/sign-in` returned that bare Better Auth session cookie; sending the returned `better-auth.session_token` value to `GET /` returned `200` with `Demo User` and `Sign out`.
