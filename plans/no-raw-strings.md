...
| `@kovojs/server#DocumentTemplate`                             | Closed: the public full-document string template surface was removed; app document customization uses structured `DocumentConfig` declarations instead. | `packages/server/src/document-core.ts`, `packages/server/src/app-types.ts`, `packages/server/src/document-structured.ts`, `site/src/document-template.tsx`                         | Keep closed. Do not reintroduce public raw document templates; add future escape hatches only as named, audited trust boundaries.                                                          |
...
- [x] Decide the `DocumentTemplate` exception explicitly.
  - Evidence: `packages/server/src/document-core.ts` no longer accepts a template callback,
    `packages/server/src/app-types.ts` exposes only `structured`/`lang` document options, and
    `site/src/document-template.tsx` uses structured document primitives.
...
- [x] `DocumentTemplate` should not remain an intentional low-level escape.
  - Evidence: `document.template` is removed from app options and public rendering barrels; the request-shell guide documents structured document primitives as the only app authoring path.
