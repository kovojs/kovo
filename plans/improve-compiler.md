...
- [x] Moved component graph fact and fragment-target registry fact derivation into `src/graph.ts`, leaving `index.ts` as the compiler orchestrator.
...
- [x] Extracted component contract validators into `src/validate/component-contracts.ts`: FW230, FW301, FW303, FW311, FW320, and FW330.
...
- [x] Extract route validation into `src/validate/navigation.ts`, preserving FW220 literal
      `href`/`action` diagnostics against declared routes from SPEC §6.4.
