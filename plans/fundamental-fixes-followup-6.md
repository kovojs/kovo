...
- [ ] **C2 — Record the residual honestly in SPEC §11.2: with C1, `observed ⊆ declared` is engine-enforced for the DANGEROUS cases (cross-owner via RLS, unclassified/reference via default-deny grant); the framework declared-write choke remains only for the BENIGN case (over-declaring among the caller's own owner tables), which is a coverage/invalidation contract, not a confidentiality/integrity boundary.**
  - Acceptance: SPEC §11.2 states the layer for each case; a test proves owner-RLS + narrowed writer grant bound the blast radius even when `createDeclaredWriteDb` is stubbed. (Reserve full per-mutation confinement (b) for post-v1.)
