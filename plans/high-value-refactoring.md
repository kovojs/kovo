...
- [x] **P1.5 - Make Better Auth operation contracts, graph facts, and cookie forwarding single-source.**
  ...
  - Evidence: `pnpm exec vitest --run packages/server/src/cookies.test.ts packages/better-auth/src/index.schema-bridge.test.ts packages/better-auth/src/index.credential-mutations.test.ts packages/better-auth/src/index.session.test.ts packages/better-auth/src/sqlite.seed-session.test.ts packages/server/src/mutation.test.ts packages/server/src/app-document.test.ts` and `pnpm --filter @kovojs/conformance-better-auth-pin test -- src/index.plugin-tables.test.ts` pass with operation contracts deriving touches/keys/access facts, the fixed binding owning auth construction, and server-owned `forwardSetCookie()` enforcing cookie forwarding posture.
