...
enforcement is *smeared across ~20k lines* (compiler recognizers + Drizzle analysis + server runtime); a bug anywhere
...
plus their brand constructors. Everything else — all app code, the *entire* compiler, all static analysis — is
**untrusted**: it may be arbitrarily buggy or hostile and the property still holds, because it cannot reach
egress/exec except through a choke. So *"is Kovo sound?"* collapses from "did we recognize all 10ᴺ authoring shapes?"
...
highest-leverage move in the plan: it is what *ends* the class rather than pushing it one layer deeper. The choke
inventory (DEC-J) proves the *sole door*; the TCB manifest + verification bar (A10/DEC-K) proves the *door is
correct*. Both are required — a verified choke that isn't the only door, or a sole door that isn't verified, each
leaves a fail-open.
...
- **DEC-L — Narrow the authoring surface so the common path is statically sound and the chokes stay small (the
  ...
  most apps are analyzable-by-construction at *zero* runtime cost, shrinking what the chokes must handle to the
  ...
  existing `check:sound-subset` to the whole security surface, failing closed on un-analyzable *code* (not
  un-analyzable *values*). Every path then lands in exactly one bucket — sound-subset (statically proven, free) or
...
- **DEC-M — Publish a precise guarantee statement + explicit non-goals (sound claims).** Replace any broad "proven
  ...
  primitives, author self-harm in their own server code, availability/DoS). The claim must be *narrower than or equal
  to* what the TCB provably enforces; a claim with no backing choke is a documentation bug.
...
| Enforcement logic added *outside* the TCB (a new server path decides a security property) | the TCB stops being the whole boundary → a fail-open anywhere | A10 + `check:tcb-boundary` lint (0.4, DEC-K) | 0.4 lint fails on out-of-manifest enforcement |
| The TCB grows until it is no longer verifiable | soundness stops being *tractable to prove* | A10 size budget + DEC-K verification bar | 0.4 budget check + exhaustive TCB property tests |
...
- **The TCB has a real maintenance discipline.** A hard size budget + a boundary lint means new features cannot smear
  enforcement back across the codebase; a genuinely new sink category costs a new *choke* (enrolled + verified), not a
...
(A10/DEC-K — so soundness remains tractable to *prove*, not merely to test), and **an external adversarial audit
