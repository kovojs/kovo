...
enforcement is _smeared across ~20k lines_ (compiler recognizers + Drizzle analysis + server runtime); a bug anywhere
in that surface is a fail-open. Confinement collapses the TCB to a **handful of small choke functions** — one DB
read-only wrapper, one declared-table writer, one non-coercible `Secret` box, one egress refusal, one renderer emit,
plus their brand constructors. Everything else — all app code, the _entire_ compiler, all static analysis — is
**untrusted**: it may be arbitrarily buggy or hostile and the property still holds, because it cannot reach
egress/exec except through a choke. So _"is Kovo sound?"_ collapses from "did we recognize all 10ᴺ authoring shapes?"
(uncomputable) to "are these few hundred lines correct, and are they the sole door?" — tractable to prove by
exhaustive tests, model-checking, or a proof assistant on functions that small. This TCB minimization is the single
highest-leverage move in the plan: it is what _ends_ the class rather than pushing it one layer deeper. The choke
inventory (DEC-J) proves the _sole door_; the TCB manifest + verification bar (A10/DEC-K) proves the _door is
correct_. Both are required — a verified choke that isn't the only door, or a sole door that isn't verified, each
leaves a fail-open.
...
Same pattern on read-only SQL, which also fixes the round-7 P2 over-block: `readonlyDb.execute(sql`select some*fn(x)`)`
— static doesn't know `some_fn`. Plan-2 → KV433 even for a pure `group_concat` (false block). This plan → static
abstains; the reader runs in a DB **read-only transaction**; if `some_fn` actually writes (`setval`) the \_engine*
...
- **A10 — The security TCB is small, explicit, budgeted, and exhaustively verified.** All enforcement lives in an
  enumerated set of choke functions (the _TCB manifest_, DEC-K) with a hard size budget; no security decision is made
  outside it (a lint fails on enforcement logic elsewhere). The TCB is verified to a higher bar than ordinary code —
  exhaustive property tests over every coercion/refusal path, plus model-checking (or a proof-assistant obligation)
  for the box non-coercibility and the egress refusal. Framework soundness reduces to **correctness of the TCB** (A10)
  **plus the sole-door proof** (A2/DEC-J). Growing the TCB past budget, or enforcing outside it, is a build failure.
  This is the "verify the chokes exhaustively" addition that ends the class instead of relocating it.
...
- **DEC-K — The TCB manifest + verification bar (A10).** Maintain `security/TCB.md` (a typed manifest) listing EVERY
  function in the security TCB: the read-only-connection wrapper, the declared-table writer wrapper, the
  `Secret`/`Untrusted` boxes + `reveal`, `emitToWire` + the egress-refusal helper, the renderer emit + escape, and the
  brand constructors. **Budget:** each choke ≤ ~150 lines; whole TCB ≤ ~600 lines. **Verification bar:** (i) exhaustive
  property tests enumerating every JS coercion path on the boxes and every value-shape into `emitToWire`; (ii)
  model-checking / a proof-assistant obligation for non-coercibility and egress refusal; (iii) a lint
  (`check:tcb-boundary`) that fails if a security decision (a throw/allow on a security property) appears outside the
  manifest, or if the TCB exceeds budget. This is the concrete, standing form of "verify the chokes exhaustively" —
  the highest-leverage item in the plan.
- **DEC-L — Narrow the authoring surface so the common path is statically sound and the chokes stay small (the
  "narrow waist").** Complementary to the chokes: restrict the static-blind channels in security-relevant positions so
  most apps are analyzable-by-construction at _zero_ runtime cost, shrinking what the chokes must handle to the
  explicit escape hatches. Concretely: raw SQL is not permitted in a query loader (only the typed builder, which is
  fully analyzable) — `sql.unsafe(...)` is an explicit, reviewed escape routed through the DEC-A/DEC-C runtime chokes;
  a trust-sink callee must be a statically-resolvable reference (no dynamically-computed brand callee). Extends the
  existing `check:sound-subset` to the whole security surface, failing closed on un-analyzable _code_ (not
  un-analyzable _values_). Every path then lands in exactly one bucket — sound-subset (statically proven, free) or
  escape hatch (runtime-confined) — eliminating the "un-analyzable AND un-confined" bucket where all seven rounds'
  bugs lived.
- **DEC-M — Publish a precise guarantee statement + explicit non-goals (sound claims).** Replace any broad "proven
  secure" language with the exact choke-backed invariants: e.g. "no value from a `secret`-classified column is
  serialized to any egress channel without an audited `reveal`"; "a reader handle performs no write, enforced by the
  database engine"; "no value reaches an executable render position without a proven-unforgeable trusted brand." State
  the threat model (DEC9-style adversaries) and explicit non-goals (timing side-channels beyond the constant-time
  primitives, author self-harm in their own server code, availability/DoS). The claim must be _narrower than or equal
  to_ what the TCB provably enforces; a claim with no backing choke is a documentation bug.
...
- [x] **0.1 Non-coercible `Secret<T>` + `Untrusted<T>` (A9b, DEC-C/DEC-I).** Module-private brands; coercion throws;
  ...
  - [x] **Non-coercion, redaction, and unforgeable brands.** `Secret`/`Untrusted` reject casual coercion and structural
        forgery, and redact in inspection/log-shaped paths.
        Evidence: `pnpm exec vitest --run packages/core/src/secret.test.ts packages/core/src/internal/wire-json.test.ts --reporter=dot`
  - [x] **Audited reveal and safe server-side operations.** Reason-bearing reveal and safe handling paths are covered
        without casual unwrapping.
        Evidence: `packages/core/src/secret.test.ts`
  - [x] **Log/status redaction floor.** Runtime secret values are refused or redacted in the current logging/task status
        egress floor.
        Evidence: `packages/server/src/secret-egress.ts`
- [x] **0.2 Paranoid mode harness (A9a, DEC-H).** `KOVO_PARANOID=1` stubs all static security classifiers to
  ...
  - [x] **Classifier stubbing.** Static security classifiers are forced advisory under `KOVO_PARANOID=1`.
        Evidence: `pnpm run check:paranoid-classifiers`
  - [x] **Runtime unsafe/legit twin harness.** Starter runtime checks show unsafe cases still hit runtime chokes while a
        legitimate route stays green.
        Evidence: `pnpm run check:paranoid-runtime`
  - [x] **CI enrollment.** The paranoid classifier/runtime checks are enrolled in GitHub Actions.
...
- [x] **0.3 Choke inventory + sole-door reachability gate (DEC-J/DEC-E).** The enumerated egress/exec sinks, each
  ...
  - [x] **Managed DB write-capable handles route through one SQL choke.**
        Evidence: `pnpm run check:single-choke`
  - [x] **Security-decision brand census is enforced.**
        Evidence: `pnpm run check:security-brands`
  - [x] **Fundamental-fixes census denominator is complete for current rows.**
        Evidence: `pnpm run check:fundamental-fixes-census`
- [x] **0.4 TCB manifest + boundary lint + verification harness (A10, DEC-K).** Create `security/TCB.md`, the
      ...
      whole TCB and they are verified" a mechanically-checked standing invariant, not a one-time claim. As each phase adds
      a choke, it is enrolled in the manifest and must pass the bar before that phase is "done."
  - [x] **Typed TCB manifest and budget.** `security/TCB.md` lists current runtime TCB entries separately from advisory
        static wrapper inventory and enforces line budgets.
        Evidence: `pnpm run check:tcb-boundary`
  - [x] **Boundary lint.** Manifest entries bind to real AST declarations and fail on unlisted or mismatched security
        wrappers.
        Evidence: `pnpm exec vitest --run scripts/check-tcb-boundary.test.mjs`
  - [x] **Finite-model property/proof harness.** Box non-coercibility and egress refusal are covered by finite modeled
        value/coercion cases.
  ...
  - [x] **Per-phase TCB enrollment.** Current runtime chokes are enrolled before their phase closures.
        Evidence: `security/TCB.md`
...
- [x] **1.1 DB read-only readers on a dedicated read-only pool (DEC-A).** Acceptance (paranoid mode): both dialects,
  ...
  - [x] **PGlite read transaction enforcement.** Read-only transactions reject write/DDL/lock attempts and allow
        legitimate read functions.
        Evidence: `vp exec vitest --run packages/server/src/managed-db.test.ts`
  - [x] **Dedicated Postgres/PGlite read-only pool/session.** Reader sessions set `default_transaction_read_only = on`
        on a dedicated pool rather than a shared writer-capable connection.
...
- [x] **1.2 Engine-enforced declared-table writes (DEC-B).** SQLite authorizer; Postgres request-scoped role
  ...
  - [x] **Managed write runtime choke and policy threading.** The runtime choke rejects undeclared `userx`,
  ...
  - [x] **Starter/test adapter declared-write fallback.** Generated PGlite/SQLite starter runtimes and test DB adapters
        expose declared-write handles that reject schema-qualified out-of-scope helper writes where native role/authorizer
  ...
  - [x] **SQLite test-harness authorizer enforcement.** File-backed SQLite declared writers use
        `sqlite3_set_authorizer` to deny writes to non-declared tables, trigger side-effect writes, DDL, and pragma at the
        engine, not only at the parser/choke layer.
        Evidence: `pnpm exec vitest --run packages/test/src/sqlite-harness.test.ts packages/test/src/pglite-harness.test.ts packages/server/src/managed-db.test.ts --reporter=dot`
...
- [x] **2.1 Tag secret values at the DB-read boundary (DEC-C 1/3).** Query-builder + view + computed + raw-SQL reads
  ...
  - [x] **Scaffold-declared custom secret columns and raw-SQL aliases.** Generated PGlite and SQLite runtime DB
        templates derive `kovo({ secret })` metadata and box custom secret columns at `context.db`.
  ...
  - [x] **Computed secret value runtime twin.** Values computed from runtime `Secret` reads are refused at query-wire
        egress with static KV435 stubbed.
...
- [x] **2.3 Every egress choke refuses `Secret` (DEC-C 4, DEC-J).** Wire, headers, redirect, static export, logs,
  ...
  - [x] **Current external-egress choke inventory refuses or redacts runtime `Secret`.** Wire JSON, framework headers,
        redirect `Location`, static export headers, logs, error reporting, and task status are covered at runtime sinks.
        Evidence: `packages/server/src/secret-egress.ts`
...
- [x] **2.4 Static KV435 → advisory (DEC-F, gated by A7).**
  ...
  - [x] **Diagnostic wording.** KV435 is advisory/defense-in-depth wording and names runtime secret/egress chokes.
        Evidence: `pnpm exec vitest --run packages/core/src/diagnostics.test.ts scripts/check-classifier-verdict-routing.test.mjs scripts/check-fail-closed-classifiers.test.mjs`
...
- [x] **3.1 `Untrusted` request tags (DEC-D, DX-only).** Accessors return tags; used for error messages, not soundness.
  - [x] **Body, route, and query request values are tagged and revealed by validation.** Parsed JSON/FormData leaves,
        route params/search, and query search inputs are tagged with `Untrusted`.
  ...
  - [x] **Kovo-owned header/cookie accessors tag values.** Request header/cookie accessors tag values, and CSRF
        ...
        Evidence: `vp exec vitest --run packages/server/src/csrf.test.ts -t 'origin|anonymous|cookie|CSRF'`
  - [x] **Request-shell accessor contract is explicit.** Native `Request.headers` remains the plain platform API by
        design; Kovo-owned accessors provide DX provenance rather than the soundness boundary.
        Evidence: `vp exec vitest --run packages/server/src/untrusted-request-body.test.ts`
- [x] **3.2 Contextual default-deny renderer over the final attribute set (DEC-D).** Escape-by-position; refuse at
  ...
  - [x] **Runtime renderer neutralizes URL/event/srcdoc/style sinks.** Runtime rendering removes, refuses, or escapes
        executable sinks with static KV426 stubbed.
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/server/src/jsx-runtime.test.ts`
  - [x] **Final attribute-set coverage for meta refresh and spreads.** `meta http-equiv=refresh content` and
        spread-delivered sinks are decided after the final runtime attribute set is known.
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/server/src/jsx-runtime.test.ts`
  - [x] **Synthetic future sink acceptance.** A generated future executable attribute fails closed or escapes under
        paranoid mode.
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/server/src/jsx-runtime.test.ts`
  - [x] **Trusted HTML/URL brands gate raw executable output.**
        Evidence: `pnpm run check:sink-policy`
- [x] **3.3 Static KV426 → advisory (DEC-F, gated by A7).**
  - [x] **Renderer choke is live with static checks stubbed.**
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/server/src/jsx-runtime.test.ts`
  - [x] **KV426/proof-scope diagnostics are advisory.**
        Evidence: `pnpm exec vitest --run packages/core/src/diagnostics.test.ts scripts/check-classifier-verdict-routing.test.mjs scripts/check-fail-closed-classifiers.test.mjs`
  - [x] **KV426 runtime-twin deletion proof.** The trusted-output/sink corpus fails at runtime with static KV426 stubbed
        to `proven-safe`.
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/server/src/jsx-runtime.test.ts`
...
- [x] **4.1 Choke unrepresentability (DEC-E).** Brand the constructors; reachability gate replaces shape recognition.
  - [x] **Current egress/DB-exec denominator has a sole-door reachability gate.**
        Evidence: `pnpm run check:single-choke`
  - [x] **`Secret`/`Untrusted` brands are module-private and non-coercible.**
        Evidence: `packages/core/src/secret.test.ts`
  - [x] **Response/wire-body constructor unrepresentability.** Query/mutation wire bodies require the framework-owned
        wire-body brand, with durable replay using reason-bearing audited rehydration.
  ...
  - [x] **DB exec constructor unrepresentability.** DB execution paths reach managed read/write chokes by type and
        reachability, not curated shape recognition.
        Evidence: `pnpm exec vitest --run packages/server/src/managed-db.test.ts packages/server/src/webhook.test.ts --reporter=dot`
  - [x] **Escape-hatch forgery rejection.** Shadowed `reveal`, `declareOffWire`, fake trusted brands, and bare casts
        fail to satisfy validating constructor paths.
...
- [x] **4.2 Reword static gates as advisory + fix proof scope (DEC-F/G).** Diagnostics name their choke; scanned roots
  ...
  - [x] **Classifier diagnostics/proof-scope wording is advisory.**
        Evidence: `pnpm exec vitest --run packages/core/src/diagnostics.test.ts scripts/check-classifier-verdict-routing.test.mjs scripts/check-fail-closed-classifiers.test.mjs`
  - [x] **Scanned source roots derive from security-marker imports.**
        Evidence: `pnpm exec vitest --run scripts/security-test-build-gate.test.mjs scripts/lib/source-files.test.mjs`
  - [x] **All KV406/KV426/KV433/KV435 messages name runtime chokes.**
        Evidence: `pnpm exec vitest --run packages/core/src/diagnostics.test.ts scripts/check-classifier-verdict-routing.test.mjs scripts/check-fail-closed-classifiers.test.mjs`
- [x] **4.3 Property-based generators (DEC-G).** Replace index enumerators with seeded grammar generators varying
  ...
  - [x] **KV426 trusted-output SINK-position generator.**
        Evidence: `pnpm run check:security-test-builds`
  ...
  - [x] **General wrapping grammar generator.** Seeded cases vary aliases, helpers, local wrappers, component props, and
        other wrapping forms across security proof surfaces.
        Evidence: `pnpm run check:security-test-builds`
  - [x] **Mutation harness kills deletion of generated enrollment.**
        Evidence: `pnpm run check:security-gate-mutations`
...
- [x] **5.1** SQLite scaffold declares/bundles `pgsql-ast-parser` (or the choke resolves it from `@kovojs/server`) so
  ...
  - [x] **SQLite scaffold dependency is declared and installed in scaffold checks.**
        Evidence: `pnpm exec vitest --run packages/create-kovo/src/index.build.scaffold.sqlite.test.ts -t 'pgsql-ast-parser|runs vp check' --reporter=dot`
  - [x] **Raw-SQL read/write live-choke assertion.** SQLite prod artifacts prove raw SQL reads succeed and raw-SQL writes
        are blocked because the parser/choke is live.
        Evidence: `KOVO_PARANOID=1 pnpm exec vitest --run packages/create-kovo/src/index.build.prod-artifact.security.test.ts -t 'boxes schema-declared secret reads' --reporter=dot`
- [x] **5.2** Drizzle view relation fixpoint terminates in bounded memory (`papercuts-26` P3); a `sqliteView`/`pgView`
  ...
  - [x] **View relation fixpoint termination.** Cyclic `sqliteView`/`pgView` relation derivation terminates and converges
        to the base read domain.
        Evidence: `vp exec vitest --run packages/drizzle/src/index.query-shapes.test.ts -t 'terminates cyclic.*View|Drizzle view'`
  - [x] **View-surfaced secret egress refusal.** A secret surfaced through a view is boxed and refused at egress with
        static KV435 stubbed.
        Evidence: `KOVO_PARANOID=1 vp exec vitest --run packages/create-kovo/src/index.build.prod-artifact.security.test.ts -t 'Drizzle view'`

### Phase 6 — Narrow waist, honest claims, external validation (complementary; land alongside 1–3)

- [x] **6.1 Narrow the security surface (DEC-L).** Disallow raw SQL in query loaders (typed builder only; `sql.unsafe`
      as an explicit escape through the DEC-A/DEC-C chokes); require statically-resolvable trust-sink callees; extend
      `check:sound-subset` to the whole security surface. Acceptance: the round-7 raw-SQL leak shape is a build error on
      the common path and, via the escape hatch, still refused at the runtime choke.
  - [x] **Common-path raw SQL is rejected in query loaders.**
        Evidence: `pnpm exec vitest --run packages/create-kovo/src/index.test.ts`
  - [x] **Trust-sink callees are statically resolvable.** Dynamically-computed trusted helper callees and alias laundering
        are rejected on the common path.
        Evidence: `pnpm exec vitest --run packages/create-kovo/src/index.test.ts --reporter=dot`
  - [x] **Whole starter security surface is enrolled.**
        Evidence: `pnpm exec vitest --run packages/create-kovo/src/index.test.ts --reporter=dot`
  - [x] **Explicit raw-SQL escape hatch remains runtime-confined.** A `trustedSql(sql.raw(...))` read path still boxes
        secret rows and is refused at query-wire egress under paranoid mode.
        Evidence: `KOVO_PARANOID=1 pnpm exec vitest --run packages/create-kovo/src/index.build.prod-artifact.security.test.ts -t 'boxes schema-declared secret reads' --reporter=dot`
- [x] **6.2 Publish the guarantee statement + non-goals (DEC-M).** A `SECURITY.md`/SPEC section states the exact
  ...
  - [x] **Guarantee statement.** `SECURITY.md` publishes the current choke-backed guarantees, threat model, and
        non-goals.
        Evidence: `SECURITY.md`
  - [x] **Machine-checkable claim backing.** The guarantee register rejects claims without TCB chokes or
        paranoid/runtime proof IDs.
        Evidence: `pnpm run check:security-guarantee`
  - [x] **No overbroad claims.** Published claims are constrained to the guarantee register backing.
        Evidence: `pnpm run check:security-guarantee`
- [x] **6.3 External adversarial audit + generative fuzzing (validation).** A non-implementer audit against the
      guarantee statement, plus the DEC-G property-based generators run under `KOVO_PARANOID`. Assurance, not soundness —
      it validates that the TCB + sole-door hold; it does not replace them.
  - [x] **Non-implementer guarantee audit.** The audit found no blocking guarantee without a choke, failed sole-door
        gate, or missing paranoid proof ID for current guarantees.
        Evidence: `plans/phase-6-3-security-guarantee-audit-20260702.md`
  - [x] **Paranoid generator validation.** DEC-G proof enrollment and paranoid runtime generator acceptance are green.
        Evidence: `pnpm run check:security-test-builds && pnpm run check:paranoid-runtime`
  - [x] **Residual-risk handoff.** Remaining assurance limits are recorded as validation risk, not replacement proof.
        Evidence: `plans/phase-6-3-security-guarantee-audit-20260702.md`
...
| Enforcement logic added _outside_ the TCB (a new server path decides a security property)            | the TCB stops being the whole boundary → a fail-open anywhere      | A10 + `check:tcb-boundary` lint (0.4, DEC-K)                         | 0.4 lint fails on out-of-manifest enforcement              |
| The TCB grows until it is no longer verifiable                                                       | soundness stops being _tractable to prove_                         | A10 size budget + DEC-K verification bar                             | 0.4 budget check + exhaustive TCB property tests           |
| Un-analyzable code in a security position (raw SQL / dynamic callee) on the common path              | the "un-analyzable AND un-confined" bucket returns                 | DEC-L narrow waist (6.1)                                             | 6.1 build error on the common path + choke on the escape   |
| A claim in the docs with no backing choke                                                            | marketing outruns enforcement (the round-1..7 credibility failure) | DEC-M guarantee statement (6.2)                                      | 6.2 test: every invariant names a choke + a paranoid proof |
...
- **The narrow waist costs expressiveness.** DEC-L forbids raw SQL in loaders and dynamically-computed trust callees;
  some authors will resent it. That is the intended shape — the un-analyzable thing must not be casual, the escape
  hatch is explicit and runtime-confined. Net: most apps get build-time proof at zero runtime cost, and the TCB stays
  small because it only handles the escape hatches.
- **The TCB has a real maintenance discipline.** A hard size budget + a boundary lint means new features cannot smear
  enforcement back across the codebase; a genuinely new sink category costs a new _choke_ (enrolled + verified), not a
  new recognizer. This is friction by design — it is what keeps "is Kovo sound?" a bounded question.
...
Two standing conditions make that acceptance _durable_: **the TCB stays within budget and exhaustively verified**
(A10/DEC-K — so soundness remains tractable to _prove_, not merely to test), and **an external adversarial audit
against the published guarantee statement** (DEC-M/6.3) finds nothing the TCB doesn't already refuse. When those hold,
"is Kovo sound?" is a bounded, checkable claim about a few hundred verified lines behind a proven-sole door — not a
hope about an unbounded authoring surface. That is the finish line the previous seven rounds never had.

## Latest verification

- Plan-ledger cleanup only on 2026-07-02; no framework behavior or `SPEC.md` changed. Current proof/gap evidence is
  nested under the child checkbox it supports.
