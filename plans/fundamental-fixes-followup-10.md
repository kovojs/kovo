...
- [ ] **DEC-A:** app-SQL `SET kovo.role='admin'`/`'system'` reads nothing (no policy consults the GUC); framework crossOwnerRead via `kovo_admin` role still works + is audited.
- [ ] **DEC-B:** app-reachable `SET kovo.principal='x'`, `RESET ROLE`, and node-pg simple `RESET ROLE; set_config(...)` are all rejected on query/mutation/task/webhook/escape paths.
- [ ] **DEC-C:** a session GUC set on one pooled borrow is absent on the next.
- [ ] **DEC-D/F:** a definer trigger on an owner table makes `kovo db check` REFUSE; the fuzzer's mechanism axis turns RED on a re-introduced B3.
