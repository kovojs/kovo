...
- [x] **Generated starter check is green while production build preflight is red.**
  - Evidence: `pnpm exec vitest run packages/create-kovo/src/index.build.test.ts --testNamePattern "production build graph gate" --run` proves unchanged generated starters run the production build preflight from the check path and pass the graph gate.
