...
- [x] Merge `site/content/guides/diagnostics.md` into generated Reference.
  - Evidence: `site/content/guides/diagnostics.md` was removed; `site/scripts/diagnostics-ref.mjs`
    continues to generate `/reference/diagnostics/`, and `pnpm --filter @kovojs/site build`
    emitted `diagnostics-ref/v1 codes=67`.
...
- [x] Merge the three example walkthroughs into the live example pages.
  - Evidence: `site/src/examples.ts` now renders walkthrough notes into
    `/examples/{commerce,crm,stackoverflow}/`; the old `example-*.md` guide files were removed, and
    `pnpm --filter @kovojs/site check:links` passed after export.
...
- [x] Merge the two app-pattern guides into those example pages or a single "App patterns" guide.
  - Evidence: CRM and Stack Overflow pattern sections are embedded in their example page notes in
    `site/src/examples.ts`; `app-pattern-*.md` files were removed.
...
- [x] Merge `starter-to-app.md` into Quickstart.
  - Evidence: `site/content/docs/quickstart.md` now has "Extend the starter safely" with the module
    sequence, auth/route guidance, and `vp check`/`vp test`; `starter-to-app.md` was removed.
...
- [x] Merge `package-imports.md` into Components and Stability.
  - Evidence: `site/content/guides/components.md` owns UI/headless/icon subpath guidance, and
    `site/content/docs/stability.md` owns broader public import boundaries; `package-imports.md` was
    removed.
...
- [x] Move useful evidence notes into their owning guides and keep the rest repository-only.
  - Evidence: static-export checks were added to `site/content/guides/static-export.md`,
    browser-integration guidance to `site/content/guides/testing.md`, and worked add-to-cart to
    `site/content/guides/mutations.md`; all `site/content/evidence/*.md` files were removed from the
    site corpus.
