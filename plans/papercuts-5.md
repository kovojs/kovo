...
- [x] **Fresh linked starter `pnpm run check` fails at `build:prod` because the virtual undici runtime cannot resolve `undici`.** (high, framework/tooling; found by baseline)
  ...
  - Fix evidence: `pnpm exec vitest --run packages/create-kovo/src/index.build.test.ts` includes an out-of-repo generated production build graph gate, proving `build:prod` no longer needs app-local `undici`; `pnpm exec vitest --run packages/cli/src/index.kovo-build.test.ts -t "boots emitted node preset output"` proves the emitted handler still boots from production dependencies without `createRequire(import.meta.url)("undici")`.

### B. Static Export

- [x] **Manifest-backed static export omits Vite `public/` assets referenced by exported HTML.** (med, framework/tooling; found by `static-export`)
  ...
  - Fix evidence: `pnpm exec vitest --run packages/server/src/static-export-assets.test.ts packages/server/src/vite-static-export-options.test.ts packages/server/src/static-export-route-guards.test.ts packages/server/src/vite-export-replay.test.ts packages/server/src/vite-build.test.ts` proves document-referenced public assets are copied from the Vite dist public root and missing public assets fail with KV229.

- [x] **`kovo export --skip-non-exportable` writes partial output but still exits 1 for skipped-route warnings.** (low, CLI; found by `static-export`)
  ...
  - Fix evidence: `pnpm exec vitest --run packages/cli/src/index.kovo-export.test.ts` proves skip-mode KV229 warnings now exit `0` after selected artifacts are written, while export diagnostics still print as warnings.

### C. UI Copy-In

- [x] **`kovo add dialog` copies a component that imports `@kovojs/headless-ui/dialog` without adding or declaring the required app dependency.** (med, CLI/UI; found by `ui-components`)
  ...
  - Fix evidence: `pnpm exec vitest --run packages/cli/src/index.kovo-add.test.ts` proves `kovo add dialog` prints `DEPENDENCIES status=missing packages=@kovojs/headless-ui install="pnpm add @kovojs/headless-ui"` when the nearest app manifest lacks the copied component's required Kovo package dependency.
...
- `pnpm exec vitest --run packages/create-kovo/src/index.build.test.ts`
- `pnpm exec vitest --run packages/cli/src/index.kovo-export.test.ts`
- `pnpm exec vitest --run packages/cli/src/index.kovo-add.test.ts`
- `pnpm exec vitest --run packages/server/src/static-export-assets.test.ts packages/server/src/vite-static-export-options.test.ts packages/server/src/static-export-route-guards.test.ts packages/server/src/vite-export-replay.test.ts packages/server/src/vite-build.test.ts`
