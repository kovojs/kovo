...
- [x] **B1 — Production client-island modules import `@kovojs/browser/generated`, so islands deploy and load but never hydrate in the `kovo build` artifact.** (high, framework/prod artifact; found by `dogfood-islands-deploy`)
  ...
  - Evidence: `pnpm exec vitest run packages/cli/src/index.kovo-build-browser.test.ts --run --reporter=dot` builds a production artifact, serves `dist/server/server.mjs`, verifies the browser-imported `/c/__v/.../counter-island.client.js` has inline generated helpers instead of `@kovojs/browser/generated`, and proves click/input local state updates without `/_m` or `/_q`.
...
- B1 closure: `pnpm exec vitest run packages/cli/src/index.kovo-build-browser.test.ts --run --reporter=dot`; `pnpm exec vitest run packages/compiler/src/vite.test.ts --run --reporter=dot`; `pnpm run check:vp`; `git diff --check`.
