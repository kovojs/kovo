...
| Is the generated touch-graph wired into the runtime? | **No** | The generated `touch-graph.ts` feeds `kovo explain`/devtools/conformance only; example `graph.ts` imports the `TouchGraph` *type*, not the runtime invalidation path. |
...
| Static / compile-time enforcement of read-set coverage? | **No** | No KV407/readset check in `packages/compiler/src`. KV411 (`Query read set includes an exempt table`) checks declared `reads` against *exempt* tables only (`packages/drizzle/src/static.ts:86`), not against the `load` AST. |
