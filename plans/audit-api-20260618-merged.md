...
**Lens:** Conservative JS framework API review — *make it internal unless we really need it public*.
...
1. **`@kovojs/ui` stays a versioned public library** (owner override) — *not* downgraded to copy-in
...
demoted to `internal`/`generated` or removed. The largest *retained* public surface is now
...
1. **`@kovojs/ui`: drop the 32 `export *` lines** (≈845 re-exported headless symbols). This is the
   ...
   exports). Components import the headless symbols they use *by name*; do **not** re-export. **Sequence:**
...
- **Open product question (assumed resolved):** `@kovojs/ui` dual distribution (versioned + copy-in). If
  versioned-*only*, also remove `registry.json` and the `kovo add` copy-in path; this report assumes
  dual and keeps both.
