...
- [x] **`headless-ui` machinery types: DOCUMENT/narrow, do NOT internalize.** Per-primitive `tsc`-assisted pass — document the `*State`/`*ChangeResult`/`*PrimitiveAttributes`/… types forced by kept `*Attributes`/handler signatures; narrow the open `Record<string, boolean|string>` tail of `*PrimitiveAttributes` to known keys; demote only the genuinely-unreferenced ones. (Overturned-from-remove; ~430 upper bound on review scope.)
  - Evidence 2026-06-29: `@kovojs/headless-ui/types` documents the shared primitive machinery types and `OtpField*Target` event targets are public where public OTP events name them; the current recursive-publicness baseline has zero `@kovojs/headless-ui` entries. `pnpm --filter @kovojs/headless-ui exec vitest --run --reporter=dot`, `pnpm --filter @kovojs/headless-ui run lint:primitives`, `pnpm run check:api-surface`, `pnpm run check`, and `git diff --check` passed.
...
### 9E — Re-tier the primitive ABI

- [x] **PREREQ — generated handler ABI + L1 authoring story.** Add an `apiBoundary.generated` tier to `@kovojs/headless-ui` so emitted client modules import handlers from `generated` (not the human-public root), and move the L1 island-authoring path off hand-imported reducers (the gallery previously hand-imported them). Required because `rules/api-surface.md` bars emitted code from `internal`. (Standing blocker recorded in the `plans/archive.md` entry for `api-cleanup-leftover`.)
  - Evidence 2026-06-29: `public-packages.json` classifies `@kovojs/headless-ui` `./generated`, `packages/compiler/src/handler-lowering.test.ts` asserts emitted client imports are rewritten to `@kovojs/headless-ui/generated`, and `examples/gallery/src/primitive-actions.ts` moves interactive gallery demos to a local L1 adapter; `pnpm --filter @kovojs/example-gallery test`, `pnpm run check:api-surface`, and `pnpm run check:publish` passed.
- [x] **Demote the 103 current `@kovoPrimitiveHandler` functions** to the new `generated` tier / tag `@generated` (`switch.ts:81` et al.). Keep importable for the compiler; off the SemVer/docs surface.
  - Evidence 2026-06-29: `packages/headless-ui/src/generated.ts` exports the 103 tagged primitive handlers, the public wrapper scan found `publicHandlers: []`, and `pnpm --filter @kovojs/headless-ui exec vitest --run --reporter=dot`, `pnpm --filter @kovojs/headless-ui run lint:primitives`, and `pnpm run check:api-surface` passed.
- [x] **Demote the 89 current reducer-style helpers** (`set*`/`toggle*`/`*Move`/`*Typeahead`/`*RovingIndex` and transition helpers) to `@kovojs/headless-ui/internal/primitive`.
  - Evidence 2026-06-29: `packages/headless-ui/src/primitive-internal.ts` exports the reducer helper tier, the public wrapper scan found `publicReducers: []`, and `pnpm --filter @kovojs/example-gallery test`, `pnpm --filter @kovojs/headless-ui exec vitest --run --reporter=dot`, and `pnpm run check:api-surface` passed.
