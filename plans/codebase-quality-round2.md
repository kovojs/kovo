...
The shared source replacement result type is now named `SourcePatchWithOffsetMap`, making offset-map
carrying explicit for the model-patch pass instead of looking like a generic patch result.
...
packages/compiler/src/compile-component.test.ts`; `pnpm exec tsc --noEmit --pretty false`; exact
  `pnpm exec vp check packages/compiler/src/shared.ts plans/codebase-quality-round2.md`;
  `git diff --check`.
...
packages/compiler/src/compile.ts plans/codebase-quality-round2.md`; `git diff --check`.
- Server-render lowering result wrapper deletion: `pnpm exec vitest --run
packages/compiler/src/stamps.test.ts packages/compiler/src/compile-component.test.ts`;
  `pnpm exec tsc --noEmit --pretty false`; exact `pnpm exec vp check
packages/compiler/src/emit/server.ts packages/compiler/src/stamps.test.ts packages/compiler/src/compile.ts
plans/codebase-quality-round2.md`; `git diff --check`.
...
- `pnpm run check` passed after checkpoint `497c1520`: inline loader check, 841
  formatted files, 741 lint/typechecked files, and 7 typechecked example/conformance projects.
- `pnpm run test` passed after checkpoint `497c1520`: 247 files and 2460 tests.
- `pnpm run test:browser` passed after checkpoint `497c1520`: 5 browser files and 16 tests.
- `pnpm run test:conformance` passed after checkpoint `497c1520`: Drizzle, Better Auth,
...
- UI/gallery H2 radio-group native form slice:
  ...
  `git diff --check`.
- UI/gallery styled radio-group static ARIA/form-owner slice:
  `pnpm exec vitest --run packages/ui/src/index.test.tsx -t radio-group`;
  ...
  `git diff --check`. Evidence: the styled static radio-group route now has a synchronized
  `data-ui-demo` wrapper, hidden label/error IDREFs, invalid/required radiogroup state, stable
  visual geometry/hash, and Chromium-backed external form-owner `FormData` checks for enabled and
  disabled native radios.
- UI/gallery H2 toggle-group keyboard/focus slice:
  ...
  `git diff --check`.
- UI/gallery H2 roving-control static visual-baseline slice:
